...
The table above is the original 1.8 path. The supplied Spectron 2.2 native
library keeps the same three modes and the same `quattroplay.com` suffix, but
it changes the host pair to `cong.quattroplay.com` and
`cong2.quattroplay.com`:

| Mode | First endpoint | Retry endpoint | Transport |
| ---: | --- | --- | --- |
| 1 | `https://cong.quattroplay.com/con.png` | `https://cong2.quattroplay.com/con.png` | HTTPS |
| 2 | `https://cong.quattroplay.com/con.gs` | `https://cong2.quattroplay.com/con.gs` | HTTPS |
| 3 | `http://cong.quattroplay.com/conf.gs` | `http://cong2.quattroplay.com/conf.gs` | HTTP |

This is a native finding, not a guess based on a string search. The target
`TServerList_enterNextConnectorMode_int` is at `0x2094c0`; it decodes the
first host from four bytes that produce `cong` and the retry host from five
bytes that produce `cong2`. The domain and path fragments decode identically
to 1.8. The complete raw-byte record and decoder are in
`artifacts/spectron_connector_endpoint_audit_20260827.json` and
`tools/audit_spectron_connector_endpoints.py`.

The target therefore has two independent compatibility questions before a
login can succeed. Its endpoint routing is different from 1.8, and its HTTPS
connector still uses the historical native trust bundle. This report does
not resolve either hostname or contact a service, so it does not claim that
`cong` is live or that its certificate matches the embedded trust material.
