...
Two nearby application helpers remain deliberately unresolved. Target
`0x24a9ec` is registered as `getsignature`, but its body walks through the
Android package and signature objects in the current pseudocode review. The
target table calls `0x24b958` `getinstallerpackagename`, while that body also
walks through a package signature and calls `toCharsString()`. Both are kept
visible as evidence conflicts instead of being assigned a guessed name. The
remaining app-facing default at `0x250090` also needs a focused review before
it is named.
