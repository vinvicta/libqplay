...
## 2026-08-28: Spectron FreeType autofit callback block

After the glyph-loader pass, the next contiguous default-name region was the
FreeType autofit block. The source and target functions again line up with a
`0xd470` displacement. This block is especially useful for translation because
the original binary retains the script and module class records. Those records
tell us which short routines are callbacks, even when their bodies are nearly
identical.

| 1.8 source | Spectron target | Applied label | Role evidence |
| ---: | ---: | --- | --- |
| `0x267ecc` | `0x27533c` | `v18_tt_driver_init` | TrueType module `module_init` slot |
| `0x267ef0` | `0x275360` | `v18_af_dummy_hints_init` | dummy script `hints_init` slot |
| `0x267f08` | `0x275378` | `v18_af_dummy_hints_apply` | dummy script `hints_apply` slot |
| `0x267f10` | `0x275380` | `v18_af_latin_hints_init` | Latin script `hints_init` slot |
| `0x267f90` | `0x275400` | `v18_af_latin2_hints_init` | Latin2 script `hints_init` slot |
| `0x268010` | `0x275480` | `v18_af_cjk_metrics_scale` | CJK script `metrics_scale` slot |
| `0x268050` | `0x2754c0` | `v18_af_cjk_hints_init` | CJK script `hints_init` slot |
| `0x2680c0` | `0x275530` | `v18_af_latin2_hints_compute_segments` | Latin2 metrics and apply callers |

The first function is the small TrueType driver initializer. Its only call is
to `TT_New_Context`; the failure branch returns the FreeType context error,
which matches the upstream `tt_driver_init` implementation. The dummy pair is
also straightforward. The initializer copies the active metrics state into
the hint object, while the apply callback is a successful no-op for scripts
that have no specialized hinting rules.

The Latin and Latin2 initializer bodies are almost indistinguishable when
viewed without their tables. Their class records resolve the roles cleanly:
the Latin callback belongs to the Latin script record and the other belongs to
the Latin2 record. The CJK scaling helper copies the scaled metric fields and
sets the values consumed by the CJK hint initializer. These table assignments
are stronger evidence than names inferred from a short function body alone.

The long function at source `0x2680c0` and target `0x275530` is
`af_latin2_hints_compute_segments`. Its loop structure builds the 88-byte
segment records used by the Latin2 hinter, and its callers are the Latin2
metrics-width probe and the hint-application routine. That call graph rules
out the nearby edge and outline helpers and identifies the operation as
segment analysis.

All eight source and target functions match the complete recorded ARM64
feature set: instruction count, mnemonic sequence, control-flow shape,
register shape, and the remaining exported feature fields. The aliases were
applied to the v310 IDA copy, closed, reopened, and checked with zero failures.
The database has 11,695 functions and 410 remaining default `sub_` names. Its
SHA-256 is
...
generated by `tools/generate_spectron_freetype_autofit_anchors.py`. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v310.json`. The anchor
record is offline-only and reports `network_contacted: false`. The source role
references are Android FreeType's [`ttobjs.c`](https://android.googlesource.com/platform/external/freetype/+/2689da543c08133100124cab3ab19523b04f2f3d/src/truetype/ttobjs.c),
...
and [`afcjk.c`](https://android.googlesource.com/platform/external/freetype/+/2689da543c08133100124cab3ab19523b04f2f3d/src/autofit/afcjk.c).
