...
function matching in another release.
...
## Method-level DEX surface pass

The earlier DEX review used string-table indicators and a small native-method
inventory. The follow-up parser now resolves the original `classes.dex`
indexes, class data, code items, and `invoke-*` references without executing
the APK. Its record is
`artifacts/original_dex_method_surface_review_20260904.json`, generated by
`tools/audit_dex_method_surface.py`.

The DEX is version 035 and contains 1,889 class definitions, 13,426 defined
methods, 11,952 methods with code, and 18 public native methods in
`com.quattroplay.GraalClassic.Natives`. The parser decoded all code items with
zero bounds or format errors. Fourteen of the native methods have 21 direct
bytecode call instructions. The four native exports without a direct DEX
caller remain `onAddScriptFunction`, `onRegisterEvent`, `onVideoFinished`, and
`onVideoLoaded`, matching the narrower callback review.

The resolved Java-to-native boundary gives a clean startup chain. The
renderer calls `System.loadLibrary`, then `Natives.QPlayMain` from
`QPlayRenderer.loadLibrary`; its frame method calls `Natives.QPlayLoop`, and
surface, input, lifecycle, purchase, and event work is posted back to the
native callbacks from the activity and GL thread. This is stronger evidence
than a name-only search because each edge comes from an `invoke-*` instruction
in a defined method body.

The same pass found four application-owned WebView call paths. The activity
enables JavaScript in `QPlayActivity$27`, loads native-supplied HTML in
`QPlayActivity$31`, loads a URL in `QPlayActivity$32`, and reloads a supplied
URL in `InsideWebViewClient.shouldOverrideUrlLoading`. The only resolved
`addJavascriptInterface` caller is the bundled Bolts app-link resolver, not
the game activity. Its bridge therefore remains a separate, narrower surface.
The application-owned code also reads the incoming intent URI in
`QPlayRenderer.loadLibrary`; the resolved calls are `Intent.getData` and
`Intent.getScheme` before native startup.

Across application and bundled dependency code, the parser recorded 175
resolved sensitive API call instructions. The largest groups are reflection
70, intent reads 56, preferences 15, network URL operations 15, WebView
operations 10, activity launches 5, device-identifier reads 2, and native
library loading 1. These counts describe call references, not proof of remote
reachability. They separate the old Android support, Facebook, Bolts, and
billing libraries from the four application-owned WebView calls, making the
security review less likely to treat third-party compatibility code as a game
connector path. No DEX code was executed and no endpoint was contacted.
