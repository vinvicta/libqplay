...
The next stripped region after the v308 runtime-tail pass was a compact
FreeType block whose source and target addresses differ by `0xd470`. I first
checked the function boundaries and feature export, then inspected the source
and target pseudocode and the call sites in `TT_RunIns`. This mattered because
the three glyph-loader functions are adjacent but have different public and
internal roles.

The recursive function at source `0x262db4` and target `0x270224` is
`load_truetype_glyph`. It takes the loader, glyph index, recursion count, and
header-only state. Its body checks component depth and glyph bounds, reads the
loca and glyf data, loads horizontal and vertical metrics, chooses the simple
or composite outline path, and recursively loads component glyphs. The
surrounding source names `TT_Load_Simple_Glyph` and
`TT_Load_Composite_Glyph` were already present, which gave the large function
the expected position in the glyph-loading call chain.

The 4,700-byte function at source `0x263d1c` and target `0x27118c` is
`TT_Load_Glyph`. It initializes the temporary loader and execution context,
calls the recursive glyph loader, moves the resulting outline or composite
records into the glyph slot, computes the final metrics, and cleans up the
temporary buffers. The source and target feature records match in every field
except `register_detail_hash`, which is recorded as a compiler-allocation
difference.

The short wrapper at source `0x264f78` and target `0x2723e8` is
`tt_glyph_load`, the TrueType driver's `Load_Glyph` callback. Its argument
order explains why it calls the larger function with the first two arguments
swapped. It validates the size, face, and glyph index, normalizes the
no-hinting, no-scale, and no-bitmap flags, and forwards the request to
`TT_Load_Glyph`. This matches the FreeType driver callback contract rather
than being a second copy of the loader implementation.

The four interpreter helpers complete the same source block. The helper at
`0x264fcc` and `0x27243c` is `Ins_SxVTL`, called from `TT_RunIns` at source
call sites `0x266650` and `0x266674` for the SPVTL and SFVTL opcode pairs.
It reads two points, applies the opcode's rotation variant, normalizes the
resulting vector, and reports point-bound errors. The helper at `0x26521c`
and `0x27268c` is `Ins_CALL`, reached from opcode `0x2b` at call site
`0x265d84`. The helper at `0x265370` and `0x2727e0` is `Ins_LOOPCALL`,
reached from opcode `0x2a` at `0x265d70`. Both resolve active function
definitions, create a call record, and transfer control into the definition's
code range.

The final helper at `0x2654d4` and `0x272944` is `Ins_UNKNOWN`. It is reached
by the undefined-opcode fallback at `0x2678a0`, scans active IDEF records for
the current opcode, and either enters the custom instruction body or reports
an invalid opcode. The official FreeType source names this helper
`Ins_UNKNOWN`, which is more precise than describing it as an arbitrary
fallback routine.

All seven source and target functions match normalized feature shape. Six
match the complete recorded ARM64 feature set, and only `TT_Load_Glyph` has a
register-detail-only difference. The reviewed aliases were applied to a new
IDA copy, closed, reopened, and verified with zero failures. The v309 copy
has 11,695 functions and 418 remaining default `sub_` names. Its SHA-256 is
...
`artifacts/spectron_translation_checkpoint_20260828_v309.json`. The source
role references are the Android FreeType implementations of
...
This pass changed only the disposable IDA database and the public research
archive. It did not patch the APK or either native library, and it performed
no DNS, HTTP, or TLS operation.
...
## 2026-08-28: Spectron short residual property aliases

The v262 pass translates six callbacks from three short property runs that
were left outside the longer class tables. The groups are the
`GuiButtonCtrl.stylesection` row at source `0x3804f0` and target `0x393550`,
the `TScriptProperty.scriptlogwritetoreadonly` row at source `0x387d90` and
target `0x39aee0`, and the `TTiles.waterheight` row at source `0x387e78` and
target `0x39afc8`. The records use the standard 0x30-byte layout, with getter
and setter pointers at offsets `0x10` and `0x18`.
...
The style-section pair copies or assigns the control string at +504. The
script-log pair reads or writes its global boolean flag. The water-height pair
reads or writes the `TTiles` static double. The target pseudocode preserves all
three operations with obfuscated helper and global names.

All six target callbacks were default `sub_` functions before this pass. The
decoded property names, table-local roles, direct field or static-global
behavior, and ARM64 feature records agree. Every row has an exact normalized
shape. The style-section pair has complete metric matches. The four script-log
and water-height callbacks differ only in register-detail hashes caused by
...
`analysis/spectron_libqplay_translated_v262.i64`, which contains 11,696
...
## 2026-08-28: Corrected GUI and Android bridge translation pass

The next pass moved from property tables into the application-facing callback
and Android bridge regions. It produced two checked-in IDA revisions. The
first revision records three cross-build correspondences. The second adds 22
target-only labels to the stripped 2.2 database where the target script table,
Java method strings, and decompiled body provide enough evidence to name the
function without pretending that a 1.8 symbol survived.
...
All three are high-confidence semantic matches. None has an exact normalized
feature match because Spectron rebuilt the helper and wrapper classes, so the
artifact records the layout and register differences explicitly. The final
reopen verification found all three names with zero failures. The saved copy
is `analysis/spectron_libqplay_translated_v263_corrected.i64`, with 11,696
functions and 734 default `sub_` names. Its SHA-256 is
`be53b6e48e2156630ce3ae418fe0da388fd11405ef093dad80d93e0cc06df1b0`.

The machine-readable anchor artifact is
`artifacts/spectron_gui_android_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_android_bridge_anchors.py`. Its
SHA-256 is
`5f2165f742df30ae89579c1eb070370f45d4ee31595b0a16cc5b91511bf8ebdc`.
The corresponding checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v263_corrected.json`.

### v264 target-only Android and JNI labels

The target application bridge occupies a mostly unnamed block from
`0x24b4ec` through `0x2531ec`. The decoded script table supplies names such
as `getandroidversioncode`, `registerforpushnotifications`, and
`firebase::addeventdata`. The native bodies add a second layer of evidence by
containing the exact Java method names and JNI calls. The final v264 pass
applies 22 `spectron_` labels covering five groups:

- Deep-link and push data: `GetIntentData`, `ClearIntentDatas`,
  `GetPushNotificationData`, and `ClearPushNotificationData`.
- Android identity and version helpers: version name, version code, generic
  static string and integer calls, and the system-property bridge.
- Google Play integration: service availability, in-app review, licensing,
  and notification permission.
- Firebase and notifications: topic subscribe or unsubscribe, event data,
  event logging, notification creation, and notification clearing.
- Key and certificate setup: the signing-certificate and Google Play key
  setters.

The exact address-level list is in
`artifacts/spectron_android_bridge_target_only_labels_20260828.json` and
`docs/SYMBOLS.md`. Every one of the 22 target functions began as a default
`sub_` row in the pre-pass feature export. All 22 were applied and verified
after reopening the database. The v264 copy contains 11,696 functions and
...
generated by `tools/generate_spectron_android_bridge_labels.py`. The v264
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v264_corrected.json`.
This was an offline IDA operation. It did not alter the APK or native library
and did not contact a DNS, HTTP, TLS, Google Play, Firebase, or game-server
endpoint.

## 2026-08-28: Spectron legacy Android and security helper pass

The next review used the 1.8 Android callback table as a bridge into the
stripped 2.2 library. This was useful for the small functions that do not
share instruction metrics because Spectron rebuilt its Android and string
wrappers. The table context and the decompiled operation still give a clear
connection.

### v265 legacy Android anchors

Four high-confidence aliases were applied to the v265 disposable database:

| 1.8 role | Source | Spectron target | Target table context | Reviewed operation |
| --- | ---: | ---: | ---: | --- |
| `MainAndroid_script_settapjoysecret` | `0x2401f4` | `0x24a240` | `0x39e788` | stores the second TapJoy credential string |
| `MainAndroid_script_settapjoyapplicationid` | `0x240204` | `0x24a254` | `0x39e758` | stores the first TapJoy credential string |
| `JNI_connectToTapJoyService` | `0x2410ac` | `0x24c7e4` | `0x39e7b8` | calls Java `connectToTapJoyService([B[B)Z` |
| `androidGetID_void` | `0x2435e8` | `0x2502f4` | not registered | reads `Settings.Secure.ANDROID_ID` |

The first two target callbacks write the strings that the connector later
converts to Java byte arrays. The connector resolves the Java method, invokes
it, releases both local arrays, and returns the boolean result. The Android ID
helper follows the ActivityThread to the application content resolver before
reading the secure setting. The target uses rebuilt helper classes and
different register allocation, so all four rows have layout and register
detail differences. None has an exact normalized or full metric match. The
semantic evidence is nevertheless direct and each name reopened successfully.

The machine-readable artifact is
`artifacts/spectron_android_legacy_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_android_legacy_anchors.py`. Its SHA-256
is
`cfa260c633ceba20a5e6f3469706d1565dea59ade5068b7b279c73df1343d8d2`. The
reopened v265 database is
`analysis/spectron_libqplay_translated_v265.i64`, with 11,696 functions and
709 default `sub_` names. Its SHA-256 is
`89eafac9cb7b6cba867fa6e39c0fd6e6814a0b45b586e5d75238b2502e566e61`. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v265.json`.

### v266 target-only Android and anti-instrumentation labels

The v266 pass names five target functions that do not have demonstrated 1.8
counterparts:

| Spectron target | Applied label | Evidence |
| ---: | --- | --- |
| `0x24a1d8` | `spectron_getandroidabi` | target script table callback `getandroidabi` |
| `0x2500ec` | `spectron_android_getjavaclassexists` | target script table callback and Java class lookup |
| `0x2501f0` | `spectron_android_getjavafuncexists` | target script table callback and Java method lookup |
| `0x24a188` | `spectron_setactiononfridadetected` | target script table callback and action global used by Frida loops |
| `0x24a2ac` | `spectron_frida_detection_sleep_loop` | nonreturning nanosleep loop called by `DetectFridaLoop1` |

The retained target names `DetectFridaLoop2` at `0x24a1ac` and
`DetectFridaLoop1` at `0x24a2e8` make the last row especially relevant to
runtime debugging. It is a description of observed control flow, not proof
that anti-instrumentation is the reason the client fails to connect or start.
The table also contains generic Java existence checks, which may help explain
why the target can take different startup paths depending on the installed
package and Java environment.

The target-only artifact is
`artifacts/spectron_android_security_target_only_labels_20260828.json`,
generated by `tools/generate_spectron_android_security_labels.py`. Its SHA-256
is
`63dc1f01af72d7c9557f65ef2f53134139c3d41554b34a84a6ec4f2a04616fc3`. The
reopened v266 database is
`analysis/spectron_libqplay_translated_v266.i64`, with 11,696 functions and
704 default `sub_` names. Its SHA-256 is
`1d9a96fb4db2f9ee1d6353ca2ea94deb30ae5e03085c2bf8ca1286a89f99616f`. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v266.json`.

The initial v266 artifact had two table-role mistakes that were held for a
focused review. It assigned the `getjavaclassexists` description to
`0x2500ec`, even though the raw row at that callback is
`getstaticjavafuncexists`, and it omitted the adjacent `0x250090` class
lookup callback. It also described the package helpers too broadly. The
correction passes below record the direct table bytes and the decoded Java
method paths.

### v267 correction for the Java existence helpers

The raw target rows at the end of the Android helper table are adjacent and
easy to shift by one row when reading the callback cells. Rechecking the
encoded names gives this corrected mapping:

| Target | Correct table name | Corrected label | Callback cell |
| ---: | --- | --- | ---: |
| `0x250090` | `getjavaclassexists` | `spectron_android_getjavaclassexists` | `0x39e4d8` |
| `0x2500ec` | `getstaticjavafuncexists` | `spectron_android_getstaticjavafuncexists` | `0x39e4a8` |
| `0x2501f0` | `getjavafuncexists` | `spectron_android_getjavafuncexists` | `0x39e478` |

The `0x250090` body calls the JNI class lookup and returns whether a class
object was found. The `0x2500ec` body calls the static-method lookup and
returns whether a method ID was found. The `0x2501f0` body performs the
corresponding instance-method lookup. The v267 IDA copy applies the corrected
names and reopens with zero failures. It contains 11,696 functions and 703
default `sub_` names. Its SHA-256 is
...
generated by `tools/generate_spectron_android_security_labels.py`. Its SHA-256
is
`cb87a0e2bd483d2f8c1791c2a5731a9c89a4d34b6ce3a258be26b7cb4b0a2ecc`. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v267.json`.

### v268 package-signature helper

The target table entry at `0x39f0f0` points to `0x24a9ec` and decodes to
`quattro::android::getsignature`. The target body now makes the role clear.
It obtains the application package name, calls
`PackageManager.getPackageInfo(packageName, 0x40)`, reads `signatures[0]`,
and calls `toCharsString()`. If the signature object is absent, it returns
the script string `null`. This is a package-signature path, not an
`ANDROID_ID` path. The nearby `0x24b958` callback is a separate
`getinstallerpackagename` implementation that calls
`PackageManager.getInstallerPackageName(packageName)`.

The v268 IDA copy applies the target-only label
`spectron_quattro_android_getsignature` at `0x24a9ec` and reopens with zero
failures. It contains 11,696 functions and 702 default `sub_` names. Its
SHA-256 is
...
generated by `tools/generate_spectron_android_package_identity_labels.py`.
Its SHA-256 is
`ca3c390cdb7309f7ce623a3d069d2e438f2728829ca144729d13a9fe02d5718f`, and
the checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v268.json`.

This pass changed only disposable IDA databases and JSON evidence. It did not
modify the APK or native library and did not contact a DNS, HTTP, TLS,
Google Play, Firebase, or game-server endpoint.

## 2026-08-28: Spectron TGraalVar script-runtime callback pass

The next review focused on the small TGraalVar script-function callbacks that
were still default `sub_` names in the stripped 2.2 database. The source rows
were checked in the 1.8 callback tables, then matched against the target
callback cells and decompiled bodies. This avoids treating nearby addresses as
identity evidence.

### Five cross-build callback anchors

The v269 copy applies five `v18_` labels:

| 1.8 callback | Source | Spectron target | Target table row | Target callback cell | Result |
| --- | ---: | ---: | ---: | ---: | --- |
| `TGraalVar_script_clearvars` | `0x20d26c` | `0x21362c` | `0x39a700` | `0x39a718` | exact metrics |
| `TGraalVar_script_savejsontostring` | `0x20d4dc` | `0x2137ec` | `0x39acd0` | `0x39ace8` | exact metrics |
| `TGraalVar_script_parsejson` | `0x20d500` | `0x213810` | `0x39ae50` | `0x39ae68` | exact metrics |
| `TGraalVar_script_loadini` | `0x20d50c` | `0x21381c` | `0x39aaf0` | `0x39ab08` | exact metrics |
| `TGraalVar_script_addnamedstring` | `0x20d3e8` | `0x2138b0` | `0x39a610` | `0x39a628` | layout change |

The first four pairs are exact across the complete recorded metric set. Their
source and target bodies are the same short dispatch wrappers, even though
the target receiver type is the obfuscated `G0gxgajWBw` class. The JSON and
INI rows also retain the same target helper roles: target `0x2385e0` is the
translated JSON writer, target `0x237dc0` is the translated JSON reader, and
target `eU79LaTf87` is the INI reader called with flag `1`.

`addnamedstring` is the one deliberate layout exception. The source obtains a
variable through virtual slot +288 and assigns the incoming string to offset
+8. The target does the same lookup and assignment, but it first creates and
clears a `CanTfaz6bZ` temporary from a third name argument. The target grows
from 72 to 120 bytes and has a different call set because its string wrapper
was rebuilt. The adjacent target table row decodes to `addnamedstring`, so the
role is high confidence without pretending the instructions are identical.

The machine-readable artifact is
`artifacts/spectron_tgraalvar_script_runtime_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_tgraalvar_script_runtime_anchors.py`.
It contains four exact metric rows and one layout-change row, with no overlap
with the existing broad semantic map. The artifact SHA-256 is
`173a32e498d2be97f71d31a2cf4f3296f603eabda261ba181cf75f2e66723329`.

### Target-only loadvarsfromarray wrapper

The target table row at `0x39aa90` points to callback cell `0x39aaa8`, which
resolves to `0x218870`. Its encoded 17-byte name at `0x2ee208` decodes exactly
to `loadvarsfromarray`. The body allocates a target `vuuHgangcF` string list,
walks the source array at object offset +56, calls the target virtual
`readString` slot +184 for each non-null child, and forwards the finished list
to `G0gxgajWBw::YyvaMa69r8`, the target `loadVarsFromArray` implementation at
`0x2187bc`. The preceding `loadvars` veneer at `0x2187b8` and following
`loadfolder` row provide useful local table context.

This label is intentionally target-only. The source database contains a
`loadVarsFromArray` method, but this review did not demonstrate an equivalent
1.8 script callback row, so the evidence does not claim one. The applied name
is `spectron_TGraalVar_script_loadvarsfromarray_TGraalVar`.

The target-only artifact is
`artifacts/spectron_tgraalvar_target_only_labels_20260828.json`, generated by
`tools/generate_spectron_tgraalvar_target_only_labels.py`. Its SHA-256 is
`8a03c23a43cfe7ad36150a248b716e9230ddf1860747f256e6ecb2b0e545315c`.

### v269 database verification

The final reopened v269 IDA database contains 11,696 functions and 696
remaining default `sub_` names. The existing 3,641 high-confidence semantic
aliases still verify. The five new cross-build names and the target-only name
all reopened with zero failures. The database SHA-256 after the final IDA
reopen is
...
## 2026-08-28: Spectron libjpeg source and destination callbacks

The v273 pass translated six residual functions in the target's standard
libjpeg I/O block. The three destination callbacks are installed by
`v18_jpeg_stdio_dest` at `0x298fcc`:

| Role | 1.8 source | Spectron target | Target callback cell | Applied label |
| --- | ---: | ---: | ---: | --- |
| `init_destination` | `0x28b9f4` | `0x298e64` | `0x298fdc` | `v18_jpeg_init_destination` |
| `empty_output_buffer` | `0x28ba30` | `0x298ea0` | `0x298fe8` | `v18_jpeg_empty_output_buffer` |
| `term_destination` | `0x28baa4` | `0x298f14` | `0x298ff4` | `v18_jpeg_term_destination` |

The target bodies make the assignment unambiguous. They initialize the 4096
byte destination buffer, write a full buffer through JFWRITE, write the final
partial buffer, call JFFLUSH, and check JFERROR. The short-write paths report
JPEG error 37 through the error manager.

The three source callbacks are installed by `v18_jpeg_stdio_src` at
`0x29920c`:

| Role | 1.8 source | Spectron target | Target callback cell | Applied label |
| --- | ---: | ---: | ---: | --- |
| `init_source` | `0x28bbbc` | `0x29902c` | `0x299220` | `v18_jpeg_init_source` |
| `fill_input_buffer` | `0x28bbcc` | `0x29903c` | `0x29922c` | `v18_jpeg_fill_input_buffer` |
| `skip_input_data` | `0x28bc88` | `0x2990f8` | `0x299238` | `v18_jpeg_skip_input_data` |

The source bodies set `start_of_file`, read up to 4096 bytes through JFREAD,
install the JPEG EOI marker `FF D9` after an empty read, and consume complete
buffers while updating the source cursor. The source and target feature
exports match in normalized shape for all six rows. `init_destination` and
`init_source` have identical complete metrics, while the other four differ
only in register allocation detail.

The machine-readable evidence is
`artifacts/spectron_jpeg_io_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jpeg_io_anchors.py`. The reopened v273
database contains 11,696 functions and 663 default `sub_` names. Its database
SHA-256 is
...
## 2026-08-28: Spectron forward-DCT literal-pool boundary repair

The v297 pass applied the IDA database-hygiene fix identified during v296.
The target interval `0x2b9870` through `0x2b98af` contains 64 bytes of four
16-byte NEON vectors. It is embedded in executable `.text` because the
floating-point DCT at `0x2b94d0` obtains the vector addresses with `ADR` and
loads them with SIMD instructions. The bytes are not a callable function.

Before the repair, IDA treated the interval as a 64-byte function named
`sub_2B9870`. Its only incoming references were the four `ADR` instructions
at `0x2b94d4`, `0x2b94e0`, `0x2b94e8`, and `0x2b94d0`. There were no `BL` or
`BLR` references into the interval. The next function began at `0x2b98b0`
and was already correctly named `v18_jpeg_fdct_ifast_int`.

The repair removed only the phantom function boundary, cleared the old code
interpretation, and defined four 16-byte data items:

| Target interval | Data label |
| ---: | --- |
| `0x2b9870` to `0x2b987f` | `xmmword_2B9870` |
| `0x2b9880` to `0x2b988f` | `xmmword_2B9880` |
| `0x2b9890` to `0x2b989f` | `xmmword_2B9890` |
| `0x2b98a0` to `0x2b98af` | `xmmword_2B98A0` |

The raw bytes before and after the repair are identical. Reopening the saved
database confirms that no function starts at `0x2b9870`, all four items are
data, and the real forward-DCT function at `0x2b98b0` is unchanged. This is a
database interpretation correction, not a native-library patch.

The repair is reproducible with
`tools/ida_repair_fdct_literal_pools.py`. The machine-readable evidence is
`artifacts/spectron_fdct_literal_pool_boundary_repair_20260828.json`,
generated by `tools/generate_spectron_fdct_literal_pool_repair_artifact.py`.
The application report `/tmp/spectron_v297_fdct_pool_application.json` and
reopen report `/tmp/spectron_v297_fdct_pool_reopen_verify.json` both have zero
failures. The saved v297 database is
`analysis/spectron_libqplay_translated_v297.i64`; its final post-verification
SHA-256 is
`c4f0175e2839ed6143413d67e1cfef35daf973bcdbb26181ad1879c355f6b3c1`.
The repaired database has 11,695 functions and 530 remaining default `sub_`
names. The checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v297.json`.

This pass changed only the local IDA database interpretation, evidence
artifacts, and documentation. It did not change the native bytes or contact
any network service.

## 2026-08-28: Spectron GIF LZW line decoder and DCT literal-pool boundary

The v296 pass reviewed the remaining GIF decoder helper immediately after the
compressor-side JPEG work. The source candidate at `0x2acb20` and target
candidate at `0x2b9f90` are both 1,508-byte static functions with 377 ARM64
instructions. Their normalized feature records are identical. The only
recorded difference is the register-detail hash, which is consistent with a
different register allocation in the stripped build.

### GIF helper role

The public GIF readers provide the caller context. In the source database,
`DGifGetLine` at `0x2ae28c` and `DGifGetPixel` at `0x2ae350` both call
`sub_2ACB20`. The corresponding target readers
`v18_DGifGetLine` at `0x2bb6ac` and `v18_DGifGetPixel` at `0x2bb770` both call
`sub_2B9F90`. The target helper takes the three arguments expected for a
line decoder: GIF state, destination pixel buffer, and line length.

The pseudocode follows the standard giflib `DGifDecompressLine` contract. It
first drains pending pixels from the decoder stack. It then obtains packed
LZW data from the configured read callback, or from `fread` when no callback
is installed, expands the bit buffer to the current code width, updates the
prefix and suffix dictionary, handles clear and end-of-data codes, and writes
decoded indices into the caller's buffer. The official reference is the
[Android giflib `dgif_lib.c` source](https://android.googlesource.com/platform/external/giflib/%2B/dc07290edccc2c3fc4062da835306f809cea1fdc/dgif_lib.c).

This helper must not be confused with the public `DGifGetLZCodes` function.
That public API is already present at source `0x2ae460` and target `0x2bb880`.
The target callers and the three-argument line-buffer contract identify the
new row as `DGifDecompressLine`, so the applied target label is
...
alias was applied to an isolated copy of the v295 database. The one-row
reopen report `/tmp/spectron_v296_gif_verify.json` and the broad report
`/tmp/spectron_v296_broad_verify.json` both have zero failures. The broad
report still verifies all 3,641 high-confidence cross-build aliases.

### Forward-DCT constant-pool boundary

The same pass corrected the interpretation of a nearby residual. Source
`0x2ac400` and target `0x2b9870` are not wrappers around
`jpeg_fdct_ifast`. Raw bytes show four repeated 16-byte NEON constant vectors
covering each 64-byte interval. The values decode as the floating-point DCT
constants used by the preceding `jpeg_fdct_float` routine. The constants are
embedded in executable `.text`, so IDA's automatic analysis created phantom
functions named `sub_2AC400` and `sub_2B9870` when it followed the `ADR`
references used to load those vectors.

The actual integer forward-DCT functions begin immediately afterward at
source `0x2ac440` and target `0x2b98b0`, matching the surviving source export
`jpeg_fdct_ifast_int` and the already translated target
`v18_jpeg_fdct_ifast_int`. The v296 database records the finding but does not
rename the target constant pool. The subsequent v297 hygiene revision applies
the target-side repair and reopens the database to confirm that the function
inventory no longer counts the pool as code.

### v296 persistence and validation

The saved v296 database is
`analysis/spectron_libqplay_translated_v296.i64`. The clean reopen report
`/tmp/spectron_v296_gif_verify.json` verified the new name with zero failures.
The broad reopen report `/tmp/spectron_v296_broad_verify.json` verified all
3,641 earlier high-confidence aliases with zero failures. The database
contains 11,696 functions and 531 remaining default `sub_` names. Its final
...
This pass changed only the local IDA copy, evidence artifacts, and
documentation. It did not patch the APK or either native library and did not
contact a DNS, HTTP, TLS, Google Play, Firebase, or game-server endpoint.
...
`jinit_phuff_encoder_jpeg_compress_struct` at `0x2aa454`. The stripped target
initializer is `v18_jinit_phuff_encoder_jpeg_compress_struct` at `0x2b78c4`.
The current upstream source calls this module `jcphuff.c`; older libjpeg
trees may use `jphuff.c` for the same compressor implementation. The role
mapping follows the [libjpeg-turbo `jcphuff.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jcphuff.c),
the target callback assignments, reviewed pseudocode, and normalized ARM64
feature metrics.

| Role | 1.8 source | Spectron target | Applied label | Target evidence | Match |
| --- | ---: | ---: | --- | --- | --- |
| `start_pass_phuff` | `0x2a73fc` | `0x2b486c` | `v18_jpeg_start_pass_phuff` | entropy start-pass field at `0x2b78f0` and `0x2b78f8` | normalized shape |
| `emit_eobrun` | `0x2a75dc` | `0x2b4a4c` | `v18_jpeg_emit_eobrun` | shared calls from encoder and finish paths | exact metrics |
| `encode_mcu_AC_refine` | `0x2a7c80` | `0x2b50f0` | `v18_jpeg_encode_mcu_AC_refine` | AC refinement branch at `0x2b4994` and `0x2b499c` | normalized shape |
...
The target start-pass body provides the main role evidence. It stores the
entropy state pointer, records `gather_statistics`, checks the progressive
spectral-selection and approximation fields, and selects one of four MCU
callbacks. The branches are DC first at `0x2b48b0`, AC first at `0x2b49c4`,
DC refinement at `0x2b4978`, and AC refinement at `0x2b4994` and `0x2b499c`.
The same dispatcher selects `finish_pass_gather_phuff` at `0x2b48c0` for a
statistics pass or `finish_pass_phuff` at `0x2b4988` for normal output. It
also builds the selected derived or statistics Huffman tables and resets
EOBRUN, correction-bit, bit-buffer, and restart state.

The large body at `0x2b4a4c` is not a finish callback. Its structure is the
shared `emit_eobrun` helper: it turns a pending end-of-band run into an AC
Huffman category and payload, emits the buffered correction bits, and clears
the pending counters. The normal finish body at `0x2b5d5c` calls this helper,
flushes the final fill bits, and writes back the destination pointers. The
gather finish body at `0x2b5f14` walks the used DC and AC table slots and
generates optimized tables from the accumulated count arrays.

The four MCU callbacks are distinguishable from their state transitions.
`0x2b669c` computes point-transformed DC differences and emits signed
magnitude values. `0x2b6214` emits one Al-th DC refinement bit per block.
`0x2b6f50` scans an AC first-pass band in natural order, emits zero-run and
magnitude symbols, and accumulates EOBRUN for trailing zeroes. `0x2b50f0`
performs the AC refinement prepass, separates newly nonzero coefficients from
correction bits, emits signs and run lengths, and preserves the correction
buffer across output operations.

The source and target addresses differ by `0xd470` for this block, but the
role mapping is not based on that offset alone. All eight source and target
rows match normalized ARM64 feature shape. Five also match the complete
recorded feature set. The three remaining rows differ only in register
allocation detail. The target direct-call list changes from source PLT names
to target C++ export names where shared helpers are exported, which does not
change the normalized body match.

The machine-readable evidence is
`artifacts/spectron_jpeg_progressive_huffman_encoder_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jcphuff_encoder_anchors.py`. The eight
aliases were applied to an isolated copy of the current v293 database. The
reopened v294 copy verified all eight names with zero failures, and the broad
high-confidence map verified all 3,641 earlier aliases with zero failures.
The v294 database contains 11,696 functions and 542 remaining default
`sub_` names. Its final post-verification SHA-256 is
...
The v293 hash was also refreshed in its checkpoint during this pass. Reading
the packed v293 IDA file for evidence persisted IDA metadata without changing
its labels or function count, so the corrected v293 SHA-256 is
`98aca07125910b711ab08522d8513d60d75dbf253c627a73f025a7d2a7213590`.

This pass changed only IDA copies, evidence artifacts, and documentation. It
did not modify the APK or native library and did not contact a DNS, HTTP,
TLS, Google Play, Firebase, or game-server endpoint.

## 2026-08-28: Spectron libjpeg compressor main and master controllers

The v293 pass translated six retained routines from the compressor-side
`jcmainct.c` and `jcmaster.c` blocks. This block sits immediately after the
Huffman encoder in the source-to-target layout, but the function roles are
defined by callback installation rather than by address order.

The main-controller initializer is
`jinit_c_main_controller_jpeg_compress_struct_int` at `0x2a57bc` in the 1.8
database and `v18_jinit_c_main_controller_jpeg_compress_struct_int` at
`0x2b2c2c` in the Spectron database. The target initializer stores
`v18_jpeg_c_start_pass_main` at `0x2b2c60`. That start-pass body selects
`v18_jpeg_c_process_data_simple_main` at `0x2b2bf4` for the controller's row
callback.
...
| `start_pass_main` | `0x2a575c` | `0x2b2bcc` | `v18_jpeg_c_start_pass_main` | main-controller callback field at `0x2b2c60` | normalized shape |

The process callback accepts the input row buffer and available-row count,
calls the preprocessing module until an iMCU row is complete, and passes the
completed strip to the coefficient controller. It preserves the source
behavior when the destination suspends by decrementing the input-row counter
and remembering the suspended state. The start-pass callback resets the iMCU
row, row-group, and suspension counters, rejects a non-pass-through buffer
mode through the same error path as the source, saves the pass mode, and
installs the row callback.

The master-controller initializer is
`jinit_c_master_control_jpeg_compress_struct_int` at `0x2a7058` in the source
database and `v18_jinit_c_master_control_jpeg_compress_struct_int` at
`0x2b44c8` in the target. The target callback table is visible in the
initializer: `prepare_for_pass` is stored at `0x2b4500`, `pass_startup` at
`0x2b4510`, and `finish_pass_master` at `0x2b451c` and `0x2b4524`. The
initializer calls `initial_setup` at `0x2b4734`.

| Role | 1.8 source | Spectron target | Applied label | Target evidence | Match |
| --- | ---: | ---: | --- | --- | --- |
| `initial_setup` | `0x2a5898` | `0x2b2d08` | `v18_jpeg_initial_setup` | master initialization call at `0x2b4734` | exact metrics |
| `pass_startup` | `0x2a6234` | `0x2b36a4` | `v18_jpeg_pass_startup` | master callback field at `0x2b4510` | exact metrics |
| `finish_pass_master` | `0x2a6270` | `0x2b36e0` | `v18_jpeg_finish_pass_master` | master callback field at `0x2b451c` and `0x2b4524` | exact metrics |
| `prepare_for_pass` | `0x2a6324` | `0x2b3794` | `v18_jpeg_prepare_for_pass` | master callback field at `0x2b4500` | exact metrics |

The large `initial_setup` body validates component sampling factors, scan
component references, quantization table indexes, and progressive parameters.
The `prepare_for_pass` body selects main, Huffman-optimization, or output
passes, then starts the matching color conversion, downsampling,
preprocessing, DCT, entropy, coefficient, and main-controller modules.
`pass_startup` emits deferred frame and scan headers through the marker
controller. `finish_pass_master` completes the entropy pass and advances the
master pass type, scan number, and pass counter.

All six rows are high-confidence normalized-shape matches. Five match the
complete recorded ARM64 feature set. Only `start_pass_main` differs in
register allocation detail. The `c_` qualifier on the two `jcmainct.c` names
is deliberate: the decompressor-side `jdmainct.c` family already uses the
unqualified `v18_jpeg_process_data_simple_main` and
`v18_jpeg_start_pass_main` names at different target addresses.
...
aliases were applied to an isolated copy of the v292 database and reopened
with zero failures. The broad high-confidence translation map and the offline
archive validator also passed with zero failures. The v293 database contains
11,696 functions and 550 remaining default `sub_` names. Its post-reopen
SHA-256 is
`98aca07125910b711ab08522d8513d60d75dbf253c627a73f025a7d2a7213590`. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v293.json`.

This pass also corrected the local v292 reproducibility metadata. An initial
failed attempt showed that opening an IDA database can persist applied labels
to the opened file even when a separate save path is supplied. The affected
v292 file was rebuilt from the clean v291 checkpoint through an isolated
temporary input, then reopened and reverified. The corrected v292 database
hash is recorded in its checkpoint and is
...
## 2026-08-28: Spectron libjpeg compressor Huffman encoder

The v292 pass translated five retained routines from the compressor-side
Huffman encoder. The source initializer is
`jinit_huff_encoder_jpeg_compress_struct` at `0x2a55bc`, and the target
initializer is `v18_jinit_huff_encoder_jpeg_compress_struct` at `0x2b2a2c`. The
role mapping follows the callback contract in the [libjpeg-turbo
`jchuff.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jchuff.c).

| Role | 1.8 source | Spectron target | Applied label | Target callback assignment |
| --- | ---: | ---: | --- | --- |
...
The target start-pass dispatcher preserves the source split between the
optimized-table gathering pass and normal compressed output. In the first
mode it installs the gather encoder and gather finish callback. In the second
it installs the normal MCU encoder and Huffman finish callback. The same body
also prepares per-component Huffman tables, resets DC predictors, and clears
the saved bit and restart state.

The gather encoder walks every coefficient block, counts DC difference
categories, counts AC magnitude and zero-run symbols, and produces no output.
The normal encoder emits restart markers and Huffman-coded DC and AC values,
handles 0xff byte stuffing, and saves its bit state across a suspended output
buffer. The normal finish callback flushes the final bits. The gather finish
callback creates optimized DC and AC tables once per used table from the
accumulated counts.

The target callback assignments provide direct role evidence. The target
initializer references `0x2b2338` at `0x2b2a58` and `0x2b2a60`. The target
start-pass body references the gather encoder at `0x2b2364` and `0x2b236c`,
the gather finish callback at `0x2b2360` and `0x2b2368`, the normal finish
callback at `0x2b2510` and `0x2b2518`, and the normal encoder at `0x2b2514`
and `0x2b251c`. This identifies the five routines without relying on simple
address adjacency.

All five source and target rows are high-confidence matches and all five
match normalized ARM64 feature shape. Two match the complete recorded
feature set. The other three differ only in register allocation detail, with
the start-pass row also carrying the target's expected shared-helper export
names. Both builds retained default IDA names before this pass, so the
applied names are semantic review labels rather than recovered debug symbols.

The machine-readable artifact is
`artifacts/spectron_jpeg_huffman_encoder_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jchuff_anchors.py`. The aliases were
applied to a copy of the v291 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v292 database contains 11,696 functions and 556 remaining
default `sub_` names. Its post-reopen SHA-256 is
`4ad75f4e344be7e0d168d3674b0bd93b743d5148448ec7a2b50f559fb48f7c09`. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v292.json`.

This pass changed only the disposable IDA copy, evidence artifacts, and
documentation. It did not modify the APK or native library and did not
contact a DNS, HTTP, TLS, Google Play, Firebase, or game-server endpoint.

## 2026-08-28: Spectron libjpeg forward-DCT manager

The v291 pass translated three retained routines from the compressor-side
forward-DCT manager. The source initializer is
`jinit_forward_dct_jpeg_compress_struct` at `0x2a3c00`, and the target
initializer is `v18_jinit_forward_dct_jpeg_compress_struct` at `0x2b1070`. The
role mapping follows the callback contract in the [libjpeg-turbo
`jcdctmgr.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jcdctmgr.c).
...
The start-pass callback prepares the per-component quantization multipliers.
Its target body preserves the source branches for the integer and
floating-point data-precision paths and the same unsupported-precision error
handling. The integer callback centers each 8 by 8 input block, calls the
selected integer forward-DCT method, quantizes all 64 values, and writes the
resulting JPEG coefficients. The floating-point callback loads centered
samples as floats, calls the float DCT method, applies the precomputed float
quantization values, and stores the packed coefficient block.

The target initializer makes the roles independently checkable. At
`0x2b10a0` it installs the start-pass routine. Its integer branches reference
`0x2b0aa8` at `0x2b1128` and `0x2b1144`, while the floating-point branch
references `0x2b0c10` at `0x2b110c`. These assignments match the source
initializer and identify the residual bodies without relying on adjacency.

All three source and target rows are high-confidence matches and all three
match normalized ARM64 feature shape. Two match the complete recorded
feature set. Only `start_pass_fdctmgr` differs in register allocation detail.
Both builds retained default IDA names before this pass, so the applied names
are semantic review labels rather than recovered debug symbols.

The machine-readable artifact is
`artifacts/spectron_jpeg_forward_dct_manager_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jcdctmgr_anchors.py`. The aliases were
applied to a copy of the v290 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v291 database contains 11,696 functions and 561 remaining
default `sub_` names. Its post-reopen SHA-256 is
`9933c9b80f7962b4e3666ad5f6eee22b42487e8027d2691fb9e36abd6d1e76a4`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg compressor coefficient controller

The v290 pass translated five retained routines from the compressor-side
coefficient controller. The source initializer is
`jinit_c_coef_controller_jpeg_compress_struct_int` at `0x2a29bc`, and the
target initializer is
`v18_jinit_c_coef_controller_jpeg_compress_struct_int` at `0x2afe2c`. The
role mapping follows the callback contract in the [libjpeg-turbo
`jccoefct.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jccoefct.c).

| Role | 1.8 source | Spectron target | Applied label | Target callback assignment |
| --- | ---: | ---: | --- | --- |
| `start_iMCU_row` | `0x2a1b50` | `0x2aefc0` | `v18_jpeg_start_iMCU_row` | row reset calls at `0x2af270`, `0x2af6a0`, and `0x2afd14` |
...
The shared row helper chooses one MCU row for an interleaved scan or the
component sampling or final-row height for a noninterleaved scan, then resets
the within-row counters. The pass-through routine constructs MCU block lists
from input sample planes, calls the forward DCT, creates right and bottom edge
dummy blocks, and sends the MCU to entropy encoding. The first-pass routine
performs the same DCT work while filling the full-image virtual coefficient
arrays. The output routine reads those arrays during subsequent passes and
feeds the entropy encoder.

The target `start_pass_coef` body is direct dispatcher evidence. It stores the
pass-through, full-buffer first-pass, or crank-destination routine according to
the requested buffer mode, matching the source `jccoefct.c` contract. The
target callback references are in the reviewed target initializer at
`0x2afe2c`, and the row helper has incoming calls from all three data paths.

All five source and target rows are high-confidence matches and all five match
normalized ARM64 feature shape. Four match the complete recorded feature set.
Only `start_pass_coef` differs in register allocation detail. Both builds had
default IDA names before the pass, so these are semantic review labels rather
than recovered debug symbols.

The machine-readable artifact is
`artifacts/spectron_jpeg_compressor_coefficient_controller_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jccoefct_anchors.py`. The aliases were
applied to a copy of the v289 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v290 database contains 11,696 functions and 564 remaining
default `sub_` names. Its post-reopen SHA-256 is
`5a74d5d6915311fedf66eb33f1c7a6901baad8ad47653ae1c445698eccbf37e7`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg compressor color converter

The v289 pass translated six retained routines from the compressor-side color
conversion block. The source initializer is
`jinit_color_converter_jpeg_compress_struct` at `0x2a2ee4`, and the target
initializer is `v18_jinit_color_converter_jpeg_compress_struct` at `0x2b0354`.
The role mapping follows the callback contract in the [libjpeg-turbo
`jccolor.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jccolor.c).
...
The first target body allocates a 2048-entry table and fills the fixed-point
coefficient and range regions used by the two conversion paths that need
them. The RGB-to-YCbCr body reads interleaved RGB triples and writes separate
Y, Cb, and Cr rows. The RGB-to-grayscale body combines the luminance table
regions and writes one byte per pixel. The CMYK-to-YCCK body inverts the first
three input components, performs the YCC conversion, and copies the black
component unchanged. The final two bodies are simple stride-aware copy loops.

The target parent gives direct callback evidence for every row. Its RGB,
YCbCr, grayscale, CMYK, and YCCK cases select the same method combinations as
the source parent. Incoming references from the target initializer are
`0x2b0608`, `0x2b0624`, and `0x2b0640` for the table initializer,
`0x2b0630` for RGB-to-YCbCr, `0x2b064c` for RGB-to-grayscale, `0x2b0614` for
CMYK-to-YCCK, `0x2b05a8` for the grayscale copy, and `0x2b0518` for the generic
copy. This assignment evidence is stronger than a size-only match.

All six source and target rows are high-confidence matches. Their complete
recorded ARM64 feature metrics are identical, including instruction counts,
control-flow counts, mnemonic hashes, operand-shape hashes, string-reference
hashes, and register-detail hashes. Both sides had default IDA names before
the pass, so the applied names are semantic review labels rather than claims
that original debug symbols survived in the target.

Two source role names are also used by the decompressor-side `jdcolor.c`
block. The compressor copies therefore use `v18_jpeg_c_grayscale_convert` and
`v18_jpeg_c_null_convert`; this keeps the IDA namespace unambiguous while
recording the source file distinction.

The machine-readable artifact is
`artifacts/spectron_jpeg_compressor_color_converter_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jccolor_anchors.py`. The aliases were
applied to a copy of the v288 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v289 database contains 11,696 functions and 569 remaining
default `sub_` names. Its post-reopen SHA-256 is
`15b16bb239ebe1e4eccc22629d68a31a8275feff61be37b2a408e19db20ea9bb`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg main decompression controller

The v288 pass translated the four retained routines around the target
`v18_jinit_d_main_controller_jpeg_decompress_struct_int` initializer. The
source initializer is `jinit_d_main_controller_jpeg_decompress_struct_int` at
`0x2a04f4`, and the target initializer is at `0x2ad964`. The role mapping
follows the main-controller callback contract in the [libjpeg-turbo `jdmainct.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdmainct.c).

| Role | 1.8 source | Spectron target | Applied label | Target callback field |
| --- | ---: | ---: | --- | --- |
| `process_data_simple_main` | `0x29fc98` | `0x2ad108` | `v18_jpeg_process_data_simple_main` | no-context path at `0x2ad5f8` |
| `process_data_context_main` | `0x29fd40` | `0x2ad1b0` | `v18_jpeg_process_data_context_main` | context-row path at `0x2ad62c` |
| `process_data_crank_post` | `0x2a00c0` | `0x2ad530` | `v18_jpeg_process_data_crank_post` | final destination pass at `0x2ad5c4` |
| `start_pass_main` | `0x2a00f8` | `0x2ad568` | `v18_jpeg_start_pass_main` | public start-pass slot at `0x2ad9a0` |

The simple row path fills the main buffer from the coefficient controller,
passes the available row groups to the postprocessor, and clears the buffer
state after the postprocessor consumes them. The context-row path alternates
two workspaces, calls the coefficient and postprocessing controllers, and
maintains the extra rows above and below the active row group. The crank helper
forwards the final two-pass destination operation to the postprocessor.

The start-pass dispatcher selects the crank helper for the final two-pass
destination mode. For a normal pass it selects the context-aware path when
the upsampler requests context rows, otherwise it selects the simple path. It
also resets the buffer and row-group state, and initializes the context state
when the funny-pointer workspaces are active. These assignments directly tie
the four target functions to the source controller's responsibilities.

All four source and target rows are high-confidence matches and all four match
normalized ARM64 feature shape. Three match the complete recorded feature
set. The only remaining difference is register allocation detail in
`start_pass_main`. Both source and target functions were default IDA names
before this pass, so the `v18_jpeg_` names are reviewed semantic roles rather
than recovered debug symbols.
...
applied to a copy of the v287 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v288 database contains 11,696 functions and 575 remaining
default `sub_` names. Its post-reopen SHA-256 is
`99d386649d7356bf021dfd87e8e153b29e24a52f14b83504d5b7b28956c68065`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg baseline-Huffman decoder

The v287 pass translated the two retained functions immediately before the
target `v18_jinit_huff_decoder_jpeg_decompress_struct` initializer. The source
initializer is `jinit_huff_decoder_jpeg_decompress_struct` at `0x29fc2c`, and
the target initializer is at `0x2ad09c`. The role mapping follows the
baseline-Huffman callback contract in the [libjpeg-turbo `jdhuff.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdhuff.c).

| Role | 1.8 source | Spectron target | Applied label | Target callback field |
| --- | ---: | ---: | --- | --- |
| `start_pass_huff_decoder` | `0x29f2d0` | `0x2ac740` | `v18_jpeg_start_pass_huff_decoder` | entropy start-pass slot at `0x2ad0c4` |
| `decode_mcu` | `0x29f734` | `0x2acba4` | `v18_jpeg_decode_mcu` | baseline decoder slots at `0x2ad0d4` and `0x2ad0dc` |

The target start-pass body validates the decompression state, derives the DC
and AC Huffman lookup tables for every scan component, initializes the
progressive bookkeeping fields used by the shared entropy state, and resets
the bit buffer, restart counter, and insufficient-data flag. Its installation
at the first function-pointer field of the target entropy object is direct
evidence for the role.

The target MCU body performs the baseline decode loop. It refills the bit
buffer as needed, looks up DC and AC symbols, sign-extends DC differences,
updates the per-component predictors, expands AC run-length symbols, writes
natural-order coefficients, and saves the reader state across suspension.
The target initializer writes this function into both baseline MCU decoder
slots. The source and target bodies retain the same loop structure and
operation order in the reviewed pseudocode.

The start-pass row matches the complete recorded ARM64 feature set. The MCU
decoder has matching size, instruction count, basic-block count, branch and
call counts, return count, and mnemonic sequence. Four operand-shape metrics
also differ because one `ADD` instruction uses a relocated `PAGEOFF` constant
whose low-page bucket differs between builds. The source uses
`dword_2EC790@PAGEOFF`, while the target uses `dword_2FB020@PAGEOFF`; this is
an address-layout difference, not a changed decoder operation, and the
artifact records it as an explicit exception.

Both rows were default IDA names before the pass, so the `v18_jpeg_` names are
reviewed semantic roles rather than recovered debug symbols. The machine-
readable artifact is
`artifacts/spectron_jpeg_baseline_huffman_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdhuff_anchors.py`. The aliases were
applied to a copy of the v286 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v287 database contains 11,696 functions and 579 remaining
default `sub_` names. Its post-reopen SHA-256 is
`5af0d676931b984f0eac35dfa49ff13f66b11f5b3a8c023f19a9d03fc26b844e`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg inverse-DCT manager

The v286 pass translated the retained inverse-DCT manager start-pass routine.
The source initializer is `jinit_inverse_dct_jpeg_decompress_struct` at
`0x29e7b0`, and the target initializer is
`v18_jinit_inverse_dct_jpeg_decompress_struct` at `0x2abc20`. The role follows
the inverse-DCT manager contract in the [libjpeg inverse-DCT manager source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jddctmgr.c).

| Role | 1.8 source | Spectron target | Applied label | Target evidence |
| --- | ---: | ---: | --- | --- |
| `start_pass` | `0x29e40c` | `0x2ab87c` | `v18_jpeg_idct_start_pass` | public IDCT start-pass slot at `0x2abc5c` |

The callback selects the scaled IDCT method for every JPEG component. It
handles the 1 by 1, 2 by 2, 4 by 4, accurate integer, fast integer, and
floating-point cases. When a component's method changes, it rebuilds the
matching multiplier table from that component's quantization table. The target
initializer stores the routine in the same public start-pass slot as the
source, and the decompiled target body retains the same method switch and
table-generation branches.

The single row is a high-confidence match. Its normalized ARM64 feature
metrics match exactly, with only register allocation detail differing. The
target's existing names for the scaled inverse-DCT implementations provide an
independent check on the switch cases, so this label is based on both callback
placement and body semantics rather than size alone.
...
applied to a copy of the v285 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v286 database contains 11,696 functions and 581 remaining
default `sub_` names. Its post-reopen SHA-256 is
`87624a85eae15f9520bfcbf1b356121aa7af6b97db66997a66bcea5baadadae9`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg color deconverter

The v285 pass translated the six retained function boundaries immediately
before the target color-deconverter initializer. The source initializer is
`jinit_color_deconverter_jpeg_decompress_struct` at `0x29dfe4`, and the target
initializer is `v18_jinit_color_deconverter_jpeg_decompress_struct` at
`0x2ab454`. The role mapping follows the callback assignments and conversion
methods in the [libjpeg-turbo `jdcolor.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdcolor.c).

| Role | 1.8 source | Spectron target | Applied label | Target evidence |
| --- | ---: | ---: | --- | --- |
| `ycc_rgb_convert` | `0x29da68` | `0x2aaed8` | `v18_jpeg_ycc_rgb_convert` | YCbCr-to-RGB initializer branch |
| `null_convert` | `0x29db28` | `0x2aaf98` | `v18_jpeg_null_convert` | direct component-interleave branches |
| `gray_rgb_convert` | `0x29dba4` | `0x2ab014` | `v18_jpeg_gray_rgb_convert` | grayscale-to-RGB initializer branch |
| `ycck_cmyk_convert` | `0x29dee8` | `0x2ab358` | `v18_jpeg_ycck_cmyk_convert` | YCCK-to-CMYK initializer branch |
| `start_pass_dcolor` | `0x29dfc8` | `0x2ab438` | `v18_jpeg_start_pass_dcolor` | empty start-pass callback |
| `grayscale_convert` | `0x29dfcc` | `0x2ab43c` | `v18_jpeg_grayscale_convert` | luminance-copy initializer branches |

The initializer assigns the output method according to input
and output colorspaces, then records the output component count. The
YCbCr-to-RGB method consumes three input planes, applies the precomputed
chroma tables and range limit, and emits three interleaved bytes. The
direct-copy method handles unchanged component layouts, while the
grayscale-to-RGB method repeats one sample across the RGB channels and keeps a
vectorized wide-row path. The YCCK-to-CMYK method converts and inverts the
first three channels while copying K unchanged. Grayscale output uses the
short wrapper that copies the first input plane, and the start-pass callback is
an intentional no-op.

The target callback assignments provide the role evidence. The YCbCr-to-RGB,
grayscale-to-RGB, and YCCK-to-CMYK bodies are reached only from the matching
colorspace cases in `v18_jinit_color_deconverter_jpeg_decompress_struct`.
The direct-copy and grayscale wrappers have the corresponding unchanged or
luminance-only branches. This also explains why the six functions are not
merely a size-based match even though all six have identical complete ARM64
feature metrics.

All six rows are high-confidence matches. Their normalized and complete
feature metrics match exactly, their incoming references are in the expected
initializer, and their bodies implement the assigned conversion. The source
and target `nullsub_6` entry is given the semantic role label
`v18_jpeg_start_pass_dcolor` so the callback is documented instead of left as
an opaque nullsub.

The machine-readable artifact is
`artifacts/spectron_jpeg_color_deconverter_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdcolor_anchors.py`. The aliases were
applied to a copy of the v284 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v285 database contains 11,696 functions and 582 remaining
default `sub_` names. Its post-reopen SHA-256 is
`9808e39ad80e9bb122ae35656c2eb48d23097bfcdd31e3b67db568d6bc13cd1d`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg coefficient controller

The v284 pass translated the seven retained function boundaries around the
target coefficient-buffer controller. The source initializer is
`jinit_d_coef_controller_jpeg_decompress_struct_int` at `0x29d8a8`, and the
target initializer is `v18_jinit_d_coef_controller_jpeg_decompress_struct_int`
at `0x2aad18`. The role mapping follows the callback contract and control
flow in the [libjpeg-turbo `jdcoefct.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdcoefct.c).

| Role | 1.8 source | Spectron target | Applied label | Target evidence |
| --- | ---: | ---: | --- | --- |
| `dummy_consume_data` | `0x29c6bc` | `0x2a9b2c` | `v18_jpeg_dummy_consume_data` | single-MCU consume slot in the initializer |
| `consume_data` | `0x29c6c4` | `0x2a9b34` | `v18_jpeg_consume_data` | virtual-array input slot in the initializer |
| `start_output_pass` | `0x29c9c8` | `0x2a9e38` | `v18_jpeg_start_output_pass` | selects normal or smoothed output |
| `decompress_smooth_data` | `0x29cb80` | `0x2a9ff0` | `v18_jpeg_decompress_smooth_data` | selected by the smoothing branch |
| `decompress_data` | `0x29d2f8` | `0x2aa768` | `v18_jpeg_decompress_data` | full-buffer inverse-DCT output |
| `start_input_pass` | `0x29d510` | `0x2aa980` | `v18_jpeg_coef_start_input_pass` | resets coefficient input cursors |
| `decompress_onepass` | `0x29d570` | `0x2aa9e0` | `v18_jpeg_decompress_onepass` | single-MCU inverse-DCT output |

The first two rows explain the controller's two input modes. With a full
coefficient buffer, `consume_data` obtains per-component virtual-array rows,
builds the MCU block pointer list, calls the entropy decoder, and advances the
input row. Without that buffer, `dummy_consume_data` reports suspension because
single-pass decoding reads and emits an MCU in the same operation.

The output callbacks line up just as clearly. `start_output_pass` checks the
buffered-image state and smoothing prerequisites before selecting either
`decompress_data` or `decompress_smooth_data`. The normal multi-pass path reads
virtual coefficient rows and invokes each component's inverse-DCT method. The
smoothed path copies each block into a workspace and estimates missing low
frequency coefficients from a 5 by 5 neighborhood before inverse DCT. The
single-pass path clears the MCU buffer, decodes entropy data, skips unneeded
components, and emits useful blocks directly.

All seven rows are high-confidence matches. Their normalized ARM64 feature
metrics match exactly, and six also match the complete recorded feature set.
The only metric difference is register allocation detail in
`start_output_pass`, whose smoothing decision is compiled into the retained
function boundary in this build. The source and target bodies, initializer
slots, incoming references, and callback transitions agree independently of
that compiler detail.

The coefficient-controller `start_input_pass` function is file-local and has
the same source-level name as the already translated `jdinput.c` routine at
target `0x299320`. The target label therefore uses
`v18_jpeg_coef_start_input_pass` to preserve both identities without creating
a duplicate IDA name.

The machine-readable artifact is
`artifacts/spectron_jpeg_coefficient_controller_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdcoefct_anchors.py`. The aliases were
applied to a copy of the v283 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v284 database contains 11,696 functions and 587 remaining
default `sub_` names. Its post-reopen SHA-256 is
`75ec25ac82f7724c4332611be2e8bfdea2dc1453f825b0bd0e845e53cfed8bf7`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg two-pass quantizer

The v283 pass translated the nine retained function boundaries in the target
`jquant2.c` block between the one-pass quantizer and the standard JPEG
compressor helpers. The source initializer at `0x297b00` and the target
initializer at `0x2a4f70` preserve the same public start and new-color-map
callbacks. The target start-pass routine selects histogram prescan, ordinary
palette mapping, or Floyd-Steinberg mapping.

| Role | 1.8 source | Spectron target | Applied label | Target dispatch |
| --- | ---: | ---: | --- | --- |
| `prescan_quantize` | `0x296270` | `0x2a36e0` | `v18_jpeg_prescan_quantize` | prescan callback at `0x2a37a8` |
| `finish_pass2` | `0x2962f4` | `0x2a3764` | `v18_jpeg_finish_pass2` | second-pass finish at `0x2a380c` |
| `new_color_map_2_quant` | `0x2962f8` | `0x2a3768` | `v18_jpeg_new_color_map_2_quant` | new-color-map slot at `0x2a4fb0` |
| `start_pass_2_quant` | `0x296308` | `0x2a3778` | `v18_jpeg_start_pass_2_quant` | public start slot at `0x2a4fa0` and `0x2a4fa8` |
| `update_box` | `0x296620` | `0x2a3a90` | `v18_jpeg_update_box` | palette-box split at `0x2a4cd4` and `0x2a4ce4` |
| `fill_inverse_cmap` | `0x296a64` | `0x2a3ed4` | `v18_jpeg_fill_inverse_cmap` | cache misses at `0x2a440c` and `0x2a4704` |
| `pass2_no_dither` | `0x296ee0` | `0x2a4350` | `v18_jpeg_pass2_no_dither` | no-dither mapping at `0x2a37fc` |
| `pass2_fs_dither` | `0x296fe4` | `0x2a4454` | `v18_jpeg_pass2_fs_dither` | Floyd-Steinberg mapping at `0x2a3874` |
| `finish_pass1` | `0x297388` | `0x2a47f8` | `v18_jpeg_finish_pass1` | prescan finish at `0x2a37b4` |

The prescan callback builds the reduced-precision histogram. The finish path
shrinks and splits occupied color-space boxes, computes representative
palette colors, and marks the inverse map for the next pass. The plain second
pass uses cached palette indexes, while the Floyd-Steinberg path alternates
scan direction, applies the error limiter, and carries three component errors
through its workspace. The new-color-map callback marks the inverse map for
clearing, and the second-pass finish callback is a four-byte no-op.

These roles match the standard [libjpeg-turbo `jquant2.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jquant2.c). The address delta from each 1.8 function to its Spectron counterpart is `0xd470`. All nine rows are high-confidence matches and all nine match normalized ARM64 features. Eight match the complete feature set, while `start_pass_2_quant` differs only in register allocation detail. The source's nearby-color and best-color routines are compiler-folded into the retained inverse-map and palette-finish boundaries in this build, so the artifact records actual target function starts rather than inventing absent boundaries. The source and target `nullsub_5` entry is treated as an IDA auto-generated name.

The machine-readable artifact is
`artifacts/spectron_jpeg_two_pass_quantizer_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jquant2_anchors.py`. The aliases were
applied to a copy of the v282 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
...
## 2026-08-28: Spectron libjpeg one-pass quantizer

The v282 pass translated the eight unnamed routines immediately before the
target `v18_jinit_1pass_quantizer_jpeg_decompress_struct` initializer at
`0x2a2f88`. The source initializer at `0x295b18` and the target initializer
retain the same jquant1 callback contract, and the target start-pass body
selects the row quantizer according to dither mode and output component count.

| Role | 1.8 source | Spectron target | Applied label | Target dispatch |
| --- | ---: | ---: | --- | --- |
| `color_quantize` | `0x294f44` | `0x2a23b4` | `v18_jpeg_color_quantize` | general components, no dither at `0x2a29b4` |
| `color_quantize3` | `0x294fd0` | `0x2a2440` | `v18_jpeg_color_quantize3` | three components, no dither at `0x2a2efc` |
| `quantize3_ord_dither` | `0x295050` | `0x2a24c0` | `v18_jpeg_quantize3_ord_dither` | three components, ordered dither at `0x2a2ef0` |
| `finish_pass_1_quant` | `0x295138` | `0x2a25a8` | `v18_jpeg_finish_pass_1_quant` | finish callback at `0x2a2fc8` and `0x2a2fd0` |
| `new_color_map_1_quant` | `0x29513c` | `0x2a25ac` | `v18_jpeg_new_color_map_1_quant` | new-color-map callback at `0x2a2fd8` and `0x2a2fe0` |
| `quantize_fs_dither` | `0x295164` | `0x2a25d4` | `v18_jpeg_quantize_fs_dither` | Floyd-Steinberg at `0x2a2910` and `0x2a2918` |
| `quantize_ord_dither` | `0x295344` | `0x2a27b4` | `v18_jpeg_quantize_ord_dither` | general components, ordered dither at `0x2a29e4` |
| `start_pass_1_quant` | `0x29545c` | `0x2a28cc` | `v18_jpeg_start_pass_1_quant` | start callback at `0x2a2fb8` and `0x2a2fc0` |

The plain color quantizers perform direct palette-index lookups. The ordered
paths use the sixteen-row dither tables, with a specialized RGB loop for the
three-component case. The Floyd-Steinberg routine clears the output rows,
handles forward and reverse scan direction, and carries its quantization
errors through the workspace. The start-pass routine builds the palette
dimensions and lookup tables, initializes the selected dither state, and
installs the active row callback. The finish callback is the shared four-byte
no-op. The new-color-map callback reports error 46 through the JPEG error
manager.

These roles match the standard [libjpeg-turbo `jquant1.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jquant1.c). The address delta from each 1.8 function to its Spectron counterpart is `0xd470`. All eight rows are high-confidence matches and all eight match normalized ARM64 features. Seven match the complete feature set, while `start_pass_1_quant` differs only in register allocation detail. The source and target `nullsub_4` entry is treated as an IDA auto-generated name in the artifact, so its applied label records a reviewed role rather than recovered debug metadata.

The machine-readable artifact is
`artifacts/spectron_jpeg_one_pass_quantizer_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jquant1_anchors.py`. The aliases were
applied to a copy of the v281 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v282 database contains 11,696 functions and 602 remaining
default `sub_` names. Its post-reopen SHA-256 is
`97e6e0fe4b2cac011692a6ed945fbabf89f4c5d66eb8a2ccc635f8c1f914308c`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg memory manager

The v281 pass translated the eleven unnamed functions between the target
`v18_jpeg_std_error` block and `v18_jinit_memory_mgr_jpeg_common_struct` at
`0x2a21b8`. The source and target initializers retain the same public memory
manager method table, so each role can be resolved from its slot as well as
from its body.

| Role | 1.8 source | Spectron target | Applied label | Method slot |
| --- | ---: | ---: | --- | ---: |
| `alloc_small` | `0x292cf8` | `0x2a0168` | `v18_jpeg_alloc_small` | 0 |
| `alloc_large` | `0x292ef8` | `0x2a0368` | `v18_jpeg_alloc_large` | 9 |
| `alloc_sarray` | `0x293058` | `0x2a04c8` | `v18_jpeg_alloc_sarray` | 1 |
| `alloc_barray` | `0x293168` | `0x2a05d8` | `v18_jpeg_alloc_barray` | 7 |
| `realize_virt_arrays` | `0x293500` | `0x2a0970` | `v18_jpeg_realize_virt_arrays` | 8 |
| `request_virt_sarray` | `0x2938b0` | `0x2a0d20` | `v18_jpeg_request_virt_sarray` | 4 |
| `request_virt_barray` | `0x293acc` | `0x2a0f3c` | `v18_jpeg_request_virt_barray` | 5 |
| `access_virt_barray` | `0x293ce8` | `0x2a1158` | `v18_jpeg_access_virt_barray` | 10 |
| `access_virt_sarray` | `0x293e50` | `0x2a12c0` | `v18_jpeg_access_virt_sarray` | 2 |
| `free_pool` | `0x2941d0` | `0x2a1640` | `v18_jpeg_free_pool` | 3 |
| `self_destruct` | `0x294534` | `0x2a19a4` | `v18_jpeg_self_destruct` | 6 |

The target `alloc_small` method rounds requests for alignment, searches the
small-pool list, grows a pool with the correct slop policy, and updates the
pool header. `alloc_large` creates a dedicated large pool. The two ordinary
array methods allocate row pointers and chunked sample or coefficient storage.
The virtual-array request methods create control blocks, `realize_virt_arrays`
divides available memory among them, and the access methods load or flush
strips through the backing store while handling zeroed rows and dirty state.
`free_pool` closes virtual stores and releases both pool lists. The
`self_destruct` method releases the pools and manager object, clears the
manager pointer, and calls `jpeg_mem_term`.

These roles match the standard [libjpeg-turbo `jmemmgr.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jmemmgr.c).
The address delta from each 1.8 function to its Spectron counterpart is
`0xd470`. All eleven rows are high-confidence method-table matches. Nine
match normalized ARM64 features, six match the complete feature set, and
three differ only in register allocation detail. `alloc_small` and
`free_pool` additionally differ in the recorded call and operand layout
because the source calls through PLT thunks while the target retains direct
allocator exports. Their decompiled pool logic and method assignments agree,
so those differences are kept explicit in the artifact.

The machine-readable artifact is
`artifacts/spectron_jpeg_memory_manager_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jmemmgr_anchors.py`. The aliases were
applied to a copy of the v280 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v281 database contains 11,696 functions and 609 remaining
default `sub_` names. Its post-reopen SHA-256 is
`6b59348c18a8346c3d915a2d536616dfa5aba43ed4605a7bcf0e09f252f8db13`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg error handlers

The v280 pass translated the five unnamed functions between the target
`v18_jinit_upsampler_jpeg_decompress_struct` block and
`v18_jpeg_std_error` at `0x2a00fc`. Their source counterparts occupy the
same relative block after `jinit_upsampler` and before `jpeg_std_error` in
the 1.8 database. The target `jpeg_std_error` initializer assigns the five
callbacks directly, which makes the role resolution independent of their
default names.

| Role | 1.8 source | Spectron target | Applied label | Target callback field |
| --- | ---: | ---: | --- | --- |
| `emit_message` | `0x292aac` | `0x29ff1c` | `v18_jpeg_emit_message` | `emit_message` at `0x2a010c` |
| `reset_error_mgr` | `0x292b24` | `0x29ff94` | `v18_jpeg_reset_error_mgr` | `reset_error_mgr` at `0x2a013c` |
| `format_message` | `0x292b34` | `0x29ffa4` | `v18_jpeg_format_message` | `format_message` at `0x2a012c` |
| `output_message` | `0x292c1c` | `0x2a008c` | `v18_jpeg_output_message` | `output_message` at `0x2a011c` |
| `error_exit` | `0x292c64` | `0x2a00d4` | `v18_jpeg_error_exit` | `error_exit` at `0x2a00fc` |

The `emit_message` body applies the standard warning and trace policy, then
updates the warning count. `reset_error_mgr` clears the warning count and
message code. `format_message` chooses the built-in or add-on message table,
handles string parameters, and formats the saved arguments. `output_message`
calls the formatter and writes the result to stderr. `error_exit` emits the
message, calls `jpeg_destroy`, and exits with status one.

These roles match the standard [libjpeg-turbo `jerror.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jerror.c).
The address delta from each 1.8 function to its Spectron counterpart is
`0xd470`. All five rows match in normalized ARM64 feature shape. Four are
complete metric matches, while `output_message` differs only in register
allocation detail.

The machine-readable artifact is
`artifacts/spectron_jpeg_error_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jerror_anchors.py`. The aliases were
applied to a copy of the v279 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v280 database contains 11,696 functions and 620 remaining
default `sub_` names. Its post-reopen SHA-256 is
`b87e3bc5ecf33c2df89d57987b7bdf80255efc3289eed8dfe976d67e24d1ff13`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg upsampler block

The v279 pass translated the nine unnamed routines immediately before the
target `v18_jinit_upsampler_jpeg_decompress_struct` initializer at `0x29fc9c`.
This is the adjacent `jdsample.c` block after the postprocessing controller.
The target initializer preserves the source callback layout: it installs the
start and row callbacks, then chooses a per-component upsampler from the
sampling factors, fancy-upsampling flag, and component-needed state.

| Role | 1.8 source | Spectron target | Applied label | Target dispatch |
| --- | ---: | ---: | --- | --- |
| `start_pass_upsample` | `0x291b6c` | `0x29efdc` | `v18_jpeg_start_pass_upsample` | upsampler start slot at `0x29fcd8` |
| `sep_upsample` | `0x291b84` | `0x29eff4` | `v18_jpeg_sep_upsample` | public upsample slot at `0x29fce8` |
| `fullsize_upsample` | `0x291ce8` | `0x29f158` | `v18_jpeg_fullsize_upsample` | matching factors at `0x29fdcc` and `0x29fdd4` |
| `noop_upsample` | `0x291cf0` | `0x29f160` | `v18_jpeg_noop_upsample` | unused component at `0x29fd58` |
| `h2v1_upsample` | `0x291cf8` | `0x29f168` | `v18_jpeg_h2v1_upsample` | simple horizontal 2:1 at `0x29ff0c` |
| `h2v1_fancy_upsample` | `0x291f6c` | `0x29f3dc` | `v18_jpeg_h2v1_fancy_upsample` | fancy horizontal 2:1 at `0x29fefc` |
| `h1v2_fancy_upsample` | `0x292220` | `0x29f690` | `v18_jpeg_h1v2_fancy_upsample` | fancy vertical 2:1 at `0x29fd30` and `0x29fd38` |
| `int_upsample` | `0x292360` | `0x29f7d0` | `v18_jpeg_int_upsample` | other integral factors at `0x29fd28` and `0x29fd48` |
| `h2v2_upsample` | `0x292568` | `0x29f9d8` | `v18_jpeg_h2v2_upsample` | simple vertical 2:1 at `0x29fd2c` and `0x29fd4c` |

The target start-pass body records the output height and maximum vertical
sampling factor. The row callback drains converted rows, invokes each
component method for new input rows, calls the color converter, and advances
the input and output row cursors. The full-size method redirects the output
row to the input row. The no-op method returns a null row for an unused
component. The simple h2v1 and h2v2 methods duplicate samples, the fancy
methods interpolate neighboring samples, and the generic method uses the
integral expansion factors stored by the initializer. The fancy vertical
method also sets the context-row flag because it reads adjacent input rows.

These roles match the standard [libjpeg-turbo `jdsample.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdsample.c).
The address delta from each 1.8 function to its Spectron counterpart is
`0xd470`, and every row has identical complete ARM64 feature metrics. This
includes register allocation detail, so this block is stronger than a match
based only on normalized instruction shape.

The machine-readable artifact is
`artifacts/spectron_jpeg_upsampler_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdsample_anchors.py`. The aliases were
applied to a copy of the v278 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v279 database contains 11,696 functions and 625 remaining
default `sub_` names. Its post-reopen SHA-256 is
`131bdd75a02324441e0cd819feacd5ee46958ba7b7d3263eada9fc8601fc5b59`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg postprocessing controller

The v278 pass translated the four unnamed callbacks in the target
decompression postprocessing controller. The target initializer
`v18_jinit_d_post_controller_jpeg_decompress_struct_int` at `0x29ef00` stores
`start_pass_dpost` in its public callback field. Its start-pass switch then
selects direct upsampling, one-pass color quantization, or either pass of
two-pass color quantization.

| Role | 1.8 source | Spectron target | Applied label | Target dispatch |
| --- | ---: | ---: | --- | --- |
| `start_pass_dpost` | `0x2916f8` | `0x29eb68` | `v18_jpeg_start_pass_dpost` | post-controller start slot |
| `post_process_1pass` | `0x291810` | `0x29ec80` | `v18_jpeg_post_process_1pass` | one-pass quantization |
| `post_process_prepass` | `0x2918a0` | `0x29ed10` | `v18_jpeg_post_process_prepass` | two-pass first pass |
| `post_process_2pass` | `0x2919a0` | `0x29ee10` | `v18_jpeg_post_process_2pass` | two-pass output pass |

The target start-pass body resets the strip cursors and maps the buffer mode
to the corresponding processing callback. The one-pass routine calls the
upsampler into a strip buffer, passes those rows to the color quantizer, and
advances the output counter. The prepass routine positions a virtual strip,
lets the upsampler fill it, and scans new rows through the quantizer without an
output buffer. The second-pass routine reads from that virtual strip, limits
the emission by available strip rows, destination rows, and remaining image
height, then sends the rows to the quantizer. These roles match the [libjpeg-
turbo `jdpostct.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdpostct.c).
...
allocation detail. Both source and target functions were default names before
this pass, so the `v18_jpeg_` labels are reviewed semantic roles rather than
recovered debug symbols.

The machine-readable artifact is
`artifacts/spectron_jpeg_postprocessing_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdpostct_anchors.py`. The aliases were
applied to a copy of the v277 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v278 database contains 11,696 functions and 634 remaining
default `sub_` names. Its post-reopen SHA-256 is
`190b42912a47a71415585174e3b78f5cd74a8f97872e3293db5aef2fab7f7228`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg progressive-Huffman decoder

The v277 pass followed the output pipeline into the progressive-Huffman
decoder block. Five target functions immediately precede the named
`v18_jinit_phuff_decoder_jpeg_decompress_struct` initializer at `0x29ea4c`.
The initializer installs the target `start_pass` routine at `0x29ea84`, and
that routine selects the four MCU decoders according to the scan's DC or AC
band and first or successive-approximation mode.

| Role | 1.8 source | Spectron target | Applied label | Target dispatch |
| --- | ---: | ---: | --- | --- |
| `start_pass_phuff_decoder` | `0x290538` | `0x29d9a8` | `v18_jpeg_start_pass_phuff_decoder` | entropy start-pass slot |
| `decode_mcu_AC_refine` | `0x29095c` | `0x29ddcc` | `v18_jpeg_decode_mcu_AC_refine` | AC with `Ah != 0` |
| `decode_mcu_AC_first` | `0x290e3c` | `0x29e2ac` | `v18_jpeg_decode_mcu_AC_first` | AC with `Ah == 0` |
| `decode_mcu_DC_refine` | `0x291154` | `0x29e5c4` | `v18_jpeg_decode_mcu_DC_refine` | DC with `Ah != 0` |
| `decode_mcu_DC_first` | `0x2912f8` | `0x29e768` | `v18_jpeg_decode_mcu_DC_first` | DC with `Ah == 0` |

The target start-pass pseudocode checks the progressive scan bounds and the
successive-approximation relationship, updates the per-coefficient
progression table, selects a decoder, builds derived DC or AC Huffman tables,
and resets the bit reader, EOB run, insufficient-data flag, and restart
counter. Its decoder assignments match the standard [libjpeg-turbo
`jdphuff.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdphuff.c).

The target AC refinement body handles EOBRUN, ZRL, new nonzero coefficients,
correction bits for existing coefficients, and rollback when input suspends.
The AC first body decodes run and size nibbles, writes natural-order
coefficients, and maintains EOBRUN. The DC refinement body reads one bit per
block and applies the current approximation bit. The DC first body decodes
signed DC differences, updates the last-DC predictor for each component, and
scales the result by `Al`. These body-level behaviors distinguish the five
roles independently of their stable address correspondence.

All five source and target rows match in normalized ARM64 feature shape. The
DC refinement row matches the complete recorded feature set. The other four
differ only in register allocation detail. Both source and target functions
were default names before this pass, so the `v18_jpeg_` labels are reviewed
semantic roles rather than recovered debug symbols.

The machine-readable artifact is
`artifacts/spectron_jpeg_progressive_huffman_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdphuff_anchors.py`. The aliases were
applied to a copy of the v276 IDA database and reopened with zero failures.
The broad high-confidence map still verified all 3,641 prior aliases with
zero failures. The v277 database contains 11,696 functions and 638 remaining
default `sub_` names. Its post-reopen SHA-256 is
`633510a1e4fe2f112bf4e5ce73532d894bedda0d67a519a5eceec66078e2318c`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg master and merged upsampler

The v276 pass continued through the next adjacent libjpeg residuals after the
marker reader. Two functions belong to `jdmaster.c`, and five belong to
`jdmerge.c`. The source and target addresses keep the same stable cross-build
delta used by the earlier JPEG passes. The target master initializer is
`v18_jinit_master_decompress_jpeg_decompress_struct` at `0x29cd30`, and the
target merged-upsampler initializer is
`v18_jinit_merged_upsampler_jpeg_decompress_struct` at `0x29d808`.

| Role | 1.8 source | Spectron target | Applied label | Target evidence |
| --- | ---: | ---: | --- | --- |
| `prepare_for_output_pass` | `0x28f2b0` | `0x29c720` | `v18_jpeg_prepare_for_output_pass` | master callback field +0 |
| `finish_output_pass` | `0x28f478` | `0x29c8e8` | `v18_jpeg_finish_output_pass` | master callback field +8 |
| `start_pass_merged_upsample` | `0x28fee0` | `0x29d350` | `v18_jpeg_start_pass_merged_upsample` | merged start callback |
| `merged_1v_upsample` | `0x28fef4` | `0x29d364` | `v18_jpeg_merged_1v_upsample` | one-row wrapper selection |
| `h2v1_merged_upsample` | `0x28ff44` | `0x29d3b4` | `v18_jpeg_h2v1_merged_upsample` | one-row conversion method |
| `h2v2_merged_upsample` | `0x290094` | `0x29d504` | `v18_jpeg_h2v2_merged_upsample` | two-row conversion method |
| `merged_2v_upsample` | `0x290294` | `0x29d704` | `v18_jpeg_merged_2v_upsample` | two-row wrapper selection |

The master initializer pseudocode stores the first two target functions in a
callback pair before it calls `jpeg_calc_output_dimensions`. The target
`prepare_for_output_pass` body selects the active one-pass or two-pass
quantizer, starts the upsampler or color deconverter and the other output
modules, and updates the pass-progress fields. The target
`finish_output_pass` body invokes the two-pass quantizer finish callback when
one is active and increments the master pass counter. These are the two
standard output-pass responsibilities described by the [libjpeg-turbo
`jdmaster.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdmaster.c).

The merged-upsampler initializer writes
`start_pass_merged_upsample` into its first callback field. It then branches
on the maximum vertical sampling factor. The one-row branch selects
`merged_1v_upsample` as the public wrapper and `h2v1_merged_upsample` as its
conversion method. The two-row branch selects `merged_2v_upsample` and
`h2v2_merged_upsample`. The target one-row conversion body reads one Y row and
the two horizontally subsampled chroma rows, expands each chroma sample over
two output pixels, handles an odd final pixel, and uses the precomputed range
limit table. The two-row body performs the corresponding operation for two Y
rows. The wrappers manage input row-group counters and the spare output row.
Those roles match the [libjpeg-turbo `jdmerge.c` implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdmerge.c).

All seven source and target functions have identical complete ARM64 feature
records, including normalized and register-detail hashes. Both source and
target candidates were still default IDA names before this pass, so the
`v18_jpeg_` labels are reviewed role labels rather than claims that original
debug symbols were restored.

The machine-readable artifact is
`artifacts/spectron_jpeg_master_merge_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdmaster_jdmerge_anchors.py`. The seven
aliases were applied to a copy of the v275 IDA database and reopened with zero
failures. The broad high-confidence map still verified all 3,641 prior aliases
with zero failures. The v276 database contains 11,696 functions and 643
remaining default `sub_` names. Its post-reopen SHA-256 is
`787cdf2e8483cd7845dd29f6a488d1d746f10cfb38c43024623555bc4491f0eb`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg marker reader

The v275 pass continued through the target marker-reader block and resolved
nine residual functions around `v18_jinit_marker_reader_jpeg_decompress_struct`
at `0x29c498`. The direct callback table and the call sites inside the marker
loop provide a clean mapping back to `jdmarker.c`.

| Role | 1.8 source | Spectron target | Applied label | Evidence |
| --- | ---: | ---: | --- | --- |
| `get_sof` | `0x28cbb8` | `0x29a028` | `v18_jpeg_get_sof` | SOF dispatches in `read_markers` |
| `examine_app0` | `0x28cffc` | `0x29a46c` | `v18_jpeg_examine_app0` | APP0 metadata paths |
| `skip_variable` | `0x28d2ec` | `0x29a75c` | `v18_jpeg_skip_variable` | default COM and APPn handler |
| `reset_marker_reader` | `0x28d400` | `0x29a870` | `v18_jpeg_reset_marker_reader` | marker-reader reset slot |
| `get_dht` | `0x28d424` | `0x29a894` | `v18_jpeg_get_dht` | DHT dispatch in `read_markers` |
| `save_marker` | `0x28d804` | `0x29ac74` | `v18_jpeg_save_marker` | `jpeg_save_markers` selection |
| `get_interesting_appn` | `0x28db3c` | `0x29afac` | `v18_jpeg_get_interesting_appn` | default APP0 and APP14 handlers |
| `read_markers` | `0x28dd9c` | `0x29b20c` | `v18_jpeg_read_markers` | marker-reader read slot |
| `read_restart_marker` | `0x28eb20` | `0x29bf90` | `v18_jpeg_read_restart_marker` | marker-reader restart slot |

The initializer installs `reset_marker_reader`, `read_markers`, and
`read_restart_marker` directly. It uses `skip_variable` for generic COM and
APPn markers, and `get_interesting_appn` for APP0 and APP14. The public
`jpeg_save_markers` entry can select `save_marker` for retained payloads. The
body of `read_markers` calls the target SOF and DHT helpers and contains the
other small marker operations, including SOI, SOS, DQT, DRI, and marker-byte
scanning. The restart helper contains the corresponding next-marker scan and
restart-number check.

All nine source and target rows match in normalized ARM64 feature shape. Eight
are complete metric matches, and `read_markers` differs only in register
allocation detail. The target body checks the same marker codes, table
indexes, input-buffer boundaries, APP0 metadata, and JPEG status transitions
as the 1.8 body. The role names follow the [libjpeg-turbo `jdmarker.c` source](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdmarker.c).

The machine-readable evidence is
`artifacts/spectron_jpeg_marker_reader_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdmarker_anchors.py`. The aliases were
applied to a copy of the v274 IDA database and reopened with zero failures. The
v275 database contains 11,696 functions and 650 default `sub_` names. Its
post-reopen SHA-256 is
`d16670d36212d270fc1ac476aae6ab5cb540ed4d209c50a8981edf2804dff9ee`. The
checkpoint is
...
## 2026-08-28: Spectron libjpeg input controller

The v274 pass continued through the same libjpeg block and resolved the four
unnamed functions installed by the target `jinit_input_controller` routine at
`0x299fb8`. The initializer writes the callbacks into the controller in the
standard order: marker consumption, reset, scan start, and scan finish.

| Role | 1.8 source | Spectron target | Controller slot | Applied label |
| --- | ---: | ---: | --- | --- |
| `consume_markers` | `0x28c378` | `0x2997e8` | `consume_input` at `0x299fdc` | `v18_jpeg_consume_markers` |
| `reset_input_controller` | `0x28be58` | `0x2992c8` | `reset_input_controller` at `0x299fec` | `v18_jpeg_reset_input_controller` |
| `start_input_pass` | `0x28beb0` | `0x299320` | `start_input_pass` at `0x299ffc` | `v18_jpeg_start_input_pass` |
| `finish_input_pass` | `0x28be44` | `0x2992b4` | `finish_input_pass` at `0x29a008` | `v18_jpeg_finish_input_pass` |

The slot assignment is stronger than address order alone. The 20-byte finish
helper stores `v18_jpeg_consume_markers` into the controller's `consume_input`
field. The 88-byte reset helper clears the scan flags, restores the marker
consumer, resets the error and marker readers, and clears coefficient bits.
The 1224-byte scan-start helper computes MCU membership and dimensions,
materializes missing quantization tables, starts the entropy and coefficient
passes, and selects the coefficient consumer. The 2000-byte marker consumer
reads marker state, validates image dimensions and component sampling, builds
the image and scan state, and returns the controller status values for normal
progress, suspension, and end of image. Its body includes the initial setup
work that the standard `jdinput.c` state machine performs while consuming the
header.

All four target bodies correspond to the 1.8 functions with identical
normalized ARM64 feature metrics. The `start_input_pass` and
`consume_markers` rows are complete metric matches. The reset and finish rows
differ only in register allocation detail. The role names follow the
[libjpeg-turbo `jdinput.c` controller implementation](https://github.com/libjpeg-turbo/libjpeg-turbo/blob/main/src/jdinput.c).

The machine-readable evidence is
`artifacts/spectron_jpeg_input_controller_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_jdinput_controller_anchors.py`. The
aliases were applied to a copy of the v273 IDA database and reopened with zero
failures. The v274 database contains 11,696 functions and 659 default `sub_`
names. Its post-reopen SHA-256 is
`0f9298e5426fde565eaa76b46e31c8241dcc640f0ea27937d3ccd4d31230bc13`. The
checkpoint is
...
## 2026-08-28: Spectron zlib inflate_fast residual

The v272 pass reviewed the next large default-name function in the target
zlib block. The target `v18_inflate` routine at `0x291608` calls the default
function at `0x297764` from `0x292adc`. The corresponding 1.8 `inflate`
routine at `0x284198` calls its default function at `0x28a2f4` from
`0x28566c`.

Both candidates are 2,724-byte ARM64 functions with 135 basic blocks and 678
instructions. Their feature exports have the same size, control-flow shape,
mnemonic hash, normalized opcode and register shape, return count, and string
reference hash. The only complete-metric difference is the register-detail
hash, which is expected from a separate compiler build.

The bodies provide an independent semantic check. Both contain the zlib
errors `invalid literal/length code`, `invalid distance code`, and `invalid
distance too far back`. Both decode literal/length and distance Huffman
entries, handle extra bits, validate the distance, and copy the backreference
into the sliding output window. That is the characteristic `inflate_fast`
hot loop, not a generic decompression wrapper.

The applied target label is `v18_zlib_inflate_fast` at `0x297764`. The `v18_`
prefix is intentional. The source function itself was still `sub_28A2F4`, so
the artifact does not claim that the old debug name was recovered. The
machine-readable evidence is
`artifacts/spectron_zlib_inflate_fast_manual_translation_anchor_20260828.json`,
generated by `tools/generate_spectron_zlib_inflate_fast_anchor.py`. The
reopened v272 database contains 11,696 functions and 669 default `sub_`
...
The residual audit also recorded a rejected collision. Target `0x2a25ac`
matched the source `jpeg_open_backing_store` feature shape in a broad search,
but its pseudocode writes `46` to a JPEG object field and dispatches through
the object's callback. Its callers are in the target one-pass quantizer
initialization block. Since the body and caller context do not describe
`jpeg_open_backing_store`, no name was applied. This rejection is kept in the
notes to make the conservative matching boundary explicit.
...
property block. The source `TBodyPanel` registration row is at `0x38af98` and
the corresponding Spectron row is at `0x39e0e8`. Both records use the standard
0x30-byte property layout, with the getter pointer at offset `0x10`.

| 1.8 callback | Source | Spectron target | Property | Source record | Target record | Role |
| --- | ---: | ---: | --- | ---: | ---: | --- |
| `TBodyPanel_get_bodycacheperplayer` | `0x23bf5c` | `0x245e0c` | `bodycacheperplayer` | `0x38af98` | `0x39e0e8` | getter |

The source and target pseudocode both return the panels-per-player static
integer. The target uses an obfuscated global name, but the registration row,
getter role, normalized instruction shape, and static-return operation agree.
The target callback was the only default function in this property block. It
has one register-detail difference caused by target register allocation, with
no layout change.
...
`analysis/spectron_libqplay_translated_v261.i64`, which contains 11,696
functions and 743 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron TGraalVar property residual aliases
...
Spectron table is at `0x39a460`. Both use 0x30-byte records, with getter and
...
The name setter keeps the source guard: it checks the current name state and
the associated object before assigning the string at object offset +8. The
paused-state pair forwards through virtual slots 152 and 160. The
joined-classes getter follows the script object to its hash, looks up the
`joinedclasses` key, and refreshes the returned array string when an object is
found. The target pseudocode preserves these operations with obfuscated
string and hash helpers.

The property table provides a narrower mapping than broad feature matching.
The two pause callbacks were ambiguous among many same-shaped wrappers, and
the name setter and joined-classes getter were unmatched by the earlier broad
map. The table rows and decompiled operations resolve all four. All four
target callbacks were default `sub_` functions before this pass. The pause
pair has exact normalized and full metric matches. The name setter and
joined-classes getter have rebuilt-wrapper shape changes, which remain visible
in the artifact and account for two register-detail differences.
...
The aliases are materialized in
`analysis/spectron_libqplay_translated_v260.i64`, which contains 11,696
functions and 744 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron GuiTextEditCtrl property aliases
...
Spectron table is at `0x395240`. Both use 0x30-byte records, with getter and
setter pointers at record offsets `0x10` and `0x18`.
...
The denied-sound pair copies or assigns the string at object offset +576. The
history-size getter reads the integer at +584. The input-type getter forwards
to the text-edit input-type method. The show-cursor and tab-completion pairs
read or write bytes at +589 and +588. The text getter forwards through the
control's virtual text getter slot. The target pseudocode preserves each of
these operations.

All nine selected target callbacks were default `sub_` functions before this
pass. The decoded property rows, callback roles, direct field behavior, and
complete ARM64 feature records agree. Every row has exact normalized shape and
full metric match. The password getter and setter, the history-size setter,
the input-type setter, and the text setter already had target names and were
not duplicated.
...
The aliases are materialized in
`analysis/spectron_libqplay_translated_v259.i64`, which contains 11,696
functions and 748 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron GuiStretchCtrl property aliases

The v258 pass translates seven residual callbacks from the `GuiStretchCtrl`
property block. The source table begins at `0x382090` and the corresponding
Spectron table begins at `0x3950f0`. The first three rows are native
`GuiStretchCtrl` properties. The `maxchars` and `text` rows use inherited
`GuiTextCtrl` source callbacks because the 1.8 registration metadata points to
those callbacks. All records use the standard 0x30-byte layout, with getter
and setter pointers at offsets `0x10` and `0x18`.
...
The client-extent getter converts the point at +384. The client-height and
client-width getters read the integers at +388 and +384. The inherited
`maxchars` pair accesses the integer at +464. The text getter and setter
forward through the control's virtual text slots. The target pseudocode
preserves all of these operations.

All seven target callbacks were default `sub_` functions before this pass.
The decoded property rows, inheritance-aware source names, callback roles,
direct field behavior, and ARM64 feature records agree. Every row has an exact
normalized shape and full metric match. The clientextent, clientheight, and
clientwidth setters already had target names and were not duplicated.
...
The aliases are materialized in
`analysis/spectron_libqplay_translated_v258.i64`, which contains 11,696
functions and 757 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron GuiScrollCtrl property aliases
...
Spectron table is at `0x394e50`. Both use 0x30-byte records, with getter and
setter pointers at record offsets `0x10` and `0x18`.
...
The child-margin and scroll-position getters convert the same point fields
from the control into script values. The constant-thumb-height and tile pairs
read or write bytes at +680 and +660. The horizontal and vertical scrollbar
getters index their respective name tables using fields at +672 and +676.
The wheel-scroll getter reads the integer at +684, while the first-responder
pair accesses the byte at +681. The target pseudocode preserves these field
operations and table roles.

All 11 target callbacks were default `sub_` functions before this pass. The
decoded rows, class-local order, callback roles, direct field behavior, and
ARM64 feature records agree. Every row has an exact normalized shape. Nine
rows match the full metric set. The horizontal and vertical scrollbar getters
retain only register-detail differences caused by target register allocation.
The child-margin, hscrollbar, scrollpos, vscrollbar, and wheel-scroll setters
already had target names and were not duplicated.
...
The aliases are materialized in
`analysis/spectron_libqplay_translated_v257.i64`, which contains 11,696
functions and 764 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron GuiGraalCtrl isrendering property aliases

The v256 pass translates the two remaining callbacks for the `isrendering`
property in the `GuiGraalCtrl` property table. The source table row is at
`0x3816d0` and the corresponding Spectron row is at `0x394730`. The property
records use the standard 0x30-byte layout, with getter and setter pointers at
record offsets `0x10` and `0x18`.
...
The getter reads the rendering flag at object offset +456. The setter stores
the incoming flag at the same offset. The target pseudocode is instruction
for instruction equivalent to the source for both callbacks, despite the
target's obfuscated surrounding class names.

Both target callbacks were default `sub_` functions before this pass. The
decoded property row, getter or setter role, direct field behavior, and full
ARM64 feature records agree. Both rows have exact normalized shape and full
metric matches, with no layout or register-detail differences.
...
The aliases are materialized in
`analysis/spectron_libqplay_translated_v256.i64`, which contains 11,696
functions and 775 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron GuiControl property tail aliases
...
corresponding Spectron table is at `0x393700`. Both use 0x30-byte records, with
getter and setter pointers at record offsets `0x10` and `0x18`.
...
The cursor getter forwards the control's cursor string into the script
result. The flickering setter compares and stores the byte at object offset
+408, then invalidates the rectangle when the value changes. The two animation
setters stop ordinary or in-or-out animations when their incoming flag is
false. The target pseudocode preserves these operations, although the helper
names are obfuscated.

All four target callbacks were default `sub_` functions before this pass. The
table-local property names, callback roles, decompiled behavior, and complete
ARM64 feature records agree. Every row has an exact normalized shape and full
metric match, with no layout or register-detail difference. The surrounding
rows already had target names, so this pass only fills the four remaining
tail entries.
...
The aliases are materialized in
`analysis/spectron_libqplay_translated_v255.i64`, which contains 11,696
functions and 777 remaining default `sub_` names. The database SHA-256 is
...
## 2026-08-28: Spectron GuiBitmapButtonCtrl and GuiButtonBaseCtrl property aliases
...
at `0x3803a0`; the target table is at `0x393400`. Each table uses 0x30-byte
records with getter and setter pointers at offsets `0x10` and `0x18`.
...
The bitmap-button getters copy the three image strings from object offsets
+504, +488, and +520 for mouse-over, normal, and pressed states. The setters
forward the incoming string to one shared routine with mode values 1, 0, and
2. This preserves the source distinction between the three image slots even
though the target helper is named with an obfuscated C++ symbol.

The button-type getter indexes the target's button-type name list using the
integer at +468. Its setter scans the same list, compares each name with the
script argument, and stores the matching index. The group-number pair reads and
writes +472. The text getter forwards to the button control's text accessor.
The target pseudocode retains each operation and the same field offsets. The
checked getter and setter and the text setter were already named target ABI
functions, so they are deliberately absent from this residual batch.

All 11 selected target callbacks began as default `sub_` functions. Every row
matches normalized ARM64 instruction shape and has no layout change. Nine
match the complete feature record. The two button-type wrappers have the same
size, instruction count, basic-block count, branches, calls, returns, and
normalized shape as 1.8, but their register-detail hashes differ because of
target register allocation. Table order, callback role, direct field behavior,
shared image modes, and the exact feature comparison all support the aliases.
...
`analysis/spectron_libqplay_translated_v254.i64`, with 11,696 functions and
781 remaining default `sub_` names. Its SHA-256 is
`078918adcdeadc3fa6a894d07e0f9b1929dacaeb2043de3f9952ed8e2f9289e8`.
A clean serial reopen verified all 11 aliases with zero failures. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v254.json`. This was an
offline IDA pass with no DNS, HTTP, TLS, APK, or native-library operation.

## 2026-08-28: Spectron GuiBitmapCtrl property aliases

The v253 pass translates the remaining default callbacks in the
`GuiBitmapCtrl` property table. The source table begins at `0x380250` and the
corresponding Spectron table begins at `0x3932b0`. Both use the usual 0x30-byte
property records, with getter and setter pointers at offsets `0x10` and
`0x18`.
...
There are six registration rows but only five distinct target functions. The
`tile` and `wrap` properties intentionally share `GuiBitmapCtrl_get_tile` in
both builds, so the artifact records one alias and one duplicate registration.
The bitmap setter, bitmap-rectangle setter, and shared tile or wrap setter
already had target names, including their obfuscated ABI labels, and were not
renamed again.

The decompiled behavior is direct and consistent across the builds. The
bitmap getter copies the control string at object offset +472. The rectangle
getter converts the rectangle at +484 to the script rectangle representation.
The full-bitmap getter and setter read or write the byte at +481. The tile or
wrap getter reads the byte at +480. The target's string and rectangle helper
names are obfuscated, but the operations are the same and the object offsets
are unchanged.

All five selected target callbacks began as default `sub_` functions. Their
source and target feature records match in size, instruction count, basic
blocks, branches, calls, returns, mnemonic shape, normalized instruction
shape, and register detail. These matches are supported by the decoded table
names, table-local order, getter or setter role, shared callback structure, and
the decompiled field operations.
...
The disposable database is
`analysis/spectron_libqplay_translated_v253.i64`, with 11,696 functions and
792 remaining default `sub_` names. Its SHA-256 is
`924bca24389cf9c6f8d07ade1f6a7b31726c8bc7991f7fdbacf6e94967a5028c`.
A clean serial reopen verified all five aliases with zero failures. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v253.json`. This was an
offline IDA pass with no DNS, HTTP, TLS, APK, or native-library operation.

## 2026-08-28: Spectron TGUIAnimation property residual aliases

The v252 pass translates the remaining default callbacks in the
`TGUIAnimationProperties` table. The source table begins at `0x3823c0` and the
corresponding Spectron table begins at `0x395420`. Both tables use 0x30-byte
records, with the getter pointer at record offset `0x10` and the setter pointer
at record offset `0x18`.
...
The decoded property order is `currenttime`, `alpha`, `amplitude`, `bounds`,
`delay`, `duration`, `interval`, `rotation`, `sound`, `tabfirstonshow`,
`timing`, and `transition` in both builds. The alpha and rotation getters and
the sound setter already had reviewed names in the target database. The alpha,
rotation, timing, and transition setters retain target ABI jump labels, so
they were not given duplicate `v18_` names in this pass. Only the 17 callbacks
that were still default `sub_` functions are included in the artifact.

The direct scalar wrappers preserve the source fields: `currenttime` reads or
writes the float at object offset +120, `delay` uses +136, `duration` uses
+140, `amplitude` uses +144, and `interval` uses +148. The tab-first-on-show
flag is the byte at +152. The sound getter copies the string at +168. The
bounds getter converts the rectangle returned by `getBounds` into a script
value, and the setter parses a script rectangle before calling `setBounds`.
The timing and transition getters return their corresponding string wrappers.
The same operations are visible in the target pseudocode, although the
supporting helper names are obfuscated.

All 17 rows are high-confidence semantic correspondences supported by table
order, decoded property names, getter or setter role, target function
boundaries, pseudocode, and feature comparison. Every row matches the
normalized instruction shape and the complete recorded metric set. There are
no layout changes or register-detail differences in this group, and all 17
target functions were default names before the rename.
...
The disposable database is
`analysis/spectron_libqplay_translated_v252.i64`, with 11,696 functions and
797 remaining default `sub_` names. Its SHA-256 is
`90a0d433ed61969714d1c853823693ce4286e2d785e159535e7f68e06548af4b`.
A clean serial reopen verified all 17 aliases with zero failures. The
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v252.json`. This was an
offline IDA pass with no DNS, HTTP, TLS, APK, or native-library operation.

## 2026-08-28: Spectron TPlayer findweapon callbacks

The v251 pass resolves both source `findweapon` callbacks instead of treating
the name collision as one function. The property callback is registered in
the `TPlayerProperties` function table, while the second callback is
registered by `TPlayer_initStaticScriptVars` and first resolves the active
player.
...
Both source bodies iterate a weapon collection through the same count and
indexed-item virtual slots, then compare the weapon name with the script
argument and return the matching item. The property callback receives the
player object directly. The static callback reads the global active player
first and returns zero if it is absent. Spectron preserves those two calling
contexts in separate target functions. Its target implementation uses a
temporary `C8THgaTQxF` string and an explicit comparison, while the 1.8 build
uses the original `TString` assignment-and-test helper. The target player
collection has also moved from source field +1976 to target field +2640.

The source property body is 160 bytes and the target is 200 bytes. The source
static body is 172 bytes and the target is 212 bytes. Both pairs retain six
basic blocks and two returns, but neither pair matches the normalized feature
shape because of the rebuilt string and player-layout helpers. This is still
a high-confidence semantic translation because the table rows, argument
contexts, collection slots, and pseudocode all agree. The two target callbacks
were default `sub_` functions before the pass.

The aliases are recorded in
`artifacts/spectron_tplayer_findweapon_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_tplayer_findweapon_anchors.py`. The
disposable database is
`analysis/spectron_libqplay_translated_v251.i64`, with 11,696 functions and
814 remaining default `sub_` names. Its SHA-256 is
`7ab7b98f01f2a4e5241187e1f5864006a7b8b21f6fa163e61fc3c76081a65e9c`.
The v251 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v251.json`. A clean
serial reopen verified both aliases with zero failures. This was an offline
IDA pass with no DNS, HTTP, TLS, APK, or native-library operation.

## 2026-08-28: Spectron TDrawingPanel property and script residual aliases

The v250 pass translates the remaining unnamed callbacks in the drawing-panel
property table and the `drawimagestretched` script table row. The source
property table starts at `0x377d38`, the target `.data` copy starts at
`0x38ad48`, and the source and target function tables for script callbacks
start at `0x377f48` and `0x38af58` respectively.
...
This group contains 12 residual table rows and 10 distinct target callbacks.
The target height getter is registered as both `height` and `parth`, while the
target part-width getter is registered as both `partw` and `width`. Those
duplicate rows are preserved in the artifact. The profile getter, the
`useownprofile` setter, and the `enablecache` setter were already named in the
target database, so they are intentionally not duplicated here.

The direct scalar getters read the same panel offsets in both builds:
`partx` at +172, `party` at +176, `partw` at +180, `height` at +184,
`isclear` at +189, and `enablecache` at +140. The `useownprofile` getter tests
the profile pointer at +152. The profile setter performs the same dynamic cast
to a `GuiControlProfile` before forwarding the assignment. The
`availablefilters` getter builds the script value from the panel's filter-name
list, and the `drawimagestretched` row forwards all ten script arguments to
the implementation method.

All 10 callback anchors match normalized instruction shape. Eight match the
complete recorded ARM64 metrics. The profile setter and available-filter
getter retain register-detail differences caused by rebuilt target helper
classes, but their table positions and pseudocode agree with the source.
...
The disposable database is
`analysis/spectron_libqplay_translated_v250.i64`, with 11,696 functions and
816 remaining default `sub_` names. Its SHA-256 is
`d9fa44a190b1b5014dd9e56651fd416c0e1923cba4e2cd8e361314a9ba7a046f`.
The v250 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v250.json`. A clean
serial reopen verified all 10 renamed callbacks with zero failures. This was
...
table. The source table starts at `0x37b508`, and the corresponding Spectron
table copy starts at `0x38e538`. Each record is 0x30 bytes. The target table
keeps the source property order and getter or setter roles, but the target
callback addresses are in Spectron's obfuscated address space.
...
There are 30 registration rows but only 27 distinct target callbacks. This is
not a conflict in the mapping. The source table itself registers the same
getter for `hearts` and `hp`, the same getter for `shield` and `shieldimg`, and
the same getter for `sword` and `swordimg`. The artifact keeps each table row
so the script-facing names and record addresses remain auditable.

The direct bodies for chat, horse image, on-horse state, shield, and sword
match the complete recorded ARM64 metric set. The seven complete rows are the
three direct single-property rows plus the two duplicate shield rows and two
duplicate sword rows. The remaining 23 rows match normalized instruction
shape and retain a register-detail difference caused by the rebuilt target
ABI. No row requires a layout-change exception. All 30 target callbacks were
default `sub_` functions before this pass.

The aliases are recorded in
`artifacts/spectron_player_property_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_player_property_anchors.py`. The
disposable database is
`analysis/spectron_libqplay_translated_v248.i64`, with 11,696 functions and
855 remaining default `sub_` names. Its SHA-256 is
`780a8ac4584699546ef14a692bd520f13389f5c3918f45b37e33256718028165`.
The v248 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v248.json`. A clean
serial reopen verified all 30 registration rows with zero rename failures.
This was an offline IDA pass with no DNS, HTTP, TLS, APK, or native-library
operation.

## 2026-08-28: Spectron TTilesLayer property aliases

The v247 pass completes the nine-property `TTilesLayer` registration table.
The source table starts at `0x37fb00`, the target table starts at `0x392b60`,
and each record is 0x30 bytes with getter and setter pointers in the usual
property slots.
...
The channel callbacks are direct byte or integer accessors. The layer index is
the corresponding scalar getter, while offset is the point-valued property
with the larger object wrapper. The X, Y, and Z pairs retain the same
coordinate fields in the target. This table-level grouping corrected the
earlier loose address notes for the red and coordinate rows.

All 17 target callbacks were default `sub_` functions before the pass. Every
row matches the complete feature record, including the 36-byte offset getter
and 52-byte offset setter. There are no normalized-shape, control-flow,
layout, or register-detail differences.

The aliases are materialized in
`analysis/spectron_libqplay_translated_v247.i64`. The machine-readable record
is `artifacts/spectron_tiles_layer_property_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_tiles_layer_property_anchors.py`.
The v247 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v247.json`. A clean
serial reopen verified all 17 names with zero failures. This pass changed only
the disposable IDA database and performed no DNS, HTTP, TLS, APK, or
native-library operation.

## 2026-08-28: Spectron TServerNPC showimg callbacks

The v246 pass resolves the two larger image-display callbacks in the same
`TServerNPC` script-function table. The source table starts at `0x37c308`,
the target table starts at `0x38f368`, and the callback pointer is stored at
record offset `+0x18`.
...
Both source functions first choose the active server object, find the image
entry by index, allocate a `TShowImg` object when necessary, reset its image
part, assign the image string, update its coordinates, and refresh the owning
object. `showimg2` additionally writes the Z coordinate. The target
pseudocode preserves this sequence and the same virtual-call offsets. Its
rebuild makes the image-string temporary explicit through a `CanTfaz6bZ`
assignment and cleanup, which accounts for the extra instructions.

The source bodies are 344 bytes and the target bodies are 372 bytes. Both
target functions were default `sub_` names before the pass. Neither row
matches normalized shape or the complete feature record because of the
explicit target temporary, but the table registration, argument shape, direct
callback pointer, and decompiled operation provide a high-confidence semantic
correspondence. Those differences are recorded rather than hidden.

The aliases are materialized in
`analysis/spectron_libqplay_translated_v246.i64`. The machine-readable record
is `artifacts/spectron_server_npc_showimg_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_server_npc_showimg_anchors.py`.
The v246 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v246.json`. A clean
serial reopen verified both names with zero failures. This pass changed only
the disposable IDA database and performed no DNS, HTTP, TLS, APK, or
native-library operation.

## 2026-08-28: Spectron TServerNPC script callbacks

The v245 pass translates seven small callbacks from the `TServerNPC`
script-function table. The source table starts at `0x37c308`; the canonical
Spectron copy starts at `0x38f368`; each record is 0x30 bytes and stores its
...
These wrappers expose the NPC movement and timing policy to GS2. Their
meaning is supported by the decoded script names, direct callback pointers,
and the matching compact wrapper bodies. All seven target callbacks were
default `sub_` functions before the pass. Every row matches normalized ARM64
instruction shape. The only recorded difference is register-detail
allocation, with no size, branch, or layout mismatch.

The aliases are materialized in
`analysis/spectron_libqplay_translated_v245.i64`. The machine-readable record
is `artifacts/spectron_server_npc_script_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_server_npc_script_anchors.py`.
The v245 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v245.json`. A clean
serial reopen verified all seven names with zero failures. This pass changed
only the disposable IDA database and performed no DNS, HTTP, TLS, APK, or
native-library operation.

## 2026-08-28: Spectron TServerNPC property aliases

The v244 pass resolves six residual callbacks in the `TServerNPC` property
table at source address `0x37be28` and target address `0x38ee88`. The table
keeps the same clear-text property names and getter or setter roles in the
stripped target. The larger `TServerNPC` script-function table is intentionally
left for a separate pass because its callbacks have more varied bodies.
...
The horse-image pair reads and writes the same string field. The image getter
is a separate string-return path, and `peltwithnpc` is the boolean policy
flag. The X and Y setters retain the coordinate-update behavior rather than
being simple one-field stores. All six target callbacks were default `sub_`
functions before the pass.

Every row matches normalized ARM64 instruction shape. The horse-image getter
and setter match the complete feature record. The image getter, policy getter,
and two coordinate setters differ only in register-detail metadata. No row
has a size, control-flow, or layout mismatch.

The aliases are materialized in
`analysis/spectron_libqplay_translated_v244.i64`. The machine-readable record
is `artifacts/spectron_server_npc_property_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_server_npc_property_anchors.py`.
The v244 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v244.json`. A clean
serial reopen verified all six names with zero failures. This pass changed
only the disposable IDA database and performed no DNS, HTTP, TLS, APK, or
native-library operation.

## 2026-08-28: Spectron TPlayer and TTranslations property aliases

The v243 pass follows the same decoded registration-table method through the
auxiliary `TPlayer` property block and the translation preferences block. The
source and target tables preserve the same property names, getter or setter
roles, and callback order. This is particularly useful for the `TPlayer`
flags, because each getter and setter is a tiny function and address-only
matching would otherwise be easy to over-trust.
...
The `TPlayer` rows cover the list exposed to script code and three server or
action-policy flags. The `TTranslations` rows keep the configured language
preference separate from the installed-language list. All nine target
callbacks were default `sub_` functions before the pass. Their normalized
instruction shapes match the source rows, with register-detail allocation as
the only recorded metric difference. There are no size, control-flow, or
layout mismatches.

The aliases are materialized in
`analysis/spectron_libqplay_translated_v243.i64`. The machine-readable record
is `artifacts/spectron_player_translation_property_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_player_translation_property_anchors.py`.
The v243 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v243.json`. A clean
serial reopen verified all nine names with zero failures. This pass changed
only the disposable IDA database and performed no DNS, HTTP, TLS, APK, or
native-library operation.

## 2026-08-28: Spectron small world-object property aliases

The v242 pass moves the translation work from the particle system into five
small registration-table families. These tables are useful because the target
still stores the script property names in clear text, and the callback order
matches the 1.8 tables. That gives us stronger evidence than address proximity
alone, especially for a stripped rebuild whose application C++ names are
...
The projectile table separates the ordinary coordinate and motion properties
from the two action-projectile flags. The target preserves the same split,
which is a useful check that the four flag rows were not merely matched by
similar one-field function shapes. The level-link table likewise keeps the
four scalar accessors together, and the level table retains the preload tile
callback in its neighboring server-level registration block.

The two JPEG-quality callbacks are 16-byte wrappers. Their normalized shape
matches exactly, while the target register-detail hash differs. The four
level-link getters and the server-weapon getter match every recorded metric.
Across the full batch, all 22 rows match normalized instruction shape, eight
match the complete feature record, and the remaining 14 differ only in
register-detail metadata. No row has a layout or control-flow mismatch.

The aliases are materialized in
`analysis/spectron_libqplay_translated_v242.i64`. The machine-readable record
is `artifacts/spectron_world_object_property_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_world_object_property_anchors.py`.
The v242 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v242.json`. A clean
reopen verified all 22 names with zero failures. This pass changed only the
disposable IDA database and performed no DNS, HTTP, TLS, APK, or native-library
operation.

## 2026-08-28: Spectron TParticleEmitter GS2 modifier callbacks

The v241 pass resolves the three residual callbacks in the
`TParticleEmitter` script-function table. The source table begins at
`0x38ae10`, the canonical Spectron table begins at `0x39df60`, and the
callback pointer is stored at record offset `+0x18`.

| 1.8 callback | Source | Spectron target | Source record | Target record | Script function |
| --- | ---: | ---: | ---: | ---: | --- |
...
The source wrappers parse script arguments and forward them to the emitter's
global, local, or template modifier path. The target uses obfuscated helper
names, but the function-table names, argument structure, and decompiled
dispatch are the same. Every selected source and target row matches the
complete feature metrics, including size, instruction count, basic-block and
branch shape, calls, normalized hashes, and register detail.

The aliases were applied to a copy of v240 with
`tools/ida_apply_spectron_manual_anchors.py` and verified after reopening with
`tools/ida_verify_spectron_manual_anchors.py`. All three target callbacks were
default `sub_` functions before the pass, and all three reopened with zero
failures. The v241 database contains 11,696 functions and 945 remaining
default `sub_` names. Its SHA-256 is
...
the database identity is in
`artifacts/spectron_translation_checkpoint_20260828_v241.json`.

This was a static IDA pass. It performed no DNS, HTTP, TLS, APK, or
native-library operation.
...
tables. The source `TGaniObjectProperties` table starts at `0x37a5b0`. Spectron
adds `adventure_getbuildtime` before the matching block, so the target `ani`
row begins at `0x38d5d0`. The separate movie-reposition property table is at
`0x37ab50` in the source and `0x38db70` in the target.

| 1.8 callback | Source | Spectron target | Source record | Target record | Property |
| --- | ---: | ---: | ---: | ---: | --- |
...
The `body` getter and `bodyimg` getter share one callback at `0x160cc0` in
the target, just as they share `0x15da68` in the source. This duplicate
registration is recorded in the JSON artifact instead of generating a false
second function. The body setter was already translated earlier.

The remaining getters are simple field copies. `aniparams` copies a string at
offset +304, `anistep` reads +292, `attr` reads +320, `colors` reads +448,
and `gmap` reads +576. The movie-reposition getter and setter access one
global flag. These operations were visible in both decompilations even though
the target class and global names are obfuscated.

Five rows match the complete feature metrics. The gmap getter keeps the same
normalized shape with a register-detail difference. The movie-reposition pair
has the same decompiled operation but a different target instruction form for
global access, which is why those metric differences remain explicit.

All eight target callbacks were default `sub_` functions before the pass. The
aliases were applied to a copy of v237 and verified after reopening. The v238
database contains 11,696 functions and 1,020 remaining default `sub_` names,
with SHA-256
...
generated by `tools/generate_spectron_gani_property_anchors.py`, and the
checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v238.json`. The clean
reopen verified all eight names. No DNS, HTTP, TLS, APK, or native-library
operation was part of this pass.

## 2026-08-28: Spectron TLevelObject property aliases

The v237 pass completes the remaining default callbacks in the target's
`TLevelObject` property table and recovers the target boundary for the `z`
getter. The source table starts at `0x37b048`. The target has the matching
property rows in its `.data` copy at `0x38e068`; a duplicate `.data.rel.ro`
copy exists but is not used as the canonical address here.

| 1.8 callback | Source | Spectron target | Source record | Target record | Property |
| --- | ---: | ---: | ---: | ---: | --- |
| `TLevelObject_getLevel` | `0x1698b0` | `0x16d308` | `0x37b048` | `0x38e068` | `level` |
| `TLevelObject_getX` | `0x1698b8` | `0x16d310` | `0x37b078` | `0x38e098` | `x` |
| `TLevelObject_setX` | `0x1698ec` | `0x16d344` | `0x37b078` | `0x38e098` | `x` |
| `TLevelObject_getY` | `0x169960` | `0x16d3b8` | `0x37b0a8` | `0x38e0c8` | `y` |
| `TLevelObject_setY` | `0x169994` | `0x16d3ec` | `0x37b0a8` | `0x38e0c8` | `y` |
| `TLevelObject_getZ` | `0x169a08-0x169a28` | `0x16d460-0x16d480` | `0x37b0d8` | `0x38e0f8` | `z` |
| `TLevelObject_getLayer` | `0x169a28` | `0x16d480` | `0x37b108` | `0x38e128` | `layer` |

The level getter returns the owning level pointer. The x and y getters add the
tile-space offsets to the virtual base coordinates using 64 pixels per tile.
The setters preserve the ordinary-object clamp from 0.0 through 64.0 before
forwarding the delta through the vtable. The layer getter maps the internal
values into script-visible layer numbers. The target `TLevelObject_setZ`
callback was already translated by an earlier pass and is adjacent to the
newly recovered getter.

The z row needed a little more care. The target property pointer enters
`0x16d460`, but IDA had not created a function there in the v236 database. A
raw ARM64 walk shows the complete sequence:
...
The next known target function begins at `0x16d480`. The source getter has the
same eight-instruction vtable dispatch and the same 32-byte extent. After
materializing the target range and reopening the database, the exported
target feature row matched every source metric, including the vtable offset
of 360 bytes. This is a boundary recovery backed by raw code, not an address
delta guess.

All seven aliases are high confidence and all seven match the complete feature
record. The v237 database contains 11,696 functions and 1,028 remaining
default `sub_` names, with SHA-256
`5229c4d4d67261076bd57c46c8331426ac775afdac6a578f409764b68e5ef872`.
The aliases were reopened with zero failures. The reproducible record is
`artifacts/spectron_level_object_property_manual_translation_anchors_20260828.json`,
generated by `tools/generate_spectron_level_object_property_anchors.py`, and
the checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v237.json`. No DNS, HTTP,
...
GuiControl property tables. The source and target registration records, direct
callback pointers, decompiled behavior, and feature metrics all agree on the
following semantic pairs:
...
The five carried-object getters return whether the action player is carrying
the named bush, sign, vase, stone, or blackstone sprite. The target uses the
rebuilt action-player class and shifted virtual-table offsets, but the
operation is unchanged. The mouse rows read or update the canvas cursor after
subtracting or adding the active player's origin. This explains why the
getter and setter rows share the same target table records.

The `clientheight` and `clientwidth` setters reconstruct a point from the
control bounds and invoke the target's corresponding virtual layout callback.
The `isinanimation` getter checks for a positive animation frame count. All
12 rows have the same normalized feature shape. Nine differ only in
`register_detail_hash`, while the three GuiControl rows match the complete
recorded metrics. This is consistent with a rebuilt target layout, not a
different property implementation.

The aliases were applied to a copy of v234 and reopened with zero failures.
...
This pass changed only a disposable IDA database and performed no DNS, HTTP,
or TLS operation.

## 2026-08-28: Spectron tclient_setplayerhurt boundary recovery

The v234 pass recovers a target function boundary that IDA had not created for
the `tclient_setplayerhurt` property callback. The source callback is the
reviewed function `TClient_script_tclient_setplayerhurt` at `0x1ed158`, with
the source table record at `0x384fb0`. The target table record is `0x398010`,
and its callback pointer at `0x398028` enters `0x1f1b08`.

The target raw entry checks the active-player singleton, the target object
state, and the target no-hurt byte. Its continuation calls the obfuscated
no-hurt helper, preserves the script arguments, and tail-branches to the
already translated `v18_TClient_hurtPlayer` routine at `0x1f1b90`. The next
known target function starts at `0x1f1b94`, so the recovered callback range is
`0x1f1b08-0x1f1b94`. The target field offsets moved with the rebuilt class
layout, which is why the evidence is semantic rather than byte-identical.

The range was materialized and named
`v18_TClient_script_tclient_setplayerhurt`. A clean IDA reopen verified both
the boundary and the alias. The v234 database contains 11,695 functions and
...
## 2026-08-28: Spectron-only property callback labels

The v233 pass names three target callbacks that were previously left as
`sub_` functions because the 1.8 source table did not provide separate
counterparts. The labels are intentionally target-specific:

| Target | Property name | Descriptive label | Evidence |
| ---: | --- | --- | --- |
| `0x1f00f8` | `setdebugdatahandlers` | `spectron_setdebugdatahandlers` | table `0x398670`, callback xref `0x398688` |
| `0x1f0010` | `adventure_setdebugdatahandlersauthorization` | `spectron_adventure_setdebugdatahandlersauthorization` | table `0x3986a0`, callback xref `0x3986b8` |
| `0x1f2160` | `tclient_setotherplayerprops` | `spectron_tclient_setotherplayerprops_adapter` | table `0x398430`, callback xref `0x398448` |

The two debug-handler callbacks use the target's array-style accessors to
collect integer values, clear their separate 1024-byte globals, and copy at
most 256 values. The other-player callback is four instructions long. It
checks that the result is positive and then forwards to the already reviewed
`v18_TClient_updateGlobalPlayer` body. That makes it an ABI adapter, not a
second implementation of the update operation.

There is no demonstrated 1.8 registration for any of these rows. The new
`spectron_` names therefore describe target behavior without pretending to
restore a source symbol. The artifact reports three high-confidence labels,
zero source counterparts, two debug-handler callbacks, and one adapter.

The labels reopened successfully in the v233 disposable database at
`/home/v/Desktop/graal-decomp/analysis/spectron_libqplay_translated_v233.i64`.
The database contains 11,694 functions and 1,068 remaining default `sub_`
names. Its SHA-256 is
