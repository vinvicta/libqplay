...
in `artifacts/ida_translation_validation.json`; the latest role audit and
residual list are in `artifacts/cyassl_static_role_audit_20260826.json` and
...
latest CyaSSL copy has 2,753 named non-ELF functions and 448 default entries.
The private exported inventory is not committed, but its hash and counts are
...
latest copy, leaving 448 default functions. The final address-level
accounting is in `docs/IDA_RESIDUALS.md` and
`artifacts/ida_residual_profile.json`.
...
It is a triage aid, not an IDA rename script. The base 459-entry report and
the latest 448-entry persisted-copy report are generated by
