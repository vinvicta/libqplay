...
`artifacts/unresolved_function_profile.json`. Of the 488 entries, 330 sit in
...
bzip2, 3 in GIF support, and 2 in YAJL. The fourth GPC entry is the small
helper at `0xe01a0`, outside the main contiguous GPC gap, which is called by
`gpc_tristrip_clip` and formats the library's `gpc malloc failure` diagnostic
for tristrip node creation. Another 19 are addresses referenced by
the ELF `.init_array` or `.fini_array`, and one is the 20-byte AArch64 PLT
resolver slot. That leaves 138 application or engine entries without a safe
