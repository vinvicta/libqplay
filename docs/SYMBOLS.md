...
database reports 11,271 total functions, 9,229 with names, and 2,042 default
`sub_` names. Those figures describe the IDA database; the 8,601 count
describes the reproducible symbol import and rename pass. The 69 semantic labels
...
| IDA default `sub_` names | 2,042 |
| Named by IDA but not backed by an ELF record | 1,133 |
