...
The v266 copy adds five target-only labels. These names are intentionally
separate from the 1.8 correspondences. Four come from the target Android
helper table and one describes the nonreturning nanosleep loop called by the
retained Frida detection routines.

| Target address | Applied label | Evidence role |
| ---: | --- | --- |
| `0x24a1d8` | `spectron_getandroidabi` | target table callback `getandroidabi` |
| `0x2500ec` | `spectron_android_getjavaclassexists` | target table callback `getjavaclassexists` |
...
reason the client fails to start. The target-only evidence is in
`artifacts/spectron_android_security_target_only_labels_20260828.json`,
generated by `tools/generate_spectron_android_security_labels.py`. The v266
database has 11,696 functions and 704 default `sub_` names. Its SHA-256 is
`1d9a96fb4db2f9ee1d6353ca2ea94deb30ae5e03085c2bf8ca1286a89f99616f`, and the
checkpoint is
