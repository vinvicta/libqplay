...
`artifacts/unresolved_function_profile.json`. Of the 488 entries, 329 sit in
gaps inside recognizable static library families: 149 in libjpeg, 144 in
FreeType, 14 in zlib, 11 in CyaSSL or its bundled crypto, 3 each in GPC,
bzip2, and GIF support, and 2 in YAJL. Another 19 are addresses referenced by
the ELF `.init_array` or `.fini_array`, and one is the 20-byte AArch64 PLT
resolver slot. That leaves 139 application or engine entries without a safe
