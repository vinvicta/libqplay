...
The name-independent bridge in
`artifacts/cross_version_exact_byte_match_review_20260904.json` compares full
function-byte hashes and sizes against the translated 1.8 `.text` inventory.
It found 957 matching 2.2 functions. The review exposes 574 unique candidates
in `symbols/libqplay_2.2_exact_byte_unique_matches.csv`; 383 repeated byte
sequences remain ambiguous, and 4,825 2.2 functions have no exact match. This
...
The direct-only result contains 170 named undefined symbols, 169 PLT entries,
168 imports with direct transfers, and 3,307 direct `BL` or unconditional `B`
transfers. The complete record is
`artifacts/comparison_2_2_aarch64_import_callsite_review_20260904.json` and is
generated by `tools/audit_two_two_aarch64_import_calls.py`.

Several callsites deserve separate review. `fork` is called twice and
`execvp` once from the same retained function at `0x19bb08`. `dlopen` and
`dlsym` are called from `_Z10_al0FaXqUXPKcPc` at `0x253164`, so runtime loading
is a capability in qplay itself, not only in the companion hook library.
`inet_aton` and `sscanf` are called in the named anti-instrumentation cluster.
The native socket helpers reach `connect`, `bind`, `listen`, `accept`, UDP and
stream send/receive, and the file layer reaches `unlink`. These are confirmed
static import callsites, not proof of trigger, argument control, or runtime
reachability.
