...
| Connector TLS fallback | Trace `THTTPRequest_saveDownloadedData_void` at ARM64 `0x200010` together with `TSocketConnection_read_void` at `0x2074d4` and `TSocketConnection_sendData_void_const_int` at `0x207118` | The private x86_64 control reached the expired TLS responder without an HTTP GET, then issued plain `GET /conf.gs` on the separate fallback port. Returning the archived `/con.png` package for that path did not reach the synthetic game server | Runtime transport transition verified; path-specific response still open |
