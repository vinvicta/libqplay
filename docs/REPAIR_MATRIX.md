...
| Connector TLS fallback | Trace `THTTPRequest_saveDownloadedData_void` at ARM64 `0x200010` together with `TSocketConnection_read_void` at `0x2074d4` and `TSocketConnection_sendData_void_const_int` at `0x207118` | Static IDA evidence shows a nonzero CyaSSL error in HTTPS mode 1 or 2 jumps directly to mode 3 HTTP `conf.gs`; the two-port diagnostic patch is now prepared, but the runtime control still needs a device | Static finding, runtime control prepared |
