...
| Connector TLS fallback | Trace `THTTPRequest_saveDownloadedData_void` at ARM64 `0x200010` together with `TSocketConnection_read_void` at `0x2074d4` and `TSocketConnection_sendData_void_const_int` at `0x207118` | The private x86_64 control reached the expired TLS responder without an HTTP GET, then issued plain `GET /conf.gs` on the separate fallback port. A role-correct `StartScript_Fail` package passed native RSA verification and executed on that path | Runtime transport and local mode-3 package handoff verified; current service response still open |
...
| Signed-package malformed-input controls | Re-sign targeted ZIP mutations with the private test key and serve them through the mode-3 loopback path | Bad signature, missing EOCD, invalid local offset, and oversized entry were rejected without a crash indicator. A one-byte-short ZIP still executed `StartScript_Fail` | Format-integrity concern; add complete end-record and exact-read validation |
...
## Cache and resource-state diagnostics

The connection can reach the file loader even when the final screen still
looks like a loading screen. That makes cache state worth checking before
changing the packet table or disabling more verification.

The packet-102 path appends the response body to a dynamic cached stream and
then calls `TCachedStream_saveAndUpdate_TCachedStream_TString_const` at
`0x1fb744`. The ordinary writer, `TStream_SaveToFile_TString_const_uint` at
`0xf0aa8`, opens the destination, calls `fwrite`, ignores the returned count
and stream error state, and closes the file. The resource object is refreshed
afterward. A storage-full event, interrupted process, or other short write can
therefore leave a partial file at the expected name while the client treats
the resource as present on the next load.

The focused evidence is in
`artifacts/cache_filename_policy_review_20260902.json`, generated by
`tools/generate_cache_filename_policy_review.py`. It also records that
URL-derived names are escaped before entering `webfiles`, while ordinary
resource paths rely on lexical prefixes and `stat` without a visible
canonical-root or no-follow operation. `URLCACHE.txt` is line-oriented and
has no visible byte, entry, or integrity budget.

For a private device run, preserve the native application-files directory,
record the requested filename and final file size, and remove only the
affected resource before retrying. A zero or unexpectedly short file after a
successful packet-102 exchange points to the cache writer or storage layer,
not to TLS. A release repair should write to a temporary file, check the full
write, and atomically replace the destination before refreshing its resource
metadata.
