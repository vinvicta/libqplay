...
The cross-ABI comparison confirms that all four native variants contain the
same connector and CyaSSL marker set and the same 12,820-byte embedded trust
text hash. Their five native dependencies and baseline ELF hardening flags are
also shared. This makes the stale connector material a build-wide compatibility
lead, while the different code layouts and ARM64 `0x10000` alignment still
require a real ARM64 loader test. The compact metadata-only report is
...
The follow-up socket-policy pass resolved the matching rules and found a real
range bug. Exact host and port entries are checked before native socket
creation, but slash-separated ranges compare the comma-separated entry index
instead of the requested port in both outbound and bind paths. The native bind
helper also zeros the IPv4 address, so a permitted listener is wildcard-bound
rather than statically loopback-only. The same pass records the 100 MiB
selected-download limit and ten-request HTTP pool default. Its evidence is in
`artifacts/original_socket_policy_review_20260902.json`. The rerunnable IDA
exporter is `tools/ida_export_original_socket_policy_review.py`.
...
The follow-up method-level DEX pass resolves the actual `invoke-*` references
behind that inventory. It parsed all 11,952 methods with code from the 1,889
class definitions without a bounds error, and records 21 direct calls into 14
of the 18 native methods. It also separates the four application-owned
WebView callers from the bundled Bolts bridge and identifies the renderer's
intent reads before `QPlayMain`. The compact record is
