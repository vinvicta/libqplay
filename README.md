...
class-local method order. It covers the event forwarder, variable-name
enumeration, GS2 dynamic-parameter parser, formatted string execution,
script save and load methods, numeric value accessors, object-array mutation,
and array-string cache update. The target rebuild changes the string and
container wrapper classes, so every row is recorded as a layout-change
translation with its source and target pseudocode fingerprints. The v322
database still has 11,707 functions and zero audited default names, with
6,240 reviewed `v18_` aliases. Its dynamic-symbol coverage classifies 4,564
...
The v326 revision continues through the adjacent format-parameter and
property-runtime block. It translates the `TScriptMachine::FormatParameters`
destructor and eight numeric or string accessors, the complete
`TCallStackEntryProperties` destructor family, the `TProperties` destructor
family, and two derived-property object writers. All 20 rows are high
confidence. Eleven retain exact normalized feature metrics and nine record
the target's added fields or rebuilt string-wrapper behavior. The v326
...
The v338 revision resolves seven raw `THTMLPage` lifecycle methods in the
obfuscated `AS80gaE4zW` class. It covers tab stops, line tags, styles,
sub-pages, and the list stack. Every row has direct source and target
Hex-Rays pseudocode and an exact normalized ARM64 feature match. The v338
...
The v297 revision repairs the target IDA database boundary at `0x2b9870`:
the phantom function is removed, four 16-byte data items are defined, and
the 64 bytes are preserved exactly. The repaired database has one fewer
function and one fewer default `sub_` name.
The v298 revision adds two high-confidence FreeType base cleanup labels. They
identify the internal `destroy_size` and `destroy_face` helpers from
...
default `sub_` names.
The v302 revision adds nine high-confidence FreeType gray-raster labels. They
cover the outline move and line callbacks, conic and cubic curve reduction,
banded glyph conversion, scanline coverage, and span output. Seven pairs match
the complete recorded feature set, while the other two differ only in register
allocation detail. Every pair matches the normalized control-flow shape. The
target now has 481 remaining default `sub_` names.
The v303 revision adds 19 high-confidence FreeType TrueType labels. Three
cover the driver and glyph-loader helpers for kerning, loca-table lookup, and
size initialization. Sixteen cover fixed-point interpreter helpers, movement
and projection callbacks, and the NPUSHW, PUSHW, GC, SCFS, GETINFO, MD, and
...
The v305 revision adds eight high-confidence FreeType TrueType interpreter
labels. Seven are the remaining `Round_*` callbacks selected by
`Compute_Round`, and the eighth is `Compute_Funcs`, which selects projection
and movement callbacks. All eight match normalized ARM64 shape. Seven match
the complete recorded feature set; `Compute_Funcs` differs only in register
allocation detail. The target now has 448 remaining default `sub_` names.
The v306 revision adds six high-confidence FreeType TrueType opcode-state
labels. Four cover the `SZP0`, `SZP1`, `SZP2`, and `SZPS` zone-pointer
...
The v308 revision adds eleven high-confidence FreeType TrueType runtime-tail
labels. They cover the `ENDF`, `FDEF`, `IDEF`, `DELTAP`, `DELTAC`, `SHC`,
`SHP`, and `ISECT` opcode handlers, the `tt_size_done` destructor, the
`Dual_Project` callback, and `TT_Load_Context`. All eleven match normalized
ARM64 shape, nine match the complete recorded feature set, and two differ
only in register allocation detail. The pass also corrects target `0x26bab0`
from the earlier `v18_TT_DotFix14` label to `v18_Project` after the
`Compute_Funcs` callback topology was checked. The target now has 425
remaining default `sub_` names.
The v309 revision adds seven high-confidence FreeType TrueType labels for the
glyph-loader and interpreter helper block. They cover
`load_truetype_glyph`, `TT_Load_Glyph`, `tt_glyph_load`, `Ins_SxVTL`,
`Ins_CALL`, `Ins_LOOPCALL`, and `Ins_UNKNOWN`. All seven match normalized
...
The v322 revision continues from that verified copy and applies twelve
high-confidence TGraalVar runtime aliases. The source and target Hex-Rays
decompilations agree on data flow even where the target replaces `TString`,
`TStringList`, and related containers with obfuscated wrapper classes. The
new aliases cover event delivery, variable enumeration, formatted execution,
script persistence, value lookup, object-array assignment, and array-string
rebuilding. All twelve applications and all twelve reopen checks passed.
...
Its name-origin counts are 6,240 translated `v18_` aliases, 417 target-only
descriptive labels, 990 retained target names, seven JNI exports, and 4,053
other IDA or PLT names. The dynamic-symbol audit still finds 5,782 exact
function starts, 482 data items, 336 other non-code items, and 170 undefined
imports. The v322 records are
...
It continues from the v325 database hash
`229e4729eed1be2759935c1604ac6e3987ffe6fbe91c2b5a0dca16ae344c0757`.
The fresh copy contains 11,707 functions and zero audited default names. Its
name-origin counts are 6,315 translated `v18_` aliases, 417 target-only
descriptive labels, 915 retained target names, seven JNI exports, and 4,053
other IDA or PLT names. The dynamic-symbol audit reports 4,647 source-backed
aliases, 1,803 exact retained names, 143 other retained target names, seven
linker-boundary aliases, 169 PLT veneers, and one undefined `__sF` import
without an in-library veneer.

The v326 pass applies 20 aliases in the target range `0x22c810` through
`0x22cea0`. The format-parameter accessors are especially strong matches:
each target method calls the corresponding script-machine reader, performs
the same biased integer conversion where appropriate, and appears in the
same class-local order. The destructor rows are supported by the D1, D2, and
D0 C++ ABI forms, receiver-adjustment thunks, and matching ownership cleanup.
The two object writers preserve the source callback and temporary-string
cleanup sequence while using the target's rebuilt string classes.
...
The v329 revision continues through the residual `N67CMatrxw` TScriptSpace
block. It adds the exact `freeSuspendedStates` alias and the layout-aware
`joinClass(..., bool)` alias. It also gives two target-only boundaries clear
descriptive names: a `receiveEvent` overload that accepts the target's
`CanTfaz6bZ` event-name wrapper, and a no-argument helper that clears scheduled
events and marks pending actions canceled. The v329 database contains 6,334
reviewed `v18_` aliases, 419 target-only descriptive labels, 894 retained
target names, 4,673 source-backed dynamic rows, and 1,782 exact retained
dynamic names. The source matcher totals are unchanged because the exact
`freeSuspendedStates` row was promoted after direct review rather than added
as a new semantic match.
...
It continues from the v328 database hash
`01e5dc66c7446c46101a09486f23c1a86822e9973b57b5897fa93a4d1f11526a`.
The fresh copy contains 11,707 functions and zero audited default names. Its
name-origin counts are 6,334 translated `v18_` aliases, 419 target-only
descriptive labels, 894 retained target names, seven JNI exports, and 4,053
other IDA or PLT names. Dynamic-symbol coverage reports 4,673 source-backed
aliases, 1,782 exact retained names, 136 other retained target names, seven
linker-boundary aliases, two target-only descriptive rows, 169 PLT veneers,
and one undefined `__sF` import without an in-library veneer.

The v329 pass applies two source-backed aliases in the `N67CMatrxw`
TScriptSpace family. `0x230198` is an exact normalized match for
`TScriptSpace_freeSuspendedStates_void`. `0x233114` is the target rebuild of
`TScriptSpace_joinClass_TString_const_bool`; its control flow and string
literals line up while the target wrapper cleanup makes the body larger. Two
neighboring target-only methods receive descriptive `spectron_` labels:
`0x23332c` is a `receiveEvent` overload with a `CanTfaz6bZ` event-name
argument, and `0x2339b4` clears scheduled events and marks pending actions
canceled. They are deliberately excluded from the source mapping count.
...
The v329 application renamed two source-backed target functions and two
target-only functions, with zero failures. Reopening the fresh copy verified
...
The v338 pass starts from the verified v337 database and resolves seven raw
entries in the `AS80gaE4zW` HTML-page implementation. Direct compact Hex-Rays
pseudocode was captured for every source and target row, and all seven rows
have exact normalized ARM64 feature matches.
...
The initializer methods clear the same member offsets and return the receiver.
The two cleanup methods walk their linked lists, destroy or clear each child,
delete it, and reset the list head. The target method order is interleaved
with already translated methods exactly where the source `THTMLPage` order
places the same lifecycle steps.

All seven aliases were applied to a fresh v337-derived database and verified
after reopening. The v338 database has 11,707 functions and zero audited
default names. Its name audit reports 6,417 translated aliases, 812 retained
target names, 419 target-only descriptive labels, seven JNI exports, and
4,052 other IDA or PLT names. Dynamic coverage reports 4,769 source-backed
aliases, 1,700 exact retained dynamic names, and 5,782 exact dynamic function
starts.
...
`tools/generate_spectron_translation_checkpoint_v338.py`. This is a static
IDA translation checkpoint. It did not patch the APK, rerun the loopback
client, change TLS behavior, contact a game server, or test a live endpoint.
...
The current v297 database is kept locally as
...
The current v302 database is kept locally as
...
The current v305 database is kept locally as
...
The current v308 database is kept locally as
...
The v321 database is kept locally as
`analysis/spectron_libqplay_translated_v321_gui_missing_function_aliases_final.i64`.
...
real `jpeg_fdct_ifast` functions begin at `0x2ac440` and `0x2b98b0`.

The v297 database-hygiene evidence is in
`artifacts/spectron_fdct_literal_pool_boundary_repair_20260828.json`. It
removes the target phantom function at `0x2b9870`, creates four 16-byte data
items named `xmmword_2B9870` through `xmmword_2B98A0`, and preserves every
native byte. Reopen verification passed with zero failures. The checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v297.json`; it records
11,695 functions and 530 remaining default names.

The v298 FreeType cleanup evidence is in
`artifacts/spectron_freetype_base_cleanup_manual_translation_anchors_20260828.json`.
It labels the target `destroy_size` helper at `0x25e304` as
`v18_destroy_size` and the target `destroy_face` helper at `0x260300` as
`v18_destroy_face`. The source counterparts are `0x250e94` and `0x252e90`.
The first helper is passed to `FT_List_Finalize` while releasing a
`FT_Size`; the second releases a face, its glyph slots and sizes, driver data,
stream, internal storage, and the face object. The official FreeType Donut
`src/base/ftobjs.c` source, target callers, reviewed pseudocode, and feature
metrics all support these roles. The machine-readable anchor artifact was
generated by `tools/generate_spectron_freetype_base_cleanup_anchors.py`.
Both aliases were applied to an isolated v297 copy, reopened with zero anchor
failures, and followed by a zero-failure regression over all 3,641 earlier
high-confidence aliases. The v298 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v298.json`; it records
...
The v302 FreeType gray-raster evidence is in
`artifacts/spectron_freetype_gray_internal_manual_translation_anchors_20260828.json`.
It translates nine internal helpers in the gray raster worker. The original
outline callback table at `0x35e4e8` identifies `gray_move_to`, `gray_line_to`,
`gray_render_conic`, and `gray_render_cubic`. The worker call graph then
identifies `gray_convert_glyph`, `gray_convert_glyph_inner`,
`gray_render_scanline`, `gray_render_line`, and `gray_render_span`. Seven
pairs match the complete feature record, while the span writer and inner
conversion helper differ only in register allocation detail. All nine match
the normalized control-flow shape, and all nine names reopened with zero
failures. The v302 checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v302.json`; it records
11,695 functions and 481 remaining default names. Its database SHA-256 is
`71ab01a1215b46848c4ae511fbd065432be83d357d80a2f17a1efa045d814ab5`.

The v303 FreeType TrueType evidence is in
`artifacts/spectron_freetype_tt_interpreter_manual_translation_anchors_20260828.json`.
It translates 19 default target names. The first three are the TrueType
driver and glyph-loader helpers `tt_get_kerning`, `tt_face_get_location`, and
`tt_size_init`. The remaining 16 cover the fixed-point arithmetic and vector
callbacks used by the interpreter, followed by the NPUSHW, PUSHW, GC, SCFS,
GETINFO, MD, and IUP opcode handlers. The source addresses map to the target
block by the same `0xd470` displacement seen throughout this FreeType region.
All 19 pairs match the complete feature record, all 19 names reopened with
zero failures, and the v303 checkpoint records 11,695 functions with 462
...
The v305 FreeType TrueType rounding evidence is in
`artifacts/spectron_freetype_tt_rounding_manual_translation_anchors_20260828.json`.
It translates the seven remaining rounding callbacks from the source's
`Round_*` family, plus `Compute_Funcs`. The seven rounding helpers implement
grid, half-grid, downward, upward, double-grid, super, and precise super-45
modes. `Compute_Funcs` installs the projection and movement callbacks for the
active graphics state.
...
The source and target pseudocode preserve the same arithmetic, callback
selection, and `0xd470` address displacement. All eight names reopened with
zero failures, and the broad regression still verifies all 3,641 earlier
high-confidence aliases. The v305 checkpoint records 11,695 functions with
448 remaining default names. Its database SHA-256 is
`28920bb7cd08c4b94bc16b82bd3a4770e9873b55af3ff2269bec87755876c931`.

The upstream reference for the v305 pass is FreeType's
...
The v306 FreeType TrueType opcode-state evidence is in
`artifacts/spectron_freetype_tt_opcode_state_manual_translation_anchors_20260828.json`.
It translates the four zone-pointer opcode handlers `SZP0`, `SZP1`, `SZP2`,
and `SZPS`, followed by the point-state handlers `AlignRP` and `UTP`.
...
The target handlers are reached from the parallel `TT_RunIns` opcode
dispatcher. The four `SZP` callbacks update the interpreter's zone pointers
and graphics-state selectors. `AlignRP` projects the reference point and
moves the selected points, while `UTP` clears touch state according to the
free vector. The source and target functions preserve the `0xd470`
displacement and all recorded feature metrics. All six names reopened with
zero failures, and the v306 checkpoint records 11,695 functions with 442
remaining default names. Its database SHA-256 is
`4a4eb58f2245daf73e262e87c447370e9e8da96329e5a0ab19ca8e2740ad91df`.

The upstream reference for the v306 pass is FreeType's
[`ttinterp.c`](https://android.googlesource.com/platform/external/freetype/+/f720f0dbcf012d6c984dbbefa0875ef9840458c6/src/truetype/ttinterp.c).
The checkpoint is
...
The v308 FreeType TrueType runtime-tail evidence is in
`artifacts/spectron_freetype_tt_runtime_tail_manual_translation_anchors_20260828.json`.
It translates eleven default target names from the next contiguous block.
Eight are opcode handlers: `ENDF`, `FDEF`, `IDEF`, `DELTAP`, `DELTAC`, `SHC`,
`SHP`, and `ISECT`. The remaining three are `tt_size_done`, `Dual_Project`,
and `TT_Load_Context`. Their source and target functions share the same
`0xd470` displacement. All eleven match normalized feature shape, nine match
the complete recorded ARM64 feature set, and the two remaining pairs differ
only in register allocation detail. The reopened v308 database contains
11,695 functions and 425 remaining default names.

The same pass corrects the earlier projection label at target `0x26bab0`.
The source helper at `0x25e640` is installed by `Compute_Funcs` as the
`func_project` callback. Its arithmetic includes the operation implemented by
`TT_DotFix14`, but its callback role is `Project`. The correction is recorded
separately in
`artifacts/spectron_freetype_tt_projection_name_correction_20260828.json` so
the historical v307 artifact remains reproducible. The target now carries
`v18_Project` at that address.

The machine-readable checkpoint is
`artifacts/spectron_translation_checkpoint_20260828_v308.json`. Both the new
aliases and the correction were reopened and verified with zero failures, and
the full regression still verifies all 3,641 earlier high-confidence aliases.
The v308 database SHA-256 is
`2ac5e911c27e2cc07642c7b8433d54b708a536062114b4b2bea3609524c3bab8`.

The upstream references for the v308 pass are FreeType's
[`ttinterp.c`](https://android.googlesource.com/platform/external/freetype/+/f720f0dbcf012d6c984dbbefa0875ef9840458c6/src/truetype/ttinterp.c)
and
[`ttobjs.c`](https://android.googlesource.com/platform/external/freetype/+/3053d1b9db55099918843889e4809ce97483ca9f/src/truetype/ttobjs.c).
...
The upstream references for the v302 pass are FreeType's
...
`artifacts/spectron_translation_checkpoint_20260828_v307.json`, followed by
`artifacts/spectron_translation_checkpoint_20260828_v308.json`. The reviewed
legacy, Android, script-table, runtime, property, zlib, libjpeg, GIF, and
FreeType labels through v308 were reopened and verified with zero failures.
...
passes, the v306 TrueType opcode-state aliases, the v307 TrueType core
aliases, and the v308 TrueType runtime-tail aliases and projection correction
added in the latest passes.
These passes were
...
* `tools/` contains IDAPython, parsing, replay, and diagnostic patch helpers.
  ...
  `tools/generate_spectron_tgraalvar_runtime_gap_anchors.py` records the v322
  TGraalVar source and target pseudocode fingerprints, metric differences, and
  reviewed method identities. `tools/generate_spectron_translation_checkpoint_v322.py`
  ...
  `tools/generate_spectron_format_parameters_property_anchors.py` records the
  v326 format-parameter, call-stack property, TProperties, and derived-property
  ...
  `tools/generate_spectron_tscript_space_residual_anchors.py` records the v329
  exact and layout-aware TScriptSpace residual aliases, while
  `tools/generate_spectron_tscript_space_residual_labels.py` records the two
  target-only overload and cleanup labels. `tools/generate_spectron_translation_checkpoint_v329.py`
  records their applications, reopen verification, name audit,
  dynamic-symbol coverage, and refreshed semantic map.
