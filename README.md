...
`analysis/spectron_libqplay_translated_v225.i64`. The newest pass adds one
high-confidence alias for the residual popup `get_rows` accessor. The target
uses rebuilt string and hash-list helpers, so the correspondence is recorded
as a semantic wrapper change rather than an exact instruction match. The
reopened v225 copy has 11,694 functions and 1,094 remaining default `sub_`
...
generated by `tools/generate_spectron_gui_popup_rows_anchor.py`, and the v225
checkpoint is
