...
* `symbols/libqplay.function_inventory.csv` and
  ...
  starts, including the 1,861 remaining analysis-created `sub_` entries that have no
  surviving ELF name.
...
* `artifacts/ida_semantic_labels.json` records 250 evidence-backed names
  applied to formerly unnamed login, file-download, handler-table, UI, TLS,
  HTTP, socket, and animation helpers.
