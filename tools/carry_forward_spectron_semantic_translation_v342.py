#!/usr/bin/env python3
"""Carry the stable semantic map onto the v342 input-modifier pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v342 target feature count changed")
    if anchors.get("artifact") != "spectron_input_modifiers_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v342 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v341; "
        "v342 adds the reviewed TInput shift, control, and alt accessor names "
    ...
    result["inputs"]["input_modifiers_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["input_modifiers_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v342 pass changes three target names and does not alter the "
            "function bodies or target function count. The reviewed semantic map "
            "therefore remains the v341 map, with target feature provenance updated "
            "explicitly."
        ),
