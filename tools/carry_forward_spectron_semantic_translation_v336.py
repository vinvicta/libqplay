#!/usr/bin/env python3
"""Carry the stable semantic map onto the v336 Format2 residual pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v336 target feature count changed")
    if anchors.get("artifact") != "spectron_format2_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v336 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v335; "
        "v336 adds the reviewed GSFunctionsInitstaticscriptvars and TFormat2 residual names without changing function bodies"
    )
    ...
    result["inputs"]["format2_residual_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["format2_residual_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v336 pass changes nine target names and does not alter the function "
            "bodies or target function count. The reviewed semantic map therefore "
            "remains the v335 map, with target feature provenance updated explicitly."
        ),
