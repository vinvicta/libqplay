#!/usr/bin/env python3
"""Create the v270 checkpoint from the v269 record and reopen reports."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v270"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v269"
...
def main() -> None:
    ...
    parser.add_argument("--surface-anchor", required=True, type=Path)
    parser.add_argument("--surface-verification", required=True, type=Path)
    parser.add_argument("--translation-verification", required=True, type=Path)
    parser.add_argument("--feature-export", type=Path)
    ...
    surface = load(args.surface_anchor)
    surface_count = surface["summary"]["anchor_count"]
    surface_report = checked_report(
        args.surface_verification, surface_count, "script-table surface"
    )
    ...
    if args.feature_export is not None:
        result["inputs"]["v270_feature_export"] = str(args.feature_export)
        result["inputs"]["v270_feature_export_sha256"] = sha256_path(args.feature_export)
    result["script_table_surface_anchors"] = anchor_record(
        surface,
        args.surface_anchor,
        args.surface_verification,
        surface_report,
    )
    result["interpretation"].append(
        "The v270 database adds 17 reviewed script-table callback anchors, including the correction that separates pushdialog at 0x1b5cf8 from popdialog at 0x1b58c4."
    )
    result["interpretation"].append(
        "The v270 script-table pass reduced the default sub_ count to %d while preserving the prior TGraalVar runtime and target-only labels."
