#!/usr/bin/env python3
"""Create the v286 checkpoint from v285 and the reopened jddctmgr report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v286"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v285"
...
def main() -> None:
    ...
    parser.add_argument("--jddctmgr-anchor", required=True, type=Path)
    parser.add_argument("--jddctmgr-verification", required=True, type=Path)
    ...
    jddctmgr_anchor = load(args.jddctmgr_anchor)
    jddctmgr_count = jddctmgr_anchor["summary"]["anchor_count"]
    jddctmgr_report = checked_report(
        args.jddctmgr_verification, jddctmgr_count, "libjpeg inverse-DCT manager"
    )
    ...
    result["inputs"]["v286_feature_export"] = str(args.feature_export)
    result["inputs"]["v286_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jddctmgr_anchors"] = anchor_record(
        jddctmgr_anchor,
        args.jddctmgr_anchor,
        args.jddctmgr_verification,
        jddctmgr_report,
    )
    result["interpretation"].append(
        "The v286 database adds one high-confidence libjpeg jddctmgr role anchor: the inverse-DCT manager start_pass callback."
    )
    result["interpretation"].append(
        "The target inverse-DCT manager preserves the source method-selection and multiplier-table setup contract, selecting scaled IDCT implementations per component."
    )
    result["interpretation"].append(
        "The v286 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jddctmgr_anchor_count": jddctmgr_count,
            },
            sort_keys=True,
        )
    )
