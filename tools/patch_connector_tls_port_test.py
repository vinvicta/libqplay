...
SPECTRON_ARM64_PATCHES = (
    (0x2065E0, bytes.fromhex("61 37 80 52")),
    (0x206764, bytes.fromhex("61 37 80 52")),
)
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=("original", "spectron"), default="original")
    ...
    if args.arch == "arm64-v8a":
        source_patches = (
            SPECTRON_ARM64_PATCHES
            if args.variant == "spectron"
            else ORIGINAL_ARM64_PATCHES
        )
        patches = [(address, original, arm64_mov_w(1, args.port)) for address, original in source_patches]
    else:
