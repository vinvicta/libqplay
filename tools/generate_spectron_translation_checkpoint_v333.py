#!/usr/bin/env python3
"""Create the v333 checkpoint for the residual THashIntVar destructor pair."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v333"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v332"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "f77edbe5076211bd3bd5a18c549f0c3cbaeeb88d2da7bc9c52a2733c1d87cdc2"
EXPECTED_DATABASE_SHA256 = "c6f31412206a9a893fedf594fac90dff2f13be69f2db28fcda80cc2c67ad7f4d"
EXPECTED_ANCHOR_ARTIFACT = "spectron_hashintvar_residual_manual_translation_anchors_20260829"
...
EXPECTED_TARGETS = ["0x11df60", "0x11df74"]
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v332")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected THashIntVar anchor artifact")
    require(anchors.get("network_contacted") is False, "THashIntVar anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "THashIntVar anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "THashIntVar alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "THashIntVar target order changed")
    require(
        ...
        "THashIntVar alias application did not pass",
    )
    require(
        ...
        "THashIntVar alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v333 dynamic boundary audit changed",
    )
    require(
        ...
        "v333 dynamic symbol coverage changed",
    )
    require(
        ...
        "v333 semantic map changed",
    )
    require(feature_export.get("function_count") == 11707, "v333 feature export count changed")
    require(args.database.is_file(), "database path is not a regular file")
    database_sha256 = sha256_path(args.database)
    require(database_sha256 == EXPECTED_DATABASE_SHA256, "v333 database hash changed")
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v333_hashintvar_anchor_artifact": str(args.anchor_artifact),
        "v333_hashintvar_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v333_hashintvar_application_report": str(args.application_report),
        "v333_hashintvar_application_report_sha256": sha256_path(args.application_report),
        "v333_hashintvar_verification_report": str(args.verification_report),
        "v333_hashintvar_verification_report_sha256": sha256_path(args.verification_report),
        "v333_name_audit": str(args.name_audit),
        "v333_name_audit_sha256": sha256_path(args.name_audit),
        "v333_boundary_audit": str(args.boundary_audit),
        "v333_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v333_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v333_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v333_semantic_map": str(args.semantic_map),
        "v333_semantic_map_sha256": sha256_path(args.semantic_map),
        "v333_feature_export": str(args.feature_export),
        "v333_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["hashintvar_residual_translation_v333"] = {
    ...
    result["name_coverage_audit_v333"] = {
    ...
    result["dynamic_function_boundary_audit_v333"] = {
    ...
    result["dynamic_symbol_coverage_audit_v333"] = {
    ...
    result["semantic_function_translation_v333"] = {
    ...
    result["interpretation"].append(
        "The v333 pass adds two reviewed aliases for the residual THashIntVar complete and deleting destructor boundaries."
    )
    result["interpretation"].append(
        "Both rows are high-confidence layout-change matches. Their source and target bodies reset the vtable, clear the embedded string-like member at offset 8, and the deleting form calls operator delete."
    )
