#!/usr/bin/env python3
"""Create the v290 checkpoint from v289 and the reopened jccoefct report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v290"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v289"
...
def main() -> None:
    ...
    parser.add_argument("--jccoefct-anchor", required=True, type=Path)
    parser.add_argument("--jccoefct-verification", required=True, type=Path)
    ...
    jccoefct_anchor = load(args.jccoefct_anchor)
    jccoefct_count = jccoefct_anchor["summary"]["anchor_count"]
    jccoefct_report = checked_report(
        args.jccoefct_verification, jccoefct_count, "libjpeg compressor coefficient controller"
    )
    ...
    result["inputs"]["v290_feature_export"] = str(args.feature_export)
    result["inputs"]["v290_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jccoefct_anchors"] = anchor_record(
        jccoefct_anchor,
        args.jccoefct_anchor,
        args.jccoefct_verification,
        jccoefct_report,
    )
    result["interpretation"].append(
        "The v290 database adds five high-confidence libjpeg jccoefct role anchors: start_iMCU_row, compress_output, compress_data, start_pass_coef, and compress_first_pass."
    )
    result["interpretation"].append(
    ...
    result["interpretation"].append(
        "The v290 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jccoefct_anchor_count": jccoefct_count,
            },
            sort_keys=True,
        )
    )
