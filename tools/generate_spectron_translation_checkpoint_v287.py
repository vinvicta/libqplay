#!/usr/bin/env python3
"""Create the v287 checkpoint from v286 and the reopened jdhuff report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v287"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v286"
...
def main() -> None:
    ...
    parser.add_argument("--jdhuff-anchor", required=True, type=Path)
    parser.add_argument("--jdhuff-verification", required=True, type=Path)
    ...
    jdhuff_anchor = load(args.jdhuff_anchor)
    jdhuff_count = jdhuff_anchor["summary"]["anchor_count"]
    jdhuff_report = checked_report(
        args.jdhuff_verification, jdhuff_count, "libjpeg baseline Huffman decoder"
    )
    ...
    result["inputs"]["v287_feature_export"] = str(args.feature_export)
    result["inputs"]["v287_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jdhuff_anchors"] = anchor_record(
        jdhuff_anchor,
        args.jdhuff_anchor,
        args.jdhuff_verification,
        jdhuff_report,
    )
    result["interpretation"].append(
        "The v287 database adds two high-confidence libjpeg jdhuff role anchors: start_pass_huff_decoder and decode_mcu."
    )
    result["interpretation"].append(
        "The target huff initializer ties the two routines to the entropy start-pass slot and both baseline MCU decoder slots."
    )
    result["interpretation"].append(
        "The decode_mcu comparison records one approved relocated PAGEOFF operand-bucket difference; instruction count, mnemonic sequence, control-flow counts, and reviewed pseudocode remain aligned."
    )
    result["interpretation"].append(
        "The v287 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jdhuff_anchor_count": jdhuff_count,
            },
            sort_keys=True,
        )
    )
