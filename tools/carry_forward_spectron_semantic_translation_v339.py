#!/usr/bin/env python3
"""Carry the stable semantic map onto the v339 geometry pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v339 target feature count changed")
    if anchors.get("artifact") != "spectron_geometry_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v339 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v338; "
        "v339 adds the reviewed rectangle and region geometry names without changing function bodies"
    )
    ...
    result["inputs"]["geometry_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["geometry_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v339 pass changes four target names and does not alter the function "
            "bodies or target function count. The reviewed semantic map therefore "
            "remains the v338 map, with target feature provenance updated explicitly."
        ),
