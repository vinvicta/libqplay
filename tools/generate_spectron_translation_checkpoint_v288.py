#!/usr/bin/env python3
"""Create the v288 checkpoint from v287 and the reopened jdmainct report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v288"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v287"
...
def main() -> None:
    ...
    parser.add_argument("--jdmainct-anchor", required=True, type=Path)
    parser.add_argument("--jdmainct-verification", required=True, type=Path)
    ...
    jdmainct_anchor = load(args.jdmainct_anchor)
    jdmainct_count = jdmainct_anchor["summary"]["anchor_count"]
    jdmainct_report = checked_report(
        args.jdmainct_verification, jdmainct_count, "libjpeg main decompression controller"
    )
    ...
    result["inputs"]["v288_feature_export"] = str(args.feature_export)
    result["inputs"]["v288_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jdmainct_anchors"] = anchor_record(
        jdmainct_anchor,
        args.jdmainct_anchor,
        args.jdmainct_verification,
        jdmainct_report,
    )
    result["interpretation"].append(
        "The v288 database adds four high-confidence libjpeg jdmainct role anchors: process_data_simple_main, process_data_context_main, process_data_crank_post, and start_pass_main."
    )
    result["interpretation"].append(
        "The target main-controller initializer installs start_pass_main, whose dispatcher selects the simple, context-row, or crank-post callback according to pass mode and upsampler context requirements."
    )
    result["interpretation"].append(
        "The v288 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jdmainct_anchor_count": jdmainct_count,
            },
            sort_keys=True,
        )
    )
