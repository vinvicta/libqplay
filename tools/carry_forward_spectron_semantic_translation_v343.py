#!/usr/bin/env python3
"""Carry the stable semantic map onto the v343 drawing-panel pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v343 target feature count changed")
    if anchors.get("artifact") != "spectron_drawing_panel_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v343 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v342; "
        "v343 adds the reviewed TDrawingPanel clearCache, drawImage, and "
        "drawText names without changing function bodies"
    )
    ...
    result["inputs"]["drawing_panel_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["drawing_panel_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v343 pass changes three target names and does not alter the "
            "function bodies or target function count. The reviewed semantic map "
            "therefore remains the v342 map, with target feature provenance updated "
            "explicitly."
        ),
