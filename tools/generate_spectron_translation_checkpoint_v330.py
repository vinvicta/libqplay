#!/usr/bin/env python3
"""Create the v330 checkpoint for the residual TScriptUniverse pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v330"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v329"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "c84c8bd4abe51302092c82db16003712e870b0ed8a541a9417f6c563f540b6ee"
EXPECTED_DATABASE_SHA256 = "be32d09e08a76b3641beff951644ec78167fcc2735d5fc5ea58f9ee12acf97a1"
EXPECTED_ANCHOR_ARTIFACT = "spectron_tscript_universe_residual_manual_translation_anchors_20260829"
...
EXPECTED_TARGETS = ["0x234bc0", "0x234d98", "0x234db4", "0x235000", "0x235a50", "0x235bf8"]
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v329")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected TScriptUniverse anchor artifact")
    require(anchors.get("network_contacted") is False, "TScriptUniverse anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "TScriptUniverse anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "TScriptUniverse alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "TScriptUniverse target order changed")
    require(
        application.get("apply")
        and application.get("expected_artifact") == EXPECTED_ANCHOR_ARTIFACT
        and application.get("anchor_count") == 6
        and application.get("resolved_count") == 6
        and application.get("renamed_count") == 6
        and application.get("comments_added") == 6
        and application.get("failure_count") == 0
        and application.get("saved")
        and application.get("verified")
        and application.get("verified_name_count") == 6,
        "TScriptUniverse alias application did not pass",
    )
    require(
        verification.get("verified")
        and verification.get("expected_artifact") == EXPECTED_ANCHOR_ARTIFACT
        and verification.get("anchor_count") == 6
        and verification.get("verified_name_count") == 6
        and verification.get("failure_count") == 0
        and verification.get("function_count") == 11707,
        "TScriptUniverse alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v330 dynamic boundary audit changed",
    )
    require(
        ...
        "v330 dynamic symbol coverage changed",
    )
    require(
        semantic_map.get("network_contacted") is False
        and semantic_map.get("summary") == EXPECTED_SEMANTIC_SUMMARY,
        "v330 semantic map changed",
    )
    require(feature_export.get("function_count") == 11707, "v330 feature export count changed")
    require(args.database.is_file(), "database path is not a regular file")
    database_sha256 = sha256_path(args.database)
    require(database_sha256 == EXPECTED_DATABASE_SHA256, "v330 database hash changed")
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v330_tscript_universe_residual_anchor_artifact": str(args.anchor_artifact),
        "v330_tscript_universe_residual_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v330_tscript_universe_residual_application_report": str(args.application_report),
        "v330_tscript_universe_residual_application_report_sha256": sha256_path(args.application_report),
        "v330_tscript_universe_residual_verification_report": str(args.verification_report),
        "v330_tscript_universe_residual_verification_report_sha256": sha256_path(args.verification_report),
        "v330_name_audit": str(args.name_audit),
        "v330_name_audit_sha256": sha256_path(args.name_audit),
        "v330_boundary_audit": str(args.boundary_audit),
        "v330_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v330_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v330_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v330_semantic_map": str(args.semantic_map),
        "v330_semantic_map_sha256": sha256_path(args.semantic_map),
        "v330_feature_export": str(args.feature_export),
        "v330_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["tscript_universe_residual_translation_v330"] = {
    ...
    result["name_coverage_audit_v330"] = {
    ...
    result["dynamic_function_boundary_audit_v330"] = {
    ...
    result["dynamic_symbol_coverage_audit_v330"] = {
    ...
    result["semantic_function_translation_v330"] = {
    ...
    result["interpretation"].append(
        "The v330 pass adds six reviewed TScriptUniverse aliases. The execution-state setters and static-object and free-machine methods are supported by direct pseudocode and class-local order, while both destructor rows are exact normalized matches."
    )
    result["interpretation"].append(
        "Two setter rows differ only in normalized register-detail allocation, and all other target layout changes are recorded in the anchor artifact. This checkpoint records semantic source-role recovery, not restoration of stripped 2.2 symbols."
    )
