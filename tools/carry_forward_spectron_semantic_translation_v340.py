#!/usr/bin/env python3
"""Carry the stable semantic map onto the v340 tile-panel pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v340 target feature count changed")
    if anchors.get("artifact") != "spectron_tiles_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v340 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v339; "
        "v340 adds the reviewed TTilesBlock and TTilesPanel names without changing function bodies"
    )
    ...
    result["inputs"]["tiles_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["tiles_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v340 pass changes four target names and does not alter the function "
            "bodies or target function count. The reviewed semantic map therefore "
            "remains the v339 map, with target feature provenance updated explicitly."
        ),
