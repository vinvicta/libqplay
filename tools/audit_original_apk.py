#!/usr/bin/env python3
"""Build an offline security inventory for the original Graal Classic APK.

The scanner reads only the supplied APK. It parses binary Android XML, DEX
string tables, packaged ELF files, ZIP metadata, and local certificates. It
does not install the package, start an emulator, resolve a hostname, or open a
socket. The result is an evidence index for follow-up review, not a
penetration test.
"""
...
DEFAULT_OUTPUT = ROOT / "artifacts" / "original_apk_security_audit_20260830.json"
...
def parse_binary_manifest(data: bytes) -> dict:
    ...
    while offset + 8 <= document_end:
        ...
        if chunk_type == 0x0001:
            ...
            for index in range(attribute_count):
                ...
                attr_namespace, attr_name, _raw_value = struct.unpack_from("<III", data, current)
                value_size, _res0, value_type, value_data = struct.unpack_from(
                    "<HBBI", data, current + 12
                )
                del value_size, _res0
                ...
                attrs[key] = typed_value(value_type, value_data, strings)
            parent_index = stack[-1] if stack else None
            node = {
                "name": name,
                "parent": nodes[parent_index]["name"] if parent_index is not None else None,
    ...
    manifest_index = next((i for i, node in enumerate(nodes) if node["name"] == "manifest"), None)
    ...
    application_index = next(
        (
            ...
            if node["name"] == "application" and node["parent_index"] == manifest_index
        ),
        None,
    )
        ...
        for filter_index, filter_node in enumerate(nodes):
            ...
            for child in nodes:
                ...
                if child["name"] == "action":
                    ...
                    data_nodes.append(
                        {key: value for key, value in child["attrs"].items() if key.startswith("android:")}
                    )
            filters.append({"actions": actions, "categories": categories, "data": data_nodes})
        attrs = node["attrs"]
        ...
        components.append(
            {
                ...
                "exported": exported,
                "exported_effective": exported if exported is not None else bool(filters),
    ...
    manifest = nodes[manifest_index]["attrs"]
    return {
        "package": attr_value(manifest, "package"),
        "version_code": attr_value(manifest, "versionCode"),
        "version_name": attr_value(manifest, "versionName"),
        "compile_sdk_version": attr_value(manifest, "compileSdkVersion"),
        ...
        "application": {
            ...
            "uses_cleartext_traffic": attr_value(application["attrs"], "usesCleartextTraffic")
            if application
            else None,
            "network_security_config": attr_value(application["attrs"], "networkSecurityConfig")
            if application
            else None,
...
def dex_strings(data: bytes) -> tuple[str, list[str]]:
    ...
    strings = []
    for index in range(string_count):
        string_offset = read_u32(data, string_ids_offset + index * 4)
...
INTERESTING_IMPORTS = {
    "chmod", "connect", "dlopen", "dlsym", "execvp", "fork", "gethostbyname",
    "ioctl", "mprotect", "open", "recv", "recvfrom", "send", "sendto", "socket",
    "unlink", "write",
}
INTERESTING_SYMBOLS = {
    "CyaSSL", "CyaInt_", "TSocketConnection", "TIdentification_", "TFileScripting_",
    "TUpdatePackage_", "TSetup_startExeReplacer", "TFiles_deleteFile",
}
...
def parse_readelf(path: Path) -> dict:
    ...
    imports, defined_functions, named_dynamic_symbols, symbol_hits = [], 0, 0, []
    for line in symbols.splitlines():
        ...
        if len(fields) < 8:
            continue
        symbol_type, section = fields[3], fields[6]
        symbol_name = " ".join(fields[7:])
        base_name = symbol_name.split("@", 1)[0]
        if symbol_name:
            named_dynamic_symbols += 1
        if section == "UND" and base_name:
...
NATIVE_INDICATORS = {
    ...
    "legacy_or_weak_cipher_names": [
        b"SSL_RSA_WITH_RC4_128_SHA", b"SSL_RSA_WITH_RC4_128_MD5",
        b"TLS_RSA_WITH_NULL_SHA", b"TLS_RSA_WITH_NULL_SHA256",
    ],
    "certificate_markers": [b"-----BEGIN CERTIFICATE-----", b"-----BEGIN RSA PRIVATE KEY-----"],
    "native_tls_names": [b"CyaSSL", b"CyaInt_ValidateDate", b"CyaSSL_CTX_load_verify_buffer"],
}
...
def apk_signing_block(data: bytes, central_directory_offset: int) -> dict:
    ...
    if central_directory_offset < 24 or data[central_directory_offset - 16 : central_directory_offset] != magic:
...
def certificate_record(name: str, data: bytes, pem: bool) -> dict | None:
    try:
        ...
        return None
    try:
        certificate = x509.load_pem_x509_certificate(data) if pem else x509.load_der_x509_certificate(data)
    except ValueError:
        return None

    def fields(subject) -> list[dict]:
        return [
            {"oid": item.oid.dotted_string, "name": item.oid._name or item.oid.dotted_string, "value": item.value}
            for item in subject
        ]
    ...
    return {
        ...
        "issuer": fields(certificate.issuer),
...
def extract_v1_certificate(data: bytes, name: str) -> dict | None:
    with tempfile.TemporaryDirectory(prefix="original-apk-cert-") as directory:
        source = Path(directory) / "signature.bin"
        pem_path = Path(directory) / "certificate.pem"
        source.write_bytes(data)
        try:
            subprocess.run(
                ["openssl", "pkcs7", "-inform", "DER", "-in", str(source), "-print_certs", "-out", str(pem_path)],
        ...
        if not pem_path.is_file():
            return None
        record = certificate_record(name, pem_path.read_bytes(), pem=True)
...
def zip_summary(apk_data: bytes, archive: zipfile.ZipFile) -> dict:
    ...
    path_anomalies = sorted(
        name for name in names if name.startswith("/") or any(part == ".." for part in Path(name).parts)
    )
    ...
    return {
        ...
        "encrypted_entries": sorted(info.filename for info in files if info.flag_bits & 0x1),
        "top_entries_by_uncompressed_size": [
...
def selected_components(manifest: dict) -> dict:
    exported = [
        ...
        if component.get("exported_effective") is True
    ]
    deep_link = [
        component for component in exported
        if any(
            data.get("android:scheme", data.get("scheme")) in {"graalclassic", "graalclassicplus"}
            for intent_filter in component["intent_filters"]
            for data in intent_filter["data"]
        )
    ]
    return {"exported_effective": exported, "exported_deep_link_components": deep_link}


def build_findings(manifest: dict, dex_reports: list[dict], native_reports: list[dict], certificates: dict) -> list[dict]:
    findings = []
    application = manifest["application"]
    indicators = {key for report in dex_reports for key in report["indicators"]}
    qplay = next((report for report in native_reports if report["name"].endswith("libqplay.so")), None)
    permissions = {item["name"] for item in manifest["permissions"]}

    if "android.permission.INTERNET" in permissions:
        findings.append({
            "id": "APK-001", "severity": "context", "confidence": "confirmed",
            "title": "The application has network access",
            "evidence": ["AndroidManifest.xml declares android.permission.INTERNET."],
            "impact": "The native connector and game socket can communicate with remote endpoints.",
            "limit": "This is expected for an online game and does not identify a vulnerable endpoint by itself.",
        })
    if application.get("uses_cleartext_traffic") is True:
        findings.append({
            "id": "APK-002", "severity": "medium", "confidence": "confirmed",
            "title": "The manifest permits cleartext traffic",
            "evidence": ["AndroidManifest.xml sets android:usesCleartextTraffic=true."],
            "impact": "Android framework HTTP clients are not blocked by the manifest policy.",
            "limit": "The connector has its own native TLS implementation, so this does not prove that login uses cleartext.",
        })

    deep_links = selected_components(manifest)["exported_deep_link_components"]
    if deep_links:
        findings.append({
            "id": "APK-003", "severity": "medium", "confidence": "confirmed capability, exploitability untested",
            "title": "An exported activity accepts custom Graal URI schemes",
            "evidence": ["An exported component accepts graalclassic:// or graalclassicplus:// intent data."],
            "impact": "Any installed application can request the activity, so URI action, data, and extras are an input boundary.",
            "limit": "This audit did not send a malicious intent or claim code execution from the URI alone.",
        })
    ...
    if {"dynamic_dex_loader", "dynamic_dex_command", "reflection_command"} <= indicators:
        findings.append({
            "id": "APK-005", "severity": "high-interest", "confidence": "confirmed capability, command reachability untested",
            "title": "The WebTop code contains a DEX loading and reflection path",
    ...
    if qplay and qplay["native_strings"].get("legacy_or_weak_cipher_names"):
        findings.append({
            "id": "APK-006", "severity": "medium", "confidence": "confirmed legacy vocabulary, active suite selection unproven",
            "title": "The bundled native TLS implementation contains legacy cipher names",
            "evidence": ["libqplay.so contains RC4 and NULL cipher identifiers and TLS version strings."],
            "impact": "If enabled by negotiation or configuration, legacy suites increase downgrade and confidentiality risk.",
            "limit": "String presence does not show that a weak suite was negotiated. Separate IDA review found peer and hostname verification in the connector path.",
        })
    fabzat = certificates.get("fabzat")
    if fabzat:
        findings.append({
            "id": "APK-007", "severity": "medium", "confidence": "confirmed embedded legacy material, active trust use unproven",
            "title": "The APK carries an expired Fabzat certificate",
            "evidence": [
                "res/raw/fabzat_com.crt is a PEM certificate for admin.fabzat.com.",
                f"Its recorded validity ends at {fabzat['not_after']} and its signature hash is {fabzat['signature_hash']}.",
            ],
            "impact": "Any code that treats this file as a trust anchor or pinned service certificate cannot validate a current service chain after expiry.",
            "limit": "The file's presence does not prove that startup or login consumes it. No live request was made.",
        })

    if qplay and qplay["native_strings"].get("embedded_trust_text"):
        trust = qplay["native_strings"]["embedded_trust_text"]
        findings.append({
            "id": "APK-008", "severity": "medium", "confidence": "confirmed embedded bundle, active validation path separately reviewed",
            "title": "The native library embeds a fixed connector trust bundle",
            "evidence": [f"The ARM64 library contains {trust['bytes']} bytes of trust text at file offset {trust['file_offset']} with SHA-256 {trust['sha256']}."],
            "impact": "A connector that still validates only against this bundle can fail after a service certificate renewal.",
            "limit": "The report does not claim that every native connection uses the bundle or that replacing it is safe without an authorized current chain.",
        })

    cert = certificates.get("apk_v1")
    if cert and cert.get("signature_hash", "").lower() == "sha1":
        findings.append({
            "id": "APK-009", "severity": "medium", "confidence": "confirmed signing metadata",
            "title": "The APK v1 signing certificate uses SHA-1",
            "evidence": [f"The extracted v1 certificate reports signature hash {cert['signature_hash']} and fingerprint {cert['sha256_fingerprint']}."],
            "impact": "SHA-1 is legacy signing metadata and can complicate installation or trust decisions on modern tooling.",
            "limit": "This does not mean the APK was forged. The report does not independently verify every APK signature scheme.",
        })

    if native_reports and all(not report["elf"]["executable_stack"] for report in native_reports):
        findings.append({
            "id": "APK-010", "severity": "informational", "confidence": "confirmed",
            "title": "Packaged native libraries do not request an executable stack",
...
def audit(apk_path: Path) -> dict:
    ...
    with zipfile.ZipFile(apk_path) as archive:
        manifest = parse_binary_manifest(archive.read("AndroidManifest.xml"))
        zip_info = zip_summary(apk_data, archive)
        dex_reports = [
            summarize_dex(info.filename, archive.read(info.filename))
            for info in sorted(archive.infolist(), key=lambda item: item.filename)
            if re.fullmatch(r"classes(?:\d+)?\.dex", info.filename)
        ]
        native_reports = []
        with tempfile.TemporaryDirectory(prefix="original-apk-native-") as directory:
            for info in sorted(
        ...
        if "res/raw/fabzat_com.crt" in archive.namelist():
            certificates["fabzat"] = certificate_record(
                "res/raw/fabzat_com.crt", archive.read("res/raw/fabzat_com.crt"), pem=True
            )
        if "META-INF/CERT.RSA" in archive.namelist():
            certificates["apk_v1"] = extract_v1_certificate(
                archive.read("META-INF/CERT.RSA"), "META-INF/CERT.RSA"
            )
        names = set(archive.namelist())
        signature_entries = sorted(
            name for name in names
            if name.upper().startswith("META-INF/") and Path(name).suffix.upper() in {".SF", ".RSA", ".DSA", ".EC"}
        )

    report = {
        "schema": "libqplay.original-apk-security-audit.v1",
        "tool": "tools/audit_original_apk.py",
        "tool_version": 1,
        "analysis_date": "2026-08-30",
        "analysis_scope": "offline static inventory of the original Graal Online Classic 1.8 APK",
        "network_contacted": False,
        "live_endpoint_tested": False,
        "input": {"name": apk_path.name, "sha256": sha256_bytes(apk_data), "bytes": len(apk_data)},
        "manifest": manifest,
        "component_security_selection": selected_components(manifest),
        "zip": zip_info,
        "dex": dex_reports,
    ...
    report["reproduction"] = {
        "command": "python3 tools/audit_original_apk.py /path/to/GraalOnline+Classic_1.8_APKPure.apk --output artifacts/original_apk_security_audit_20260830.json",
