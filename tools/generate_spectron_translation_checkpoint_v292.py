#!/usr/bin/env python3
"""Create the v292 checkpoint from v291 and the reopened jchuff report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v292"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v291"
...
def main() -> None:
    ...
    parser.add_argument("--jchuff-anchor", required=True, type=Path)
    parser.add_argument("--jchuff-verification", required=True, type=Path)
    ...
    jchuff_anchor = load(args.jchuff_anchor)
    jchuff_count = jchuff_anchor["summary"]["anchor_count"]
    jchuff_report = checked_report(
        args.jchuff_verification, jchuff_count, "libjpeg Huffman encoder"
    )
    ...
    result["inputs"]["v292_feature_export"] = str(args.feature_export)
    result["inputs"]["v292_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jchuff_anchors"] = anchor_record(
        jchuff_anchor,
        args.jchuff_anchor,
        args.jchuff_verification,
        jchuff_report,
    )
    result["interpretation"].append(
        "The v292 database adds five high-confidence libjpeg jchuff role anchors: encode_mcu_gather, finish_pass_huff, encode_mcu_huff, start_pass_huff, and finish_pass_gather."
    )
    result["interpretation"].append(
    ...
    result["interpretation"].append(
        "The v292 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jchuff_anchor_count": jchuff_count,
            },
            sort_keys=True,
        )
    )
