#!/usr/bin/env python3
"""Carry the semantic map onto the v350 layout-aware sound pass."""
...
ANCHOR_ARTIFACT = "spectron_sounds_layout_manual_translation_anchors_20260829"
...
def manual_match(anchor: dict) -> dict:
    ...
    return {
        ...
        "method": "manual-sounds-layout-anchor",
...
def main() -> None:
    ...
    if anchors.get("artifact") != ANCHOR_ARTIFACT:
        raise ValueError("unexpected sound layout artifact")
    anchor_rows = anchors.get("anchors", [])
    if anchors.get("summary", {}).get("anchor_count") != 5 or len(anchor_rows) != 5:
        raise ValueError("sound layout anchor count changed")
    ...
    for anchor in anchor_rows:
        ...
        if source_ea in source_eas or target_ea in target_eas:
            raise ValueError("sound layout anchor is already in the parent semantic map")
    ...
    if expected_categories != {"ambiguous": 1, "unmatched": 4}:
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v349; "
        "v350 adds five reviewed layout-aware TSounds and Java-audio correspondences"
    )
    ...
    result["inputs"]["sounds_layout_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["sounds_layout_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["sounds_layout_translation_v350"] = {
        ...
        "reason": (
            "The target pseudocode preserves the sound-cache initializer, sound-effect constructor, playback state machine, note parser, and Java playback lifecycle. "
            "Changed wrapper layout, preserved literals, caller context, and class-local order support the correspondences without claiming exact normalized shape."
        ),
    }
    result["carried_forward"] = {
        ...
        "reason": (
            "The v350 pass keeps the existing semantic map and promotes one explicit ambiguity plus four unmatched sound rows using layout-aware evidence."
        ),
