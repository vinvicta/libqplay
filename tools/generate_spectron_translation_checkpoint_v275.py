#!/usr/bin/env python3
"""Create the v275 checkpoint from v274 and the reopened marker-reader reports."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v275"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v274"
...
def main() -> None:
    ...
    parser.add_argument("--jdmarker-anchor", required=True, type=Path)
    parser.add_argument("--jdmarker-verification", required=True, type=Path)
    ...
    jdmarker_anchor = load(args.jdmarker_anchor)
    jdmarker_count = jdmarker_anchor["summary"]["anchor_count"]
    jdmarker_report = checked_report(
        args.jdmarker_verification, jdmarker_count, "libjpeg jdmarker"
    )
    ...
    result["inputs"]["v275_feature_export"] = str(args.feature_export)
    result["inputs"]["v275_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jdmarker_anchors"] = anchor_record(
        jdmarker_anchor,
        args.jdmarker_anchor,
        args.jdmarker_verification,
        jdmarker_report,
    )
    result["interpretation"].append(
        "The v275 database adds nine high-confidence libjpeg jdmarker role anchors: get_sof, examine_app0, skip_variable, reset_marker_reader, get_dht, save_marker, get_interesting_appn, read_markers, and read_restart_marker."
    )
    result["interpretation"].append(
        "The nine marker-reader bodies match the corresponding 1.8 functions in normalized feature shape, with eight complete metric matches and one register-allocation-only difference."
    )
    result["interpretation"].append(
        "The v275 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jdmarker_anchor_count": jdmarker_count,
            },
            sort_keys=True,
        )
    )
