#!/usr/bin/env python3
"""Create the v351 checkpoint for the hash-residual translation pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v351"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v350"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "056db23f2015b33134e1fc2bcb99deb5821b96c9590646eb6100c0f7d3462870"
EXPECTED_DATABASE_SHA256 = "0fb0662dffea1f1f6223e0e52745a19505687a79cf47f207280ce098f61b87f0"
EXPECTED_LABEL_ARTIFACT = "spectron_hash_residual_manual_translation_anchors_20260829"
EXPECTED_LABEL_SUMMARY = {
    "address_delta_groups": {"+0xc4c": 1, "+0xc60": 1, "+0xc74": 1, "+0xca0": 1, "+0xce0": 1, "+0xd2c": 1, "+0xdb8": 1, "+0xdf8": 1},
    "anchor_count": 8,
    "exact_shape_anchor_count": 2,
    "high_confidence_count": 8,
    "layout_change_anchor_count": 6,
    "new_context_anchor_count": 8,
    "promoted_unmatched_count": 6,
...
EXPECTED_DYNAMIC_SUMMARY = {
    ...
    "name_match_counts": {
        "item_name_match": 1652,
        "item_name_mismatch": 5118,
        "value_name_match": 1652,
        "value_name_mismatch": 5118,
    },
    "named_dynamic_symbol_count": 6770,
    "status_counts": {
        "exact_retained_dynamic_name": 1652,
        "linker_boundary_alias_mismatch": 7,
        "other_retained_target_name": 119,
        "source_backed_v18_alias": 4800,
...
EXPECTED_SEMANTIC_SUMMARY = {
    ...
    "mapped_functions": 3745,
    "mapped_high_confidence": 3685,
    "mapped_medium_confidence": 60,
    "original_functions": 11308,
    "spectron_functions": 11707,
    "unique_spectron_targets": 3745,
    "unmatched_functions": 598,
}
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v350")
    require(labels.get("artifact") == EXPECTED_LABEL_ARTIFACT, "unexpected hash residual artifact")
    require(labels.get("network_contacted") is False, "hash residual artifact contacted the network")
    require(labels.get("summary") == EXPECTED_LABEL_SUMMARY, "hash residual summary changed")
    require(len(labels.get("anchors", [])) == 8, "hash residual anchor count changed")
    require(
    ...
    require(
        ...
        "hash residual anchor reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        == EXPECTED_BOUNDARY,
        "v351 dynamic boundary audit changed",
    )
    require(
        ...
        "v351 dynamic symbol coverage changed",
    )
    require(
        ...
        and semantic_map.get("carried_forward", {}).get("manual_matches_added") == 8
        and semantic_map.get("carried_forward", {}).get("ambiguous_rows_resolved") == 2
        and semantic_map.get("carried_forward", {}).get("unmatched_rows_promoted") == 6,
        "v351 semantic map changed",
    )
    require(
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v351_hash_residual_anchor_artifact": str(args.label_artifact),
        "v351_hash_residual_anchor_artifact_sha256": sha256_path(args.label_artifact),
        "v351_hash_residual_application_report": str(args.application_report),
        "v351_hash_residual_application_report_sha256": sha256_path(args.application_report),
        "v351_hash_residual_verification_report": str(args.verification_report),
        "v351_hash_residual_verification_report_sha256": sha256_path(args.verification_report),
        "v351_name_audit": str(args.name_audit),
        "v351_name_audit_sha256": sha256_path(args.name_audit),
        "v351_boundary_audit": str(args.boundary_audit),
        "v351_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v351_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v351_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v351_semantic_map": str(args.semantic_map),
        "v351_semantic_map_sha256": sha256_path(args.semantic_map),
        "v351_feature_export": str(args.feature_export),
        "v351_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["hash_residual_translation_v351"] = {
        ...
        "promoted_unmatched_count": semantic_map["carried_forward"]["unmatched_rows_promoted"],
        "exact_shape_count": labels["summary"]["exact_shape_anchor_count"],
        "layout_change_count": labels["summary"]["layout_change_anchor_count"],
    ...
    result["name_coverage_audit_v351"] = {
    ...
    result["dynamic_function_boundary_audit_v351"] = {
    ...
    result["dynamic_symbol_coverage_audit_v351"] = {
    ...
    result["semantic_function_translation_v351"] = {
    ...
    result["interpretation"].append(
        "The v351 pass promotes eight high-confidence hash correspondences. Two source rows were previously ambiguous and six were unmatched. The normal string add and remove wrappers match exactly, while encoded overloads and THashStrings routines retain their roles through changed target string and iterator wrappers."
    )
    result["interpretation"].append(
        "Four target aliases were new in the v351 IDA copy and four were already present. The v351 database proves the names after reopening, keeps all dynamic function boundaries intact, and carries the new source-backed claims in the semantic map."
    )
        ...
        json.dumps(
            {
                ...
                "hash_residual_anchor_count": labels["summary"]["anchor_count"],
                "resolved_ambiguous_rows": semantic_map["carried_forward"]["ambiguous_rows_resolved"],
                "promoted_unmatched_rows": semantic_map["carried_forward"]["unmatched_rows_promoted"],
            },
            sort_keys=True,
        )
    )
