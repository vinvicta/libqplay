#!/usr/bin/env python3
"""Create the v278 checkpoint from v277 and the reopened jdpostct reports."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v278"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v277"
...
def main() -> None:
    ...
    parser.add_argument("--jdpostct-anchor", required=True, type=Path)
    parser.add_argument("--jdpostct-verification", required=True, type=Path)
    ...
    jdpostct_anchor = load(args.jdpostct_anchor)
    jdpostct_count = jdpostct_anchor["summary"]["anchor_count"]
    jdpostct_report = checked_report(
        args.jdpostct_verification, jdpostct_count, "libjpeg postprocessing"
    )
    ...
    result["inputs"]["v278_feature_export"] = str(args.feature_export)
    result["inputs"]["v278_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jdpostct_anchors"] = anchor_record(
        jdpostct_anchor,
        args.jdpostct_anchor,
        args.jdpostct_verification,
        jdpostct_report,
    )
    result["interpretation"].append(
        "The v278 database adds four high-confidence libjpeg jdpostct role anchors: start_pass_dpost, post_process_1pass, post_process_prepass, and post_process_2pass."
    )
    result["interpretation"].append(
        "The target post-controller start-pass switch selects the direct upsampler, one-pass quantization, two-pass prepass, or two-pass output handler according to the buffer mode."
    )
    result["interpretation"].append(
        "The v278 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jdpostct_anchor_count": jdpostct_count,
            },
            sort_keys=True,
        )
    )
