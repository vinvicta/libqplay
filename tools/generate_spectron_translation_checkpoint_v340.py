#!/usr/bin/env python3
"""Create the v340 checkpoint for the residual tile-panel name pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v340"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v339"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "d50a0755bb461dada6b011b4df4ca01f9a0cbaf0112805b0ff1e5ab48764bebe"
EXPECTED_DATABASE_SHA256 = "24a96367fa0730d1a125d146f4fd8e304ba96f6676c15deb2807d085671734d1"
EXPECTED_ANCHOR_ARTIFACT = "spectron_tiles_residual_manual_translation_anchors_20260829"
...
EXPECTED_ALIASES = [
    "v18_TTilesBlock_destroyImage_void",
    "v18_TTilesBlock_isTransparentWithout_int_int",
    "v18_TTilesBlock_isBlackWithout_int_int",
    "v18_TTilesPanel_TTilesPanel_bool",
]
EXPECTED_TARGETS = ["0x23a9a4", "0x23aad4", "0x23ad2c", "0x23ae18"]
EXPECTED_NAME_ORIGINS = {
    "ida_named_or_other": 4052,
    "target_jni_export": 7,
    "target_named_export": 804,
    "target_only_descriptive": 419,
    "translated_v18_alias": 6425,
}
EXPECTED_DYNAMIC_SUMMARY = {
    ...
    "name_match_counts": {
        "item_name_match": 1692,
        "item_name_mismatch": 5078,
        "value_name_match": 1692,
        "value_name_mismatch": 5078,
    },
    "named_dynamic_symbol_count": 6770,
    "status_counts": {
        "exact_retained_dynamic_name": 1692,
        "linker_boundary_alias_mismatch": 7,
        "other_retained_target_name": 120,
        "source_backed_v18_alias": 4779,
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v339")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected tiles anchor artifact")
    require(anchors.get("network_contacted") is False, "tiles anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "tiles anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "tiles alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "tiles target order changed")
    require(
        ...
        "tiles alias application did not pass",
    )
    require(
        ...
        "tiles alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v340 dynamic boundary audit changed",
    )
    require(
        ...
        "v340 dynamic symbol coverage changed",
    )
    require(
        ...
        "v340 semantic map changed",
    )
    require(feature_export.get("function_count") == 11707, "v340 feature export count changed")
    require(args.database.is_file(), "database path is not a regular file")
    database_sha256 = sha256_path(args.database)
    require(database_sha256 == EXPECTED_DATABASE_SHA256, "v340 database hash changed")
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v340_tiles_anchor_artifact": str(args.anchor_artifact),
        "v340_tiles_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v340_tiles_application_report": str(args.application_report),
        "v340_tiles_application_report_sha256": sha256_path(args.application_report),
        "v340_tiles_verification_report": str(args.verification_report),
        "v340_tiles_verification_report_sha256": sha256_path(args.verification_report),
        "v340_name_audit": str(args.name_audit),
        "v340_name_audit_sha256": sha256_path(args.name_audit),
        "v340_boundary_audit": str(args.boundary_audit),
        "v340_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v340_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v340_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v340_semantic_map": str(args.semantic_map),
        "v340_semantic_map_sha256": sha256_path(args.semantic_map),
        "v340_feature_export": str(args.feature_export),
        "v340_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["tiles_translation_v340"] = {
    ...
    result["name_coverage_audit_v340"] = {
    ...
    result["dynamic_function_boundary_audit_v340"] = {
    ...
    result["dynamic_symbol_coverage_audit_v340"] = {
    ...
    result["semantic_function_translation_v340"] = {
    ...
    result["interpretation"].append(
        "The v340 pass adds four reviewed aliases for the residual TTilesBlock image, transparency, black-mask, and TTilesPanel constructor methods."
    )
    result["interpretation"].append(
