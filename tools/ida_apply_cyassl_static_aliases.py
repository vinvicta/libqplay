#!/usr/bin/env python3
"""Apply the reviewed static CyaSSL role aliases to a disposable IDA copy.

The default is review-only. Set ``CYASSL_APPLY_RENAMES=1`` and provide
``CYASSL_SAVE_PATH`` when the opened database is a disposable copy and the
result should be written as a new packed ``.i64`` file. The script never
overwrites its input database.
"""
...
REPORT_PATH = os.environ.get(
    "CYASSL_REPORT_PATH",
    "/tmp/cyassl_static_alias_application_20260826.json",
)


def load_aliases() -> list[dict]:
    document = json.loads(AUDIT_PATH.read_text(encoding="utf-8"))
    if document.get("artifact") != "cyassl_static_role_audit_20260826":
        raise RuntimeError("unexpected CyaSSL role audit artifact")
    return document["aliases"]


def append_comment(ea: int, alias: dict) -> bool:
    comment = (
        "CyaSSL static role alias: "
        + alias["role"]
        + " Evidence: "
        + "; ".join(alias["evidence"])
    )
...
def apply_aliases(aliases: list[dict]) -> dict:
    ...
    if APPLY_RENAMES and SAVE_PATH:
        ...
        if output.suffix != ".i64":
            raise RuntimeError("CYASSL_SAVE_PATH must end in .i64")
        output.parent.mkdir(parents=True, exist_ok=True)
        if not ida_loader.save_database(str(output), ida_loader.DBFL_COMP):
            raise RuntimeError("IDA could not save the packed CyaSSL alias copy")
        result["saved"] = True
    else:
        result["saved"] = False
...
print(json.dumps({key: result[key] for key in ("apply_renames", "alias_count", "resolved_count", "renamed_count", "failure_count", "saved")}, sort_keys=True))
