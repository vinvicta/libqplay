#!/usr/bin/env python3
"""Create the v273 checkpoint from v272 and the reopened residual reports."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v273"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v272"
...
def main() -> None:
    ...
    parser.add_argument("--jpeg-anchor", required=True, type=Path)
    parser.add_argument("--jpeg-verification", required=True, type=Path)
    ...
    jpeg_anchor = load(args.jpeg_anchor)
    jpeg_count = jpeg_anchor["summary"]["anchor_count"]
    jpeg_report = checked_report(
        args.jpeg_verification, jpeg_count, "libjpeg I/O"
    )
    ...
    result["inputs"]["v273_feature_export"] = str(args.feature_export)
    result["inputs"]["v273_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jpeg_io_anchors"] = anchor_record(
        jpeg_anchor,
        args.jpeg_anchor,
        args.jpeg_verification,
        jpeg_report,
    )
    result["interpretation"].append(
        "The v273 database adds six high-confidence libjpeg source and destination callback role anchors. The destination trio is installed by v18_jpeg_stdio_dest, and the source trio is installed by v18_jpeg_stdio_src."
    )
    result["interpretation"].append(
        "The six libjpeg I/O bodies match the corresponding 1.8 functions in normalized feature shape, with two complete metric matches and four register-allocation-only differences."
    )
    result["interpretation"].append(
        "The v273 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases, runtime labels, and target-only labels."
