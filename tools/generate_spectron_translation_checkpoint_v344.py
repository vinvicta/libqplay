#!/usr/bin/env python3
"""Create the v344 checkpoint for the resource-stream crypto pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v344"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v343"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "bb51b5b8ceb13acae2d5843019473ab988f0f931d2a5bce484f0ff3f32103ae8"
EXPECTED_DATABASE_SHA256 = "f8ce3bcf1d63ad596c64525e2621f1c3e9d2bbb544eccfedb36ade2b5d6baaf3"
EXPECTED_ANCHOR_ARTIFACT = "spectron_resource_stream_residual_manual_translation_anchors_20260829"
...
EXPECTED_TARGETS = ["0xede48", "0xedf70"]
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v343")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected resource-stream anchor artifact")
    require(anchors.get("network_contacted") is False, "resource-stream anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "resource-stream anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "resource-stream alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "resource-stream target order changed")
    require(
        ...
        "resource-stream alias application did not pass",
    )
    require(
        ...
        "resource-stream alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v344 dynamic boundary audit changed",
    )
    require(
        ...
        "v344 dynamic symbol coverage changed",
    )
    require(
        ...
        and semantic_map.get("resource_stream_translation_v344", {}).get("manual_match_count") == 2
        and semantic_map.get("resource_stream_translation_v344", {}).get("resolved_ambiguous_count") == 2,
        "v344 semantic map changed",
    )
    require(
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v344_resource_stream_anchor_artifact": str(args.anchor_artifact),
        "v344_resource_stream_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v344_resource_stream_application_report": str(args.application_report),
        "v344_resource_stream_application_report_sha256": sha256_path(args.application_report),
        "v344_resource_stream_verification_report": str(args.verification_report),
        "v344_resource_stream_verification_report_sha256": sha256_path(args.verification_report),
        "v344_name_audit": str(args.name_audit),
        "v344_name_audit_sha256": sha256_path(args.name_audit),
        "v344_boundary_audit": str(args.boundary_audit),
        "v344_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v344_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v344_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v344_semantic_map": str(args.semantic_map),
        "v344_semantic_map_sha256": sha256_path(args.semantic_map),
        "v344_feature_export": str(args.feature_export),
        "v344_feature_export_sha256": sha256_path(args.feature_export),
    }
    ...
    result["name_coverage_audit_v344"] = {
    ...
    result["dynamic_function_boundary_audit_v344"] = {
    ...
    result["dynamic_symbol_coverage_audit_v344"] = {
    ...
    result["semantic_function_translation_v344"] = {
        ...
        "manual_match_count": semantic_map["resource_stream_translation_v344"]["manual_match_count"],
        "resolved_ambiguous_count": semantic_map["resource_stream_translation_v344"]["resolved_ambiguous_count"],
    }
    result["interpretation"] = list(result.get("interpretation", []))
    result["interpretation"].append(
        "The v344 pass adds two high-confidence resource-stream crypto aliases. The source and target bodies have identical normalized ARM64 shape, while distinct encrypt-memory and decrypt-memory calls resolve the adjacent pair that the automatic matcher left ambiguous."
    )
    result["interpretation"].append(
        "Both helpers derive the per-resource eight-byte key from the lower-case resource name, read the TString payload, and transform the bytes in memory. The analysis label does not claim that Spectron retained the original source symbols."
    )
