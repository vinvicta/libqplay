#!/usr/bin/env python3
"""Carry the semantic map onto the v345 resource-object static pass."""
...
def manual_match(anchor: dict) -> dict:
    ...
    return {
        ...
        "method": "manual-resource-object-static-layout",
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v345 target feature count changed")
    if anchors.get("artifact") != "spectron_resource_object_static_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v345 anchor artifact")
    ...
    for row in result.get("ambiguous", []):
        if row.get("original_ea") in source_eas:
            if not target_eas.intersection(row.get("candidate_spectron_eas", [])):
                raise ValueError("resource-object anchor target is absent from its ambiguous candidates")
            removed_ambiguous.append(row)
        else:
            remaining_ambiguous.append(row)
    if len(removed_ambiguous) != len(anchors["anchors"]):
        raise ValueError("not every resource-object anchor resolved an existing ambiguous row")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v344; "
        "v345 adds the three manually resolved TResourceObject and TEncodedFileKey static helpers"
    )
    ...
    result["inputs"]["resource_object_static_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["resource_object_static_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["resource_object_static_translation_v345"] = {
        ...
        "reason": (
            "The automatic matcher left the initializer and the two TEncodedFileKey forms "
            "ambiguous because their normalized bodies are identical across the layout change. "
            "Constructor/destructor order, allocation and clear calls, and matching pseudocode "
            "resolve the three adjacent helpers manually."
        ),
    }
    result["carried_forward"] = {
        ...
        "reason": (
            "The v345 pass keeps the existing automatic map and adds three reviewed "
            "resource-object rows that were explicitly resolved from the parent ambiguous set. "
            "Target feature provenance is updated explicitly."
        ),
