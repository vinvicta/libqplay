#!/usr/bin/env python3
"""Create the v337 checkpoint for the libjpeg helper residual name pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v337"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v336"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "55662a1b9e5989c1e14350ab585015ccb6af0af123f12fab0dcab414f54ca199"
EXPECTED_DATABASE_SHA256 = "391d3bb01245f636760daeb8cef80012e602dfc04423d104a44ceb8e1e4d7113"
EXPECTED_ANCHOR_ARTIFACT = "spectron_libjpeg_helper_residual_manual_translation_anchors_20260829"
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v336")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected libjpeg helper anchor artifact")
    require(anchors.get("network_contacted") is False, "libjpeg helper anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "libjpeg helper anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "libjpeg helper alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "libjpeg helper target order changed")
    require(
        ...
        and application.get("verified_name_count") == 12,
        "libjpeg helper alias application did not pass",
    )
    require(
        ...
        "libjpeg helper alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v337 dynamic boundary audit changed",
    )
    require(
        ...
        "v337 dynamic symbol coverage changed",
    )
    require(
        ...
        "v337 semantic map changed",
    )
    require(feature_export.get("function_count") == 11707, "v337 feature export count changed")
    require(args.database.is_file(), "database path is not a regular file")
    database_sha256 = sha256_path(args.database)
    require(database_sha256 == EXPECTED_DATABASE_SHA256, "v337 database hash changed")
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v337_libjpeg_helper_residual_anchor_artifact": str(args.anchor_artifact),
        "v337_libjpeg_helper_residual_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v337_libjpeg_helper_residual_application_report": str(args.application_report),
        "v337_libjpeg_helper_residual_application_report_sha256": sha256_path(args.application_report),
        "v337_libjpeg_helper_residual_verification_report": str(args.verification_report),
        "v337_libjpeg_helper_residual_verification_report_sha256": sha256_path(args.verification_report),
        "v337_name_audit": str(args.name_audit),
        "v337_name_audit_sha256": sha256_path(args.name_audit),
        "v337_boundary_audit": str(args.boundary_audit),
        "v337_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v337_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v337_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v337_semantic_map": str(args.semantic_map),
        "v337_semantic_map_sha256": sha256_path(args.semantic_map),
        "v337_feature_export": str(args.feature_export),
        "v337_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["libjpeg_helper_residual_translation_v337"] = {
    ...
    result["name_coverage_audit_v337"] = {
    ...
    result["dynamic_function_boundary_audit_v337"] = {
    ...
    result["dynamic_symbol_coverage_audit_v337"] = {
    ...
    result["semantic_function_translation_v337"] = {
    ...
    result["interpretation"].append(
        "The v337 pass adds twelve reviewed aliases for the raw libjpeg memory-manager and utility helpers."
    )
    result["interpretation"].append(
        "All twelve rows are exact normalized ARM64 matches with direct Hex-Rays pseudocode on both builds."
    )
