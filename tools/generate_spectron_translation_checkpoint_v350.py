#!/usr/bin/env python3
"""Create the v350 checkpoint for the layout-aware sound translation pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v350"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v349"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "ede4f9187e01c4a415181f423dd9c7b8467deb38595d399dcb19341fd9203faf"
EXPECTED_DATABASE_SHA256 = "056db23f2015b33134e1fc2bcb99deb5821b96c9590646eb6100c0f7d3462870"
EXPECTED_LABEL_ARTIFACT = "spectron_sounds_layout_manual_translation_anchors_20260829"
EXPECTED_LABEL_SUMMARY = {
    "address_delta_groups": {"+0xbb0": 1, "+0xbd8": 1, "+0xbe8": 1, "+0xbf0": 2},
    "anchor_count": 5,
    "exact_shape_anchor_count": 0,
    "high_confidence_count": 5,
    "layout_change_anchor_count": 5,
    "new_context_anchor_count": 5,
...
EXPECTED_SEMANTIC_SUMMARY = {
    "ambiguous_functions": 1003,
    "mapped_functions": 3737,
    "mapped_high_confidence": 3677,
    "mapped_medium_confidence": 60,
    "original_functions": 11308,
    "spectron_functions": 11707,
    "unique_spectron_targets": 3737,
    "unmatched_functions": 604,
}
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v349")
    require(labels.get("artifact") == EXPECTED_LABEL_ARTIFACT, "unexpected sound layout artifact")
    require(labels.get("network_contacted") is False, "sound layout artifact contacted the network")
    require(labels.get("summary") == EXPECTED_LABEL_SUMMARY, "sound layout summary changed")
    require(len(labels.get("anchors", [])) == 5, "sound layout anchor count changed")
    require(
        ...
        "sound layout anchor application did not pass",
    )
    require(
        ...
        "sound layout anchor reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        == EXPECTED_BOUNDARY,
        "v350 dynamic boundary audit changed",
    )
    require(
        ...
        "v350 dynamic symbol coverage changed",
    )
    require(
        ...
        and semantic_map.get("carried_forward", {}).get("manual_matches_added") == 5
        and semantic_map.get("carried_forward", {}).get("ambiguous_rows_resolved") == 1
        and semantic_map.get("carried_forward", {}).get("unmatched_rows_promoted") == 4,
        "v350 semantic map changed",
    )
    require(
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v350_sounds_layout_anchor_artifact": str(args.label_artifact),
        "v350_sounds_layout_anchor_artifact_sha256": sha256_path(args.label_artifact),
        "v350_sounds_layout_application_report": str(args.application_report),
        "v350_sounds_layout_application_report_sha256": sha256_path(args.application_report),
        "v350_sounds_layout_verification_report": str(args.verification_report),
        "v350_sounds_layout_verification_report_sha256": sha256_path(args.verification_report),
        "v350_name_audit": str(args.name_audit),
        "v350_name_audit_sha256": sha256_path(args.name_audit),
        "v350_boundary_audit": str(args.boundary_audit),
        "v350_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v350_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v350_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v350_semantic_map": str(args.semantic_map),
        "v350_semantic_map_sha256": sha256_path(args.semantic_map),
        "v350_feature_export": str(args.feature_export),
        "v350_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["sounds_layout_translation_v350"] = {
    ...
    result["name_coverage_audit_v350"] = {
    ...
    result["dynamic_function_boundary_audit_v350"] = {
    ...
    result["dynamic_symbol_coverage_audit_v350"] = {
    ...
    result["semantic_function_translation_v350"] = {
    ...
    result["interpretation"].append(
        "The v350 pass promotes five high-confidence layout-aware sound correspondences. One source row was previously ambiguous and four were unmatched. Their target pseudocode preserves the source roles, but changed wrapper layouts keep them separate from exact-shape anchors."
    )
    result["interpretation"].append(
        "All five target aliases and review comments were already present in the v349-derived IDA copy. The v350 database therefore proves stable target naming and reopened boundaries while the semantic map carries the new source-backed claims."
    )
        ...
        json.dumps(
            {
                ...
                "sounds_layout_anchor_count": labels["summary"]["anchor_count"],
