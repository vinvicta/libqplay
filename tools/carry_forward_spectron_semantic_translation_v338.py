#!/usr/bin/env python3
"""Carry the stable semantic map onto the v338 HTML-page lifecycle pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v338 target feature count changed")
    if anchors.get("artifact") != "spectron_html_page_lifecycle_manual_translation_anchors_20260829":
        raise ValueError("unexpected v338 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v337; "
        "v338 adds the reviewed THTMLPage lifecycle names without changing function bodies"
    )
    ...
    result["inputs"]["html_page_lifecycle_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["html_page_lifecycle_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v338 pass changes seven target names and does not alter the function "
            "bodies or target function count. The reviewed semantic map therefore "
            "remains the v337 map, with target feature provenance updated explicitly."
        ),
