#!/usr/bin/env python3
"""Carry the stable semantic map onto the v334 bitmap JPEG name pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v334 target feature count changed")
    if anchors.get("artifact") != "spectron_bitmap_jpeg_static_manual_translation_anchors_20260829":
        raise ValueError("unexpected v334 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v333; "
        "v334 adds the reviewed bitmap JPEG static initializer name without changing function bodies"
    )
    ...
    result["inputs"]["bitmap_jpeg_static_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["bitmap_jpeg_static_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v334 pass changes one target name and does not alter the function "
            "bodies or target function count. The reviewed semantic map therefore "
            "remains the v333 map, with target feature provenance updated explicitly."
        ),
