#!/usr/bin/env python3
"""Create the v335 checkpoint for the Adventure residual name pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v335"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v334"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "c2002066a0412b180afd6abb36fe08f0873403d3068a2a0bdd88deb997101398"
EXPECTED_DATABASE_SHA256 = "dae970eb4edf7237544073da7badb3cfe0bd9d3ccb03e8ec9bde5b5c7de73a16"
EXPECTED_ANCHOR_ARTIFACT = "spectron_adventure_static_residual_manual_translation_anchors_20260829"
...
EXPECTED_TARGETS = ["0x15de64", "0x15e528", "0x15ef90", "0x15f27c"]
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v334")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected Adventure residual anchor artifact")
    require(anchors.get("network_contacted") is False, "Adventure residual anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "Adventure residual anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "Adventure alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "Adventure target order changed")
    require(
        application.get("apply")
        and application.get("expected_artifact") == EXPECTED_ANCHOR_ARTIFACT
        and application.get("anchor_count") == 4
        and application.get("resolved_count") == 4
        and application.get("renamed_count") == 4
        and application.get("comments_added") == 4
        and application.get("failure_count") == 0
        and application.get("saved")
        and application.get("verified")
        and application.get("verified_name_count") == 4,
        "Adventure residual alias application did not pass",
    )
    require(
        ...
        and verification.get("anchor_count") == 4
        and verification.get("verified_name_count") == 4
        and verification.get("failure_count") == 0
        and verification.get("function_count") == 11707,
        "Adventure residual alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v335 dynamic boundary audit changed",
    )
    require(
        ...
        "v335 dynamic symbol coverage changed",
    )
    require(
        ...
        "v335 semantic map changed",
    )
    require(feature_export.get("function_count") == 11707, "v335 feature export count changed")
    require(args.database.is_file(), "database path is not a regular file")
    database_sha256 = sha256_path(args.database)
    require(database_sha256 == EXPECTED_DATABASE_SHA256, "v335 database hash changed")
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v335_adventure_static_residual_anchor_artifact": str(args.anchor_artifact),
        "v335_adventure_static_residual_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v335_adventure_static_residual_application_report": str(args.application_report),
        "v335_adventure_static_residual_application_report_sha256": sha256_path(args.application_report),
        "v335_adventure_static_residual_verification_report": str(args.verification_report),
        "v335_adventure_static_residual_verification_report_sha256": sha256_path(args.verification_report),
        "v335_name_audit": str(args.name_audit),
        "v335_name_audit_sha256": sha256_path(args.name_audit),
        "v335_boundary_audit": str(args.boundary_audit),
        "v335_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v335_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v335_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v335_semantic_map": str(args.semantic_map),
        "v335_semantic_map_sha256": sha256_path(args.semantic_map),
        "v335_feature_export": str(args.feature_export),
        "v335_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["adventure_static_residual_translation_v335"] = {
    ...
    result["name_coverage_audit_v335"] = {
    ...
    result["dynamic_function_boundary_audit_v335"] = {
    ...
    result["dynamic_symbol_coverage_audit_v335"] = {
    ...
    result["semantic_function_translation_v335"] = {
    ...
    result["interpretation"].append(
        "The v335 pass adds four reviewed aliases for the residual GSFunctionsClient and TAdventure static/runtime entries."
    )
    result["interpretation"].append(
        "Three rows are exact normalized matches. The GSFunctionsClient static-variable initializer differs only in register-detail allocation after the target wrapper rebuild."
    )
