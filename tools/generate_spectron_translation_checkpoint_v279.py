#!/usr/bin/env python3
"""Create the v279 checkpoint from v278 and the reopened jdsample report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v279"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v278"
...
def main() -> None:
    ...
    parser.add_argument("--jdsample-anchor", required=True, type=Path)
    parser.add_argument("--jdsample-verification", required=True, type=Path)
    ...
    jdsample_anchor = load(args.jdsample_anchor)
    jdsample_count = jdsample_anchor["summary"]["anchor_count"]
    jdsample_report = checked_report(
        args.jdsample_verification, jdsample_count, "libjpeg upsampler"
    )
    ...
    result["inputs"]["v279_feature_export"] = str(args.feature_export)
    result["inputs"]["v279_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jdsample_anchors"] = anchor_record(
        jdsample_anchor,
        args.jdsample_anchor,
        args.jdsample_verification,
        jdsample_report,
    )
    result["interpretation"].append(
        "The v279 database adds nine high-confidence libjpeg jdsample role anchors: start_pass_upsample, sep_upsample, fullsize_upsample, noop_upsample, h2v1_upsample, h2v1_fancy_upsample, h1v2_fancy_upsample, int_upsample, and h2v2_upsample."
    )
    result["interpretation"].append(
        "The target upsampler initializer preserves the source callback table and sampling-factor dispatch, including the generic integral and simple h2v2 branches."
    )
    result["interpretation"].append(
        "The v279 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jdsample_anchor_count": jdsample_count,
            },
            sort_keys=True,
        )
    )
