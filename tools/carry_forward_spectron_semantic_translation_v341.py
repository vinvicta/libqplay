#!/usr/bin/env python3
"""Carry the stable semantic map onto the v341 color-setter pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v341 target feature count changed")
    if anchors.get("artifact") != "spectron_colorset_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v341 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v340; "
        "v341 adds the reviewed GuiControl color-setter names without changing function bodies"
    )
    ...
    result["inputs"]["colorset_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["colorset_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v341 pass changes four target names and does not alter the function "
            "bodies or target function count. The reviewed semantic map therefore "
            "remains the v340 map, with target feature provenance updated explicitly."
        ),
