#!/usr/bin/env python3
"""Create the v345 checkpoint for the resource-object static helper pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v345"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v344"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "d7d4887e86d0570d7f2518bd545d3caa139aa0a1c5e0ca5c39d5c00b50b7669a"
EXPECTED_DATABASE_SHA256 = "0b455dfb6777c8ca571f86e19612d30a7dca6c3d9b9e47590e31a6bfcea4442f"
EXPECTED_ANCHOR_ARTIFACT = "spectron_resource_object_static_residual_manual_translation_anchors_20260829"
EXPECTED_ANCHOR_SUMMARY = {
    "anchor_count": 3,
    "exact_metric_anchor_count": 0,
    "high_confidence_count": 3,
    "layout_change_anchor_count": 3,
    "new_context_anchor_count": 3,
    "normalized_shape_match_count": 3,
    "semantic_promotion_count": 0,
    "source_pseudocode_count": 3,
    "target_pseudocode_count": 3,
}
EXPECTED_ALIASES = [
    "v18_TResourceObject_initStaticVars_void",
    "v18_TEncodedFileKey_TEncodedFileKey",
    "v18_TEncodedFileKey_TEncodedFileKey__2",
]
EXPECTED_TARGETS = ["0xf1910", "0xf1940", "0xf1980"]
EXPECTED_NAME_ORIGINS = {
    "ida_named_or_other": 4052,
    "target_jni_export": 7,
    "target_named_export": 789,
...
EXPECTED_DYNAMIC_SUMMARY = {
    ...
    "name_match_counts": {
        "item_name_match": 1677,
        "item_name_mismatch": 5093,
        "value_name_match": 1677,
        "value_name_mismatch": 5093,
    },
    "named_dynamic_symbol_count": 6770,
    "status_counts": {
        "exact_retained_dynamic_name": 1677,
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v344")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected resource-object anchor artifact")
    require(anchors.get("network_contacted") is False, "resource-object anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "resource-object anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "resource-object alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "resource-object target order changed")
    require(
        ...
        "resource-object alias application did not pass",
    )
    require(
        ...
        "resource-object alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v345 dynamic boundary audit changed",
    )
    require(
        ...
        "v345 dynamic symbol coverage changed",
    )
    require(
        ...
        and semantic_map.get("resource_object_static_translation_v345", {}).get("manual_match_count") == 3
        and semantic_map.get("resource_object_static_translation_v345", {}).get("resolved_ambiguous_count") == 3,
        "v345 semantic map changed",
    )
    require(
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v345_resource_object_static_anchor_artifact": str(args.anchor_artifact),
        "v345_resource_object_static_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v345_resource_object_static_application_report": str(args.application_report),
        "v345_resource_object_static_application_report_sha256": sha256_path(args.application_report),
        "v345_resource_object_static_verification_report": str(args.verification_report),
        "v345_resource_object_static_verification_report_sha256": sha256_path(args.verification_report),
        "v345_name_audit": str(args.name_audit),
        "v345_name_audit_sha256": sha256_path(args.name_audit),
        "v345_boundary_audit": str(args.boundary_audit),
        "v345_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v345_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v345_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v345_semantic_map": str(args.semantic_map),
        "v345_semantic_map_sha256": sha256_path(args.semantic_map),
        "v345_feature_export": str(args.feature_export),
        "v345_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["resource_object_static_translation_v345"] = {
        ...
        "normalized_shape_match_count": anchors["summary"]["normalized_shape_match_count"],
        ...
        "resolved_ambiguous_count": 3,
    }
    result["name_coverage_audit_v345"] = {
    ...
    result["dynamic_function_boundary_audit_v345"] = {
    ...
    result["dynamic_symbol_coverage_audit_v345"] = {
    ...
    result["semantic_function_translation_v345"] = {
        ...
        "manual_match_count": semantic_map["resource_object_static_translation_v345"]["manual_match_count"],
        "resolved_ambiguous_count": semantic_map["resource_object_static_translation_v345"]["resolved_ambiguous_count"],
    }
    result["interpretation"] = list(result.get("interpretation", []))
    result["interpretation"].append(
        "The v345 pass adds three high-confidence resource-object static aliases. The source and target bodies have identical normalized ARM64 shape, while constructor or destructor role, adjacent method order, allocation, clear calls, and matching pseudocode resolve the cluster across the changed layout."
    )
    result["interpretation"].append(
        "The target exposes the TEncodedFileKey destructor forms as D2/D1 and D0 weak aliases. Renaming the D2 entry changes both dynamic spellings to source-backed aliases, so source-backed dynamic coverage rises by four while exact retained names fall by three and other retained target names fall by one."
    )
        ...
        json.dumps(
            {
                ...
                "anchor_count": anchors["summary"]["anchor_count"],
                "mapped_functions": semantic_map["summary"]["mapped_functions"],
