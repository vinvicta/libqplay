#!/usr/bin/env python3
"""Create the v274 checkpoint from v273 and the reopened controller reports."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v274"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v273"
...
def main() -> None:
    ...
    parser.add_argument("--jdinput-anchor", required=True, type=Path)
    parser.add_argument("--jdinput-verification", required=True, type=Path)
    ...
    jdinput_anchor = load(args.jdinput_anchor)
    jdinput_count = jdinput_anchor["summary"]["anchor_count"]
    jdinput_report = checked_report(
        args.jdinput_verification, jdinput_count, "libjpeg jdinput controller"
    )
    ...
    result["inputs"]["v274_feature_export"] = str(args.feature_export)
    result["inputs"]["v274_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jdinput_controller_anchors"] = anchor_record(
        jdinput_anchor,
        args.jdinput_anchor,
        args.jdinput_verification,
        jdinput_report,
    )
    result["interpretation"].append(
        "The v274 database adds four high-confidence libjpeg jdinput controller role anchors: consume_markers, reset_input_controller, start_input_pass, and finish_input_pass."
    )
    result["interpretation"].append(
        "The four controller bodies match the corresponding 1.8 functions in normalized feature shape, with two complete metric matches and two register-allocation-only differences."
    )
    result["interpretation"].append(
        "The v274 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jdinput_controller_anchor_count": jdinput_count,
            },
            sort_keys=True,
        )
    )
