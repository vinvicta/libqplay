#!/usr/bin/env python3
"""Create the v331 checkpoint for the residual static-variable pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v331"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v330"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "be32d09e08a76b3641beff951644ec78167fcc2735d5fc5ea58f9ee12acf97a1"
EXPECTED_DATABASE_SHA256 = "f6bb72c43b0022b372d6d98e4143aa920a7e3c43cd5a89ede10e7510cd00178c"
EXPECTED_ANCHOR_ARTIFACT = "spectron_tscript_var_residual_manual_translation_anchors_20260829"
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v330")
    require(anchors.get("artifact") == EXPECTED_ANCHOR_ARTIFACT, "unexpected TScriptVar anchor artifact")
    require(anchors.get("network_contacted") is False, "TScriptVar anchors contacted the network")
    require(anchors.get("summary") == EXPECTED_ANCHOR_SUMMARY, "TScriptVar anchor summary changed")
    require([row["proposed_name"] for row in anchors["anchors"]] == EXPECTED_ALIASES, "TScriptVar alias order changed")
    require([row["spectron_ea"] for row in anchors["anchors"]] == EXPECTED_TARGETS, "TScriptVar target order changed")
    require(
        application.get("apply")
        and application.get("expected_artifact") == EXPECTED_ANCHOR_ARTIFACT
        and application.get("anchor_count") == 22
        and application.get("resolved_count") == 22
        and application.get("renamed_count") == 22
        and application.get("comments_added") == 22
        and application.get("failure_count") == 0
        and application.get("saved")
        and application.get("verified")
        and application.get("verified_name_count") == 22,
        "TScriptVar alias application did not pass",
    )
    require(
        verification.get("verified")
        and verification.get("expected_artifact") == EXPECTED_ANCHOR_ARTIFACT
        and verification.get("anchor_count") == 22
        and verification.get("verified_name_count") == 22
        and verification.get("failure_count") == 0
        and verification.get("function_count") == 11707,
        "TScriptVar alias reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        "v331 dynamic boundary audit changed",
    )
    require(
        ...
        "v331 dynamic symbol coverage changed",
    )
    require(
        semantic_map.get("network_contacted") is False
        and semantic_map.get("summary") == EXPECTED_SEMANTIC_SUMMARY,
        "v331 semantic map changed",
    )
    require(feature_export.get("function_count") == 11707, "v331 feature export count changed")
    require(args.database.is_file(), "database path is not a regular file")
    database_sha256 = sha256_path(args.database)
    require(database_sha256 == EXPECTED_DATABASE_SHA256, "v331 database hash changed")
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v331_tscript_var_residual_anchor_artifact": str(args.anchor_artifact),
        "v331_tscript_var_residual_anchor_artifact_sha256": sha256_path(args.anchor_artifact),
        "v331_tscript_var_residual_application_report": str(args.application_report),
        "v331_tscript_var_residual_application_report_sha256": sha256_path(args.application_report),
        "v331_tscript_var_residual_verification_report": str(args.verification_report),
        "v331_tscript_var_residual_verification_report_sha256": sha256_path(args.verification_report),
        "v331_name_audit": str(args.name_audit),
        "v331_name_audit_sha256": sha256_path(args.name_audit),
        "v331_boundary_audit": str(args.boundary_audit),
        "v331_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v331_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v331_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v331_semantic_map": str(args.semantic_map),
        "v331_semantic_map_sha256": sha256_path(args.semantic_map),
        "v331_feature_export": str(args.feature_export),
        "v331_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["tscript_var_residual_translation_v331"] = {
    ...
    result["name_coverage_audit_v331"] = {
    ...
    result["dynamic_function_boundary_audit_v331"] = {
    ...
    result["dynamic_symbol_coverage_audit_v331"] = {
    ...
    result["semantic_function_translation_v331"] = {
    ...
    result["interpretation"].append(
        "The v331 pass adds 22 reviewed aliases for the residual TScriptUniverse initializer, TGraalPlayersArrayVar destructor pair, TStaticVar and TActionScriptVar factories, and their property and object destructor families."
    )
    result["interpretation"].append(
        "Every v331 row has direct source and target Hex-Rays pseudocode. Ten rows are exact normalized feature matches, while the remaining twelve differ only in register-detail allocation after obfuscation."
    )
