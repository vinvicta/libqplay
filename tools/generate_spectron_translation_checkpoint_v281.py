#!/usr/bin/env python3
"""Create the v281 checkpoint from v280 and the reopened jmemmgr report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v281"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v280"
...
def main() -> None:
    ...
    parser.add_argument("--jmemmgr-anchor", required=True, type=Path)
    parser.add_argument("--jmemmgr-verification", required=True, type=Path)
    ...
    jmemmgr_anchor = load(args.jmemmgr_anchor)
    jmemmgr_count = jmemmgr_anchor["summary"]["anchor_count"]
    jmemmgr_report = checked_report(
        args.jmemmgr_verification, jmemmgr_count, "libjpeg memory manager"
    )
    ...
    result["inputs"]["v281_feature_export"] = str(args.feature_export)
    result["inputs"]["v281_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jmemmgr_anchors"] = anchor_record(
        jmemmgr_anchor,
        args.jmemmgr_anchor,
        args.jmemmgr_verification,
        jmemmgr_report,
    )
    result["interpretation"].append(
        "The v281 database adds eleven high-confidence libjpeg jmemmgr role anchors: alloc_small, alloc_large, alloc_sarray, alloc_barray, realize_virt_arrays, request_virt_sarray, request_virt_barray, access_virt_barray, access_virt_sarray, free_pool, and self_destruct."
    )
    result["interpretation"].append(
        "The target memory-manager initializer preserves the source public method table and assigns the allocation, virtual-array, pool, and teardown methods by slot."
    )
    result["interpretation"].append(
        "The v281 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jmemmgr_anchor_count": jmemmgr_count,
            },
            sort_keys=True,
        )
    )
