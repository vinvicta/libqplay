#!/usr/bin/env python3
"""Carry the stable semantic map onto the v333 THashIntVar name pass."""
...
def main() -> None:
    ...
    if target_features.get("function_count") != 11707:
        raise ValueError("v333 target feature count changed")
    if anchors.get("artifact") != "spectron_hashintvar_residual_manual_translation_anchors_20260829":
        raise ValueError("unexpected v333 anchor artifact")
    ...
    result["scope"] = (
        "stable 1.8-to-Spectron semantic translation map carried from v332; "
        "v333 adds reviewed THashIntVar destructor names without changing function bodies"
    )
    ...
    result["inputs"]["hashintvar_anchor_artifact"] = str(args.anchor_artifact)
    result["inputs"]["hashintvar_anchor_artifact_sha256"] = sha256_path(args.anchor_artifact)
    result["carried_forward"] = {
        "from_artifact": str(args.parent_map),
        "from_artifact_sha256": sha256_path(args.parent_map),
        "reason": (
            "The v333 pass changes only two target names and does not alter the "
            "function bodies or target function count. The reviewed semantic map "
            "therefore remains the v332 map, with its stable source snapshot and "
            "target name provenance retained explicitly."
        ),
