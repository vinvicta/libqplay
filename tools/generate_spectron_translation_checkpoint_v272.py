#!/usr/bin/env python3
"""Create the v272 checkpoint from v271 and the reopened residual reports."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v272"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v271"
...
def main() -> None:
    ...
    parser.add_argument("--zlib-anchor", required=True, type=Path)
    parser.add_argument("--zlib-verification", required=True, type=Path)
    ...
    zlib_anchor = load(args.zlib_anchor)
    zlib_count = zlib_anchor["summary"]["anchor_count"]
    zlib_report = checked_report(
        args.zlib_verification, zlib_count, "zlib inflate_fast"
    )
    ...
    result["inputs"]["v272_feature_export"] = str(args.feature_export)
    result["inputs"]["v272_feature_export_sha256"] = sha256_path(args.feature_export)
    result["zlib_inflate_fast_anchor"] = anchor_record(
        zlib_anchor,
        args.zlib_anchor,
        args.zlib_verification,
        zlib_report,
    )
    result["interpretation"].append(
        "The v272 database adds one high-confidence zlib inflate_fast role anchor at target 0x297764. Both source and target functions retained default IDA names, so the artifact records the inferred library role separately from those current names."
    )
    result["interpretation"].append(
        "The source and target inflate_fast helpers share the zlib error strings, Huffman decode and backreference copy logic, caller position, complete feature metrics except register allocation detail, and a final reopened target name of v18_zlib_inflate_fast."
    )
    result["interpretation"].append(
        "The v272 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and target-only labels."
