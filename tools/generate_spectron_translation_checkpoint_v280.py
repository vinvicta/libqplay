#!/usr/bin/env python3
"""Create the v280 checkpoint from v279 and the reopened jerror report."""
...
ARTIFACT = "spectron_translation_checkpoint_20260828_v280"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260828_v279"
...
def main() -> None:
    ...
    parser.add_argument("--jerror-anchor", required=True, type=Path)
    parser.add_argument("--jerror-verification", required=True, type=Path)
    ...
    jerror_anchor = load(args.jerror_anchor)
    jerror_count = jerror_anchor["summary"]["anchor_count"]
    jerror_report = checked_report(
        args.jerror_verification, jerror_count, "libjpeg error handling"
    )
    ...
    result["inputs"]["v280_feature_export"] = str(args.feature_export)
    result["inputs"]["v280_feature_export_sha256"] = sha256_path(args.feature_export)
    result["jerror_anchors"] = anchor_record(
        jerror_anchor,
        args.jerror_anchor,
        args.jerror_verification,
        jerror_report,
    )
    result["interpretation"].append(
        "The v280 database adds five high-confidence libjpeg jerror role anchors: emit_message, reset_error_mgr, format_message, output_message, and error_exit."
    )
    result["interpretation"].append(
        "The target jpeg_std_error initializer preserves the standard callback order for message emission, error-state reset, message formatting, output, and fatal exit handling."
    )
    result["interpretation"].append(
        "The v280 residual pass reduced the default sub_ count to %d while preserving all prior verified aliases and runtime labels."
        ...
        json.dumps(
            {
                ...
                "jerror_anchor_count": jerror_count,
            },
            sort_keys=True,
        )
    )
