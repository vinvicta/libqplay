#!/usr/bin/env python3
"""Create the v347 checkpoint for the encoded-string target-only pass."""
...
ARTIFACT = "spectron_translation_checkpoint_20260829_v347"
PARENT_ARTIFACT = "spectron_translation_checkpoint_20260829_v346"
EXPECTED_BINARY_SHA256 = "f57f7da48bcddf3738f15502328b36032313ad760eea04c5cc19ef82b4232219"
EXPECTED_PARENT_DATABASE_SHA256 = "bfb7f36be1a572c5428192c90ee3288035805a2e34b7ead439437c4b1ccf2392"
EXPECTED_DATABASE_SHA256 = "fe1bbbdf27b25b2fe13d088fb01944a624e8fe8a11898a377ff66f49b892a59b"
EXPECTED_LABEL_ARTIFACT = "spectron_encoded_string_target_only_labels_20260829"
EXPECTED_LABEL_SUMMARY = {
    "defined_dynamic_symbol_count": 19,
    "exact_metric_collision_target_count": 3,
    "high_confidence_count": 19,
    "label_count": 19,
    "normalized_10_collision_target_count": 3,
    "normalized_11_collision_target_count": 3,
    "semantic_target_claimed_count": 0,
...
EXPECTED_DYNAMIC_SUMMARY = {
    ...
    "name_match_counts": {
        "item_name_match": 1657,
        "item_name_mismatch": 5113,
        "value_name_match": 1657,
        "value_name_mismatch": 5113,
    },
    "named_dynamic_symbol_count": 6770,
    "status_counts": {
        "exact_retained_dynamic_name": 1657,
...
def main() -> None:
    ...
    require(parent["database"]["sha256"] == EXPECTED_PARENT_DATABASE_SHA256, "parent database hash is not v346")
    require(labels.get("artifact") == EXPECTED_LABEL_ARTIFACT, "unexpected encoded-string label artifact")
    require(labels.get("network_contacted") is False, "encoded-string labels contacted the network")
    require(labels.get("summary") == EXPECTED_LABEL_SUMMARY, "encoded-string label summary changed")
    require(len(labels.get("labels", [])) == 19, "encoded-string label count changed")
    require(
        application.get("apply")
        and application.get("expected_artifact") == EXPECTED_LABEL_ARTIFACT
        and application.get("label_count") == 19
        and application.get("resolved_count") == 19
        and application.get("renamed_count") == 19
        and application.get("comments_added") == 19
        and application.get("failure_count") == 0
        and application.get("saved"),
        "encoded-string label application did not pass",
    )
    require(
        ...
        and verification.get("label_count") == 19
        and verification.get("verified_name_count") == 19
        and verification.get("failure_count") == 0,
        "encoded-string label reopen verification did not pass",
    )
    require(
    ...
    require(
        ...
        == EXPECTED_BOUNDARY,
        "v347 dynamic boundary audit changed",
    )
    require(
        ...
        "v347 dynamic symbol coverage changed",
    )
    require(
    ...
    result["inputs"] = {
        **parent.get("inputs", {}),
        "v347_encoded_string_label_artifact": str(args.label_artifact),
        "v347_encoded_string_label_artifact_sha256": sha256_path(args.label_artifact),
        "v347_encoded_string_application_report": str(args.application_report),
        "v347_encoded_string_application_report_sha256": sha256_path(args.application_report),
        "v347_encoded_string_verification_report": str(args.verification_report),
        "v347_encoded_string_verification_report_sha256": sha256_path(args.verification_report),
        "v347_name_audit": str(args.name_audit),
        "v347_name_audit_sha256": sha256_path(args.name_audit),
        "v347_boundary_audit": str(args.boundary_audit),
        "v347_boundary_audit_sha256": sha256_path(args.boundary_audit),
        "v347_dynamic_symbol_coverage": str(args.dynamic_symbol_coverage),
        "v347_dynamic_symbol_coverage_sha256": sha256_path(args.dynamic_symbol_coverage),
        "v347_semantic_map": str(args.semantic_map),
        "v347_semantic_map_sha256": sha256_path(args.semantic_map),
        "v347_feature_export": str(args.feature_export),
        "v347_feature_export_sha256": sha256_path(args.feature_export),
    }
    result["encoded_string_target_only_v347"] = {
        ...
        "exact_metric_collision_target_count": labels["summary"]["exact_metric_collision_target_count"],
        "normalized_11_collision_target_count": labels["summary"]["normalized_11_collision_target_count"],
        "normalized_10_collision_target_count": labels["summary"]["normalized_10_collision_target_count"],
        "semantic_target_claimed_count": labels["summary"]["semantic_target_claimed_count"],
        "reopen_verified": verification["verified"],
    }
    result["name_coverage_audit_v347"] = {
    ...
    result["dynamic_function_boundary_audit_v347"] = {
    ...
    result["dynamic_symbol_coverage_audit_v347"] = {
    ...
    result["semantic_function_translation_v347"] = {
    ...
    result["interpretation"].append(
        "The v347 pass adds nineteen high-confidence target-only descriptive labels for the C8THgaTQxF bridge and CanTfaz6bZ copy-on-write XOR-encoded string helper cluster. It does not add source-backed aliases because this target subsystem has no one-to-one 1.8 function correspondence."
    )
    result["interpretation"].append(
        "Three ordinary source feature collisions are retained in the label artifact as metrics only. Function count, source-backed semantic coverage, and exact IDA dynamic boundaries remain unchanged."
    )
